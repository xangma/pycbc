"""Fresh complete-executable timings after unchanged-CPU scientific qualification."""
import copy
import datetime
import fcntl
import hashlib
...
def main():
    ...
    with LOCK.open() as lock:
        ...
        runtime_metadata = {}
        runtime_fields = {
            'search/filter_rate_per_core', 'search/run_time',
        ...
        with h5py.File(qualified['original-cpu'] / 'triggers.hdf') as a, h5py.File(qualified['proposed-cpu'] / 'triggers.hdf') as b:
            def visit(name, obj):
                if isinstance(obj, h5py.Dataset):
                    other = b['H1'][name]
                    av, bv = obj[...], other[...]
                    result = bool(av.dtype == bv.dtype and av.shape == bv.shape and av.tobytes() == bv.tobytes())
                    (runtime_metadata if name in runtime_fields else cpu_exact)[name] = result
        ...
        save(ROOT / 'summary.json', dict(cpu_preserved=cpu_preserved, cpu_exact_datasets=cpu_exact,
            runtime_metadata_exact=runtime_metadata,
            runtime_metadata_policy='Exclude only four elapsed-time-derived H1/search fields from byte-exact scientific comparisons; all scientific tolerances unchanged.',
        ...
        assert scientific_pass, 'Scientific qualification failed; no timings are allowed'
        measurements = {arm: [] for arm in ROUTES}
        assert CONFIG['repeats'] == 4 and len(CONFIG['ordering']) == 4
        ...
        for arm, values in measurements.items():
            ...
            rows[arm] = dict(source=CONFIG['source_commits'][ROUTES[arm][0]], scheme=ROUTES[arm][1],
                ...
                template_seconds_per_wall_second=384 * 1904 / median, trigger_count=counts[arm])
        summary = read(ROOT / 'summary.json')
        summary.update(scope=CONFIG['scope'], timing_boundary=CONFIG['timing_boundary'], arms=rows,
            equal_output_speedup_eligible=True, timing_policy=CONFIG['qualification_policy'],
            timing_outputs_pass=True, sustained_capacity_established=False)
