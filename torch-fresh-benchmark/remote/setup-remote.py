...
try:
    ...
    with LOCK.open() as lock:
        ...
        for name, head in CONFIG['source_commits'].items():
            ...
            version_env.update(PYTHONPATH=str(source), PYTHONDONTWRITEBYTECODE='1')
            subprocess.run([sys.executable, '-B', str(ROOT / 'generate-version.py'), head], cwd=source, env=version_env, check=True)
            ...
            save(ROOT / (name + '-build.json'), dict(commit=head, source=str(source),
                ...
                method='Reused frozen binaries after exact native-source and binary-hash verification; generated version metadata with source setup.py get_version_info',
