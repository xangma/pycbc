...
try:
    ...
    assert all(not ref['metadata']['issues'] for ref in references.values())
        ...
        for repeat, order in enumerate(ORDER, 1):
            for name in order:
                ...
                record = dict(case=case, source=str(source), head=HEADS[name], command=command,
                              state='running', executable_cli=cli,
                              source_info=dict(commit=HEADS[name], status='', tracked_diff=''),
                              input_sha256={p: sha(p) for p in [*CONFIG['input_pins'], cli[0]]},
                ...
                sampler.take('after')
                record.update(returncode=code, state='complete' if code == 0 else 'failed',
                              source_status_after=subprocess.check_output(['git', '-C', str(source), 'status', '--porcelain'], text=True),
                              input_sha256_after={p: sha(p) for p in record['input_sha256']},
