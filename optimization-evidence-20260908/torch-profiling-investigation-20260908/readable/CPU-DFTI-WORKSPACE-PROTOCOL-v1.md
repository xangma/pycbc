# CPU DFTI workspace policy protocol v1

Status: bounded diagnostic specification, 2026-09-08; freeze implementation and accept this schedule before native execution. The optimizer owns all `len` operations and the benchmark lock. The profiling peer reviews local artifacts. Use a new `torch-cpu-dfti-workspace-20260908` artifact root; preserve every sealed earlier campaign. This experiment does not authorize source integration or publication.
