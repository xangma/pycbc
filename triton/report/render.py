#!/usr/bin/env python3
"""Qualify and render the complete 60-worker TaylorF2 Triton experiment."""
...
def load_campaign(args):
    ...
    for condition, message in (
        ...
        (
            ...
            "campaign is not the requested full 60-worker matrix",
        ),
        (
    ...
    if (
        ...
        global_problems.append(
            "manifest must contain each of the 60 worker jobs exactly once"
        )
    groups_expected = {
    ...
    if set(stored_summary.get("groups", {})) != groups_expected:
        global_problems.append(
            "summary does not contain the expected 20 route/workload groups"
        )
    ...
    if missing:
        global_problems.append(
            f"{len(missing)} of the 60 raw worker artifacts are missing; plots suppressed"
        )
    if all_workers:
        signatures = {
            json.dumps(
...
def markdown(report):
    ...
    lines = [
        ...
        f"{report['qualified_group_count']}/20 route/workload groups qualified from {report['raw_worker_files']}/60 worker files.",
        "",
    ...
    lines.extend(
        [
            "## Qualification",
            "",
            "The renderer verifies all 60 raw worker records and independently recomputes the five-group worker medians and campaign estimates. "
...
def main():
    ...
    report["plots_generated"] = (
        ...
        and report["raw_worker_files"] == 60
    )
    ...
    return int(report["qualified_group_count"] != 20)
