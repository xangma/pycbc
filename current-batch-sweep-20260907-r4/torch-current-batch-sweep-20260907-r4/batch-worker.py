...
outputs.npy (complex64), oracle.npy (complex128), and result.json. All other
...
POLICY = {
    "version": 2,
    "normalized_complex_snr": {"rtol": 0.0, "atol": 1e-3},
    "pointwise_acceptance": "every normalized complex sample versus independent oracle AND standard MKL reference; every row sigmasq versus independent oracle",
    "raw_complex_v1_audit": "diagnostic only; original R3 verdicts remain unchanged",
    "normwise_metrics": "diagnostic only; no acceptance threshold",
    ...
    "sources": {
        "normalized_complex_snr": "DRAFT extension of existing absolute trigger-SNR budget 1e-3 to every complex sample; independent oracle and MKL compatibility both required; adoption is a separate decision",
        "oracle": "NumPy complex128 product of captured complex64 inputs, unnormalized np.fft.ifft*N; sigma=4*df*sum((bank.real64**2+bank.imag64**2)/PSD64)",
...
def checked_positive(value, name):
    value = float(value)
...
def normalization_record(norm, sigmasq, source):
    record = {"norm": checked_positive(norm, "normalization"),
              "sigmasq": checked_positive(sigmasq, "sigmasq"), "source": source}
    return {**record, "sha256": canonical_hash(record)}


def validate_normalization(record):
    expected = normalization_record(record["norm"], record["sigmasq"], record["source"])
    if record != expected:
        raise ValueError("normalization hash/metadata mismatch")
    return record


def oracle_normalization(bank_row, psd, delta_f):
    """Independent sigma; neither abs(complex64)**2 nor the fixture callback."""
    if bank_row.shape != psd.shape or not np.all(np.isfinite(bank_row)):
        raise ValueError("invalid oracle bank shape/values")
    if not np.all(np.isfinite(psd) & (psd > 0)):
        raise ValueError("oracle PSD must be finite and positive")
    real = bank_row.real.astype(np.float64)
    imag = bank_row.imag.astype(np.float64)
    sgm = 4 * delta_f * np.sum((real * real + imag * imag)
                              / psd.astype(np.float64), dtype=np.float64)
    sgm = checked_positive(sgm, "oracle sigmasq")
    return normalization_record(4 * delta_f / math.sqrt(sgm), sgm,
                                "independent_float64_power_sum")


def oracle_row(bank_row, strain, size):
    """Transform the double product of the exact stored single-precision inputs."""
    if bank_row.shape != strain.shape or len(bank_row) > size:
        raise ValueError("oracle correlation shape mismatch")
    correlation = np.zeros(size, dtype=np.complex128)
    correlation[:len(bank_row)] = (bank_row.astype(np.complex128).conj()
                                   * strain.astype(np.complex128))
    output = np.fft.ifft(correlation) * size
    if not np.all(np.isfinite(output)):
        raise ValueError("nonfinite independent oracle")
    return output


def validated_row(array, block, row_id, expected_hash, dtype, size, name):
    row = array[block, row_id]
    if row.shape != (size,) or row.dtype != np.dtype(dtype):
        raise ValueError(f"{name} row shape/dtype mismatch")
    if digest_bytes(row) != expected_hash:
        raise ValueError(f"{name} row hash mismatch {block}/{row_id}")
    if not np.all(np.isfinite(row)):
        raise ValueError(f"{name} row is nonfinite")
    return row


def relative_residual(actual, reference):
...
def complex_snr_metrics(actual, reference):
    """Return a complex-distance comparison and mask for the union of two gates."""
    if actual.shape != reference.shape or actual.ndim != 1:
        raise ValueError("complex SNR shape mismatch")
    finite_a, finite_r = np.isfinite(actual), np.isfinite(reference)
    with np.errstate(invalid="ignore", over="ignore"):
        error = np.abs(actual - reference)
    finite = finite_a & finite_r & np.isfinite(error)
    limit = POLICY["normalized_complex_snr"]["atol"]
    failed = ~finite | (error > limit)
    max_error = float(np.max(error[finite])) if np.any(finite) else 0.0
    return {"samples": actual.size, "failed_samples": int(np.count_nonzero(failed)),
            "nonfinite_actual": int(np.count_nonzero(~finite_a)),
            "nonfinite_reference": int(np.count_nonzero(~finite_r)),
            "max_abs_error": max_error, "max_tolerance_ratio": max_error / limit,
            "relative_l2_error": relative_residual(actual, reference)}, failed


def qualified_row_metrics(actual, oracle, reference, normalization, oracle_norm,
                          reference_norm, correlation=None):
    """V2 acceptance; legacy raw comparisons and all normwise errors are audits."""
    for record in (normalization, oracle_norm, reference_norm):
        validate_normalization(record)
    with np.errstate(invalid="ignore", over="ignore"):
        normalized = actual.astype(np.complex128) * normalization["norm"]
        truth = oracle * oracle_norm["norm"]
        compatibility = reference.astype(np.complex128) * reference_norm["norm"]
    oracle_metrics, oracle_failed = complex_snr_metrics(normalized, truth)
    compat_metrics, compat_failed = complex_snr_metrics(normalized, compatibility)
    sgm_error = abs(normalization["sigmasq"] - oracle_norm["sigmasq"])
    sgm_passed = sgm_error <= POLICY["sigmasq"]["rtol"] * oracle_norm["sigmasq"]
    failures = oracle_failed | compat_failed | (not sgm_passed)
    raw = row_metrics(actual, reference)
    raw["classification"] = "audit_only_not_v2_acceptance"
    diagnostic = {"classification": "diagnostic_only_no_threshold",
                  "oracle_relative_l2": relative_residual(actual.astype(np.complex128), oracle),
                  "normalized_oracle_relative_l2": oracle_metrics["relative_l2_error"],
                  "same_correlation_fft_relative_l2": None}
    if correlation is not None:
        if correlation.shape != actual.shape or correlation.dtype != np.dtype("complex64"):
            raise ValueError("captured correlation shape/dtype mismatch")
        if not np.all(np.isfinite(correlation)):
            raise ValueError("nonfinite captured correlation")
        same_corr = np.fft.ifft(correlation.astype(np.complex128)) * actual.size
        diagnostic["same_correlation_fft_relative_l2"] = relative_residual(
            actual.astype(np.complex128), same_corr)
    return {"samples": actual.size, "failed_samples": int(np.count_nonzero(failures)),
            "sha256": digest_bytes(actual),
            "reference_sha256": digest_bytes(reference),
            "nonfinite_actual": oracle_metrics["nonfinite_actual"],
            "nonfinite_reference": max(oracle_metrics["nonfinite_reference"],
                                       compat_metrics["nonfinite_reference"]),
            "max_abs_error": max(oracle_metrics["max_abs_error"], compat_metrics["max_abs_error"]),
            "max_tolerance_ratio": max(oracle_metrics["max_tolerance_ratio"],
                                       compat_metrics["max_tolerance_ratio"]),
            "normalization": normalization,
            "oracle": {"sha256": digest_bytes(oracle), "normalization": oracle_norm},
            "normalized_oracle": oracle_metrics, "normalized_mkl_compatibility": compat_metrics,
            "sigmasq_oracle": {"passed": bool(sgm_passed), "relative_error": sgm_error / oracle_norm["sigmasq"]},
            "raw_complex_v1_audit": raw, "normwise_diagnostics": diagnostic}


def load_reference_arrays(directory, shape):
    arrays = []
    for name, dtype in (("outputs", "complex64"), ("oracle", "complex128")):
...
def validate_reference_metadata(reference, result, args):
    required = {"status": "pass", "mode": "qualify", "route": "branch_standard", "batch": 1,
                "input_sha256": result["input_sha256"], "tolerance_policy": POLICY,
                "policy_sha256": canonical_hash(POLICY), "worker_sha256": result["worker_sha256"]}
    for key, value in required.items():
        if reference.get(key) != value:
            raise ValueError(f"reference {key} differs or reference failed")
    if (reference["source"]["revision"] != EXPECTED_HEAD
            or reference["source"]["file_sha256"] != result["source"]["file_sha256"]
            or reference["source"]["tracked_dirty"]):
        raise ValueError("reference source mismatch")
    blocks = reference["pointwise_blocks"]
    if len(blocks) != args.num_blocks:
        raise ValueError("reference block coverage mismatch")
    for b, block in enumerate(blocks):
        if (block["block"] != b or not block["processed_once"]
                or block["processing_counts"] != [1] * args.bank_size
                or block["failed_samples"] != 0
                or block["samples"] != args.bank_size * args.size
                or len(block["rows"]) != args.bank_size):
            raise ValueError("reference row coverage/qualification mismatch")
        for i, row in enumerate(block["rows"]):
            if (row["template_id"] != 1000+i or row["samples"] != args.size
                    or row["failed_samples"] or row["nonfinite_actual"]
                    or row["nonfinite_reference"] or not row["sigmasq_oracle"]["passed"]
                    or row["normalized_oracle"]["failed_samples"]
                    or row["normalized_mkl_compatibility"]["failed_samples"]):
                raise ValueError("reference row acceptance mismatch")
            validate_normalization(row["normalization"])
            validate_normalization(row["oracle"]["normalization"])
...
class NormalizationCapture:
    """Qualification-only observation of the pinned production return paths.

    Bulk paths expose the arrays actually used in their return frame. The scalar
    path is verified by code identity and call line, and records every callback,
    including templates below threshold. No production result is substituted.
    """
    def __init__(self, args, filt, matchedfilter):
        self.args, self.filt = args, filt
        self.original_process = filt._process_batch
        self.code = self.original_process.__func__.__code__
        self.original_callbacks = [(t, t.sigmasq) for g in filt.tgroups for t in g]
        self.active = None
        self.records = None
    ...
    def __enter__(self):
        if sys.getprofile() is not None or sys.gettrace() is not None:
            raise ValueError("qualification refuses an existing trace/profile hook")
        lines, first = inspect.getsourcelines(self.original_process)
        candidates = [first+i for i, line in enumerate(lines)
                      if line.strip() == "sgm = htilde.sigmasq(psd)"]
        if len(candidates) != 1:
            raise ValueError("unknown scalar sigmasq source path")
        self.scalar_line = candidates[0]
        try:
            for template, callback in self.original_callbacks:
                def observed(psd, callback=callback, template=template):
                    value = callback(psd)
                    caller = inspect.currentframe().f_back
                    try:
                        # Bulk fallback generators also invoke callbacks. Their
                        # values are observed later in the actual bulk arrays.
                        if caller.f_code is self.code and caller.f_lineno == self.scalar_line:
                            self.validate_group(caller.f_locals)
                            if caller.f_locals.get("htilde") is not template:
                                raise ValueError("wrong scalar template identity")
                            self.observe_scalar(template, value)
                    finally:
                        del caller
                    return value
                template.sigmasq = observed
            self.installed = True
            sys.setprofile(self.observe_return)
            return self
    ...
    def __exit__(self, *_exc):
        if self.installed:
            sys.setprofile(None)
            self.installed = False
        self.filt._process_batch = self.original_process
        for template, callback in self.original_callbacks:
            template.sigmasq = callback
        self.active = None

    def begin_group(self, index):
        if self.active is not None or not 0 <= index < len(self.filt.tgroups):
            raise ValueError("overlapping or unknown group normalization capture")
        self.active = index
        self.scalar = {}
        self.return_calls = 0
        self.records = None

    def validate_group(self, local):
        if (self.active is None or local.get("self") is not self.filt
                or local.get("group_id") != self.active
                or local.get("tgroup") is not self.filt.tgroups[self.active]):
            raise ValueError("wrong-group normalization observation")

    def observe_scalar(self, template, value):
        group = self.filt.tgroups[self.active]
        if not any(template is t for t in group) or template.id in self.scalar:
            raise ValueError("wrong-group or duplicate scalar normalization")
        sgm = checked_positive(value, "observed scalar sigmasq")
        norm = 4.0 * template.delta_f / (sgm ** 0.5)
        self.scalar[template.id] = normalization_record(norm, sgm, "observed_scalar_sigmasq_callback")

    def observe_return(self, frame, event, returned):
        if event != "return" or frame.f_code is not self.code:
            return
        local = frame.f_locals
        if (self.active is None and local.get("self") is self.filt
                and self.filt.block_id == len(self.filt.tgroups)
                and returned == (None, None)):
            return  # Public process_data's end-of-groups sentinel.
        self.validate_group(local)
        self.return_calls += 1
        if (self.return_calls != 1 or not isinstance(returned, tuple) or len(returned) != 2
                or not isinstance(returned[0], dict) or not isinstance(returned[1], list)
                or returned[0] is not local.get("result")
                or returned[1] is not local.get("veto_info")):
            raise ValueError("missing/duplicate/aborted production group return")
        group = self.filt.tgroups[self.active]
        if "ondevice_result" not in local:
            raise ValueError("unknown production normalization branch")
        if local["ondevice_result"] is not None:
            source = "observed_ondevice_return_locals"
        elif "batch_magnitudes" not in local:
            raise ValueError("unknown production normalization branch")
        elif local["batch_magnitudes"] is not None:
            source = "observed_bulk_magnitude_return_locals"
        else:
            if set(self.scalar) != {t.id for t in group}:
                raise ValueError("missing scalar normalization observations")
            self.records = [self.scalar[t.id] for t in group]
            return
        if self.args.route == "branch_standard" or self.scalar:
            raise ValueError("unexpected bulk normalization branch")
        norms = np.asarray(local.get("norms")).copy()
        sigmasqs = np.asarray(local.get("sigmasqs")).copy()
        if norms.shape != (len(group),) or sigmasqs.shape != (len(group),):
            raise ValueError("bulk normalization shape mismatch")
        self.records = [normalization_record(norm, sgm, source)
                        for norm, sgm in zip(norms, sigmasqs)]

    def finish_group(self):
        if self.active is None or self.return_calls != 1 or self.records is None:
            raise ValueError("missing production normalization return observation")
...
class Capture:
    ...
    def __init__(self, args, filt, output_map, ref_result, result, oracle_map, material):
        self.args, self.filt = args, filt
        self.output_map, self.reference = output_map, ref_result
        self.result = result
        self.oracle_map, self.material = oracle_map, material
        self.oracle_norms = [oracle_normalization(row, material["psd"], material["delta_f"])
                             for row in material["bank"]]
        self.block = 0
        self.indices = {}
    ...
    def capture(self, group_index, triggers, veto, normalizations):
        group = self.filt.tgroups[group_index]
        if len(normalizations) != len(group):
        ...
        for start in range(0, len(group), row_chunk):
            ...
            for offset, row in enumerate(values):
                ...
                if not 0 <= row_id < self.args.bank_size or self.counts[row_id]:
                    raise ValueError("unknown or duplicate captured template")
                self.counts[row_id] += 1
                norm = validate_normalization(normalizations[start+offset])
                oracle_norm = self.oracle_norms[row_id]
                if self.reference is None:
                    self.output_map[self.block, row_id] = row
                    truth = oracle_row(self.material["bank"][row_id],
                                       self.material["blocks"][self.block], self.args.size)
                    self.oracle_map[self.block, row_id] = truth
                    reference_row, reference_norm = row, norm
                else:
                    metadata = self.reference["pointwise_blocks"][self.block]["rows"][row_id]
                    if metadata["template_id"] != 1000 + row_id:
                        raise ValueError("reference row identity mismatch")
                    reference_row = validated_row(self.output_map, self.block, row_id,
                                                  metadata["sha256"], "complex64", self.args.size, "reference")
                    truth = validated_row(self.oracle_map, self.block, row_id,
                                          metadata["oracle"]["sha256"], "complex128", self.args.size, "oracle")
                    reference_norm = validate_normalization(metadata["normalization"])
                    if validate_normalization(metadata["oracle"]["normalization"]) != oracle_norm:
                        raise ValueError("oracle normalization differs from independent inputs")
                # Capture the actual correlation for a separate FFT-only diagnostic.
                correlation = group[start+offset].cout.numpy().copy()
                metrics = qualified_row_metrics(row, truth, reference_row, norm,
                                                oracle_norm, reference_norm, correlation)
        ...
        for _snr, _norm, peak, template, _strain in veto:
            ...
            row = self.rows[int(template.id) - 1000]
            if row is None or float(_norm) != row["normalization"]["norm"]:
                raise ValueError("trigger normalization disagrees with captured row")

    def finish_block(self):
        rows = [r for r in self.rows if r is not None]
        summary = reduce_rows(rows)
        summary["raw_complex_v1_audit"] = reduce_rows([r["raw_complex_v1_audit"] for r in rows])
        summary["raw_complex_v1_audit"]["classification"] = "audit_only_not_v2_acceptance"
        summary["normalization_failed_rows"] = sum(not r["sigmasq_oracle"]["passed"] for r in rows)
        summary["normalized_oracle"] = reduce_rows([r["normalized_oracle"] for r in rows])
        summary["normalized_mkl_compatibility"] = reduce_rows([r["normalized_mkl_compatibility"] for r in rows])
...
def execute(args, result):
    ...
    if not is_reference:
        ...
        validate_reference_metadata(reference, result, args)
    ...
    if args.mode == "qualify":
        ...
        if is_reference:
            ...
            oracle_map = np.lib.format.open_memmap(args.output_dir / "oracle.npy", mode="w+",
                                                  dtype=np.complex128, shape=shape)
        else:
            output_map, oracle_map = load_reference_arrays(args.reference_dir, shape)
        ...
        result["oracle"] = {"created": is_reference,
                            "directory": str(args.output_dir if is_reference else args.reference_dir),
                            "filename": "oracle.npy", "shape": list(shape), "dtype": "complex128",
                            "quantity": "unnormalized NumPy complex128 IFFT of double product of stored complex64 inputs",
                            "normalization": "independent float64 power/PSD sum; per-row values and hashes in pointwise_blocks"}
    ...
    with context:
        ...
        if args.mode == "qualify":
            ...
            material = {"bank": bank_np, "psd": psd_np, "blocks": data_np, "delta_f": geom["delta_f"]}
            capture = Capture(args, filt, output_map, reference, result, oracle_map, material)
            original = filt._process_batch
            try:
                with NormalizationCapture(args, filt, matchedfilter) as observer:
                    def wrapped():
                        group_index = filt.block_id
                        if group_index >= len(filt.tgroups):
                            return original()
                        observer.begin_group(group_index)
                        triggers, veto = original()
                        norms = observer.finish_group()
                        if triggers is None or triggers is False:
                            raise ValueError("qualification group aborted or skipped")
                        capture.capture(group_index, triggers, veto, norms)
                ...
                if is_reference:
                    output_map.flush()
                    oracle_map.flush()
            result["qualification"] = "reference_created_and_oracle_qualified" if is_reference else "compared_all_samples_and_triggers"
        else:
    ...
    if hashlib.sha256(Path(__file__).read_bytes()).hexdigest() != result["worker_sha256"]:
        raise ValueError("worker changed during execution")
    if (digest_bytes(bank_np) != identity["template_bank_sha256"]
            or digest_bytes(psd_np) != identity["psd_sha256"]
            or digest_bytes(sigma) != identity["sigmasq_sha256"]
            or [digest_bytes(s) for s in data_np] != identity["overwhitened_block_sha256"]):
        raise ValueError("workload inputs changed during execution")
...
def main(argv=None):
    ...
    if any((args.output_dir / name).exists() for name in ("result.json", "outputs.npy", "oracle.npy")):
