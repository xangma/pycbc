...
def render(summary, accuracy, output):
    ...
    caption = ("Shared host len; one numerical thread, CPU 8; CUDA additionally uses one GPU. No reservation.\n"
               "Source 9578a710479b · public LiveBatchMatchedFilter.process_data; timings uninstrumented.\n"
    ...
    with plt.rc_context({"font.size": 10, "svg.hashsalt": "pycbc-r4-batch"}):
        for name in ("throughput", "accuracy"):
            ...
            fig.suptitle("PyCBC R4 batch sweep · " + name, y=0.96, fontsize=18)
            subtitle = ("1,024 templates × 3 blocks; median of 3 fresh-worker medians (5 timed iterations each).\n"
            ...
            for extension in ("png", "svg"):
                ...
                fig.savefig(output / f"batch-{name}.{extension}", dpi=160, metadata=metadata)
            plt.close(fig)
