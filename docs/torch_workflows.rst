...
samples per cell.  It plans 95 attempts and 76 comparisons.  On ``len``, allow
...
``suite_cpu_scaling.json`` is the CPU-only companion campaign for ``len``.  It
...
``run_len.sh`` writes a sealed host-preflight JSON file before starting the
...
On ``len``, launch the run through ``run_len.sh`` after the immutable parity
...
.. code-block:: bash
   ...
   BENCHMARK_EXPECTED_CPU_IDS="$scaling_cpu_set" \
       ...
       "$CURRENT_SOURCE/tools/torch_workflows/run_len.sh"
...
.. code-block:: bash
   ...
   ssh len "root=\$HOME/$ROOT_NAME; run_id=$RUN_ID; \
       log=\$root/launcher-\$run_id.log; cd \$root/current; \
       command=\$root/current/tools/torch_workflows/run_rollout_len.sh; \
       nohup setsid env PYCBC_WORKFLOW_ROOT=\$root RUN_ID=\$run_id \
       \$command >\$log 2>&1 </dev/null & pid=\$!; \
       echo host=len; echo cwd=\$root/current; echo command=\$command; \
...
``ssh len 'tail -n 100 <reported-log>'``; stop the entire rollout and active
stage with the reported negative-PID ``kill`` command.

``run_rollout_len.sh`` attempts all five stages even when an earlier stage
...
ignored-file evidence.  To run only one workflow profile, invoke ``run_len.sh``
