# SEOBNRv4PHM LAL-to-torch traceability

This is the working map for the torch-native `SEOBNRv4PHM` port. The first table
is exhaustive for functions defined in LAL's
`lalsimulation/lib/LALSimIMRSpinPrecEOBv4P.c`. The second table lists the
nontrivial external LAL helper families called by those functions.
...
| `SEOBRotatehIlmFromhJlm` | 4303 | Constant J-to-I rotation. | `pycbc/waveform/seobnrv4phm_torch.py:1031` `_rotate_modes_constant` | DONE | Direct mode-rotation counterpart. |
...
``TorchScheme`` invokes this implementation through both ``get_td_waveform``
and ``get_fd_waveform``; no second implementation gate is required. Unsupported
options and reversed component-mass labels retain the LAL/CPU path.
