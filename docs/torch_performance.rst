...
* **Standard CPU (original_cpu)**: Evaluates waveforms sequentially on host via LALSimulation
  (``XLALSimInspiralChooseFDWaveform``). Total wall time is 115.7 s (calc time 96.6 s).
* **Branch CPU (branch_cpu)**: Clean PR stack execution on host CPU. Total wall time is
  112.6 s (calc time 97.1 s).
* **Torch CUDA (LAL on host, torch_cuda_lal)**: Evaluates waveforms on host CPU via
  ...
  **52.17 s** (1.85× speedup over CPU baseline) and wall time is **62.29 s** (1.86× speedup).
  Host CPU waveform generation consumes ~47 s, severely bottlenecking the pipeline.
* **Torch CUDA (DiffGW compiled, torch_cuda_diffgw)**: Evaluates batched differentiable waveforms
  ...
  Calculation time drops to **4.26 s** (**22.66× speedup** over baseline CPU, **12.25× speedup**
  over ``torch_cuda_lal``) and wall time to **23.31 s** (**4.96× end-to-end speedup** including
...
.. figure:: images/torch/pycbc_inspiral_diffgw_speedup.png
   ...
   host-generated GPU filtering (``torch_cuda_lal``), and fully GPU-resident dynamic
   generation via ``diffgw`` (``torch_cuda_diffgw``).
...
.. code-block:: console
   ...
   # Execute Track 2 benchmark comparing CPU LAL, host LAL + CUDA, and DiffGW
   python tools/bench_inspiral_4arm_campaign.py \
     --track track2 \
     --frame-file docs/_include/H-H1_LOSC_CLN_4_V1-1187007040-2048.gwf \
     --bank-file inputs/bank_512_taylorf2.hdf \
     --arms original_cpu branch_cpu torch_cuda_lal torch_cuda_diffgw \
     --affinity 8 \
