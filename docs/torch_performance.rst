...
Benchmark measurements and plots are maintained separately from the Torch
implementation. See :ref:`torch-optimization-results` for the publication
status and required sources. Each result qualifies only its recorded revision.
