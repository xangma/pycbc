...
.. list-table:: Authoritative Benchmarking & Analysis Toolchain
   ...
   * - ``tools/bench_comprehensive_suite.py``
     - Modular microbenchmarks across 6 workloads:
       ``correlate_ifft``, ``peak_finding``, ``waveform``,
       ``detector_network``, ``relbin_likelihood``, ``synthetic_live_batch``
     - ``artifacts/len_extended_full_suite.json`` (Schema-v2 signed JSON)
   ...
   * - ``tools/generate_torch_performance_plots.py``
     - Dynamic JSON ingestion and automated rendering of 6 publication figures at 300 DPI
...
2. **Running the 4-Route Production Live-Batch Gate:**
...
4. **Verifying Parity & Documentation Build:**
