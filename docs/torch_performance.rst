...
.. figure:: images/torch/pycbc_inspiral_calc_speedup.png
   ...
   using precomputed compressed template banks (Track 1). Dynamic waveform synthesis
   via DiffGW is evaluated separately in Track 2 (Figure 3).
...
* **Standard CPU (original_cpu)**: Evaluates waveforms sequentially on host via LALSimulation
  (``XLALSimInspiralChooseFDWaveform``). Total wall time is 116.1 s (calc time 96.8 s).
* **Branch CPU (branch_cpu)**: Clean PR stack execution on host CPU. Total wall time is
  110.8 s (calc time 95.2 s, 1.02× calc speedup / 1.05× wall speedup).
* **Torch CPU (LAL, torch_cpu)**: Evaluates waveforms sequentially on host via LALSimulation paired
  with single-thread PyTorch CPU matched filtering. Total wall time is 261.9 s (calc time
  227.4 s, 0.43× calc speedup / 0.44× wall speedup vs baseline CPU).
* **Torch CPU (DiffGW compiled, torch_cpu_diffgw)**: Evaluates batched differentiable waveforms
  on CPU via ``diffgw`` compiled with ``torch.compile`` (``--enable-diffgw --enable-diffgw-compile``).
  Steady-state waveform generation runs in **9.75 ms / waveform** (**32.4× speedup** over eager CPU DiffGW),
  with total calc time 340.7 s and wall time 403.6 s (0.28× calc speedup / 0.29× wall speedup)
  due to CPU cache bounds on batched arrays and single-thread IFFT latency.
* **Torch CUDA (LAL on host, torch_cuda_lal)**: Evaluates waveforms on host CPU via
  LALSimulation before streaming to GPU for matched filtering. Calculation time is
  **50.29 s** (1.92× speedup over CPU baseline) and wall time is **59.69 s** (1.94× speedup).
  Host CPU waveform generation consumes ~45 s, severely bottlenecking the pipeline.
* **Torch CUDA (DiffGW compiled, torch_cuda_diffgw)**: Evaluates batched differentiable waveforms
  directly on device via ``diffgw`` compiled with ``torch.compile`` (``--enable-diffgw --enable-diffgw-compile``).
  Calculation time drops to **4.38 s** (**22.10× speedup** over baseline CPU, **11.48× speedup**
  over ``torch_cuda_lal``) and wall time to **23.87 s** (**4.86× end-to-end speedup** including
  JIT pre-compilation during setup).

.. figure:: images/torch/pycbc_inspiral_diffgw_speedup.png
   :alt: Speedup of dynamic GPU waveform generation on RTX 4090
   :align: center
   :width: 95%

   Figure 3: Track 2 offline search performance comparing sequential CPU LALSimulation,
   Torch CPU (LAL), Torch CPU (DiffGW compiled), host-generated GPU filtering
   (``torch_cuda_lal``), and fully GPU-resident dynamic generation via ``diffgw``
   (``torch_cuda_diffgw``).
...
.. code-block:: console
   ...
   # Execute Track 2 benchmark comparing CPU LAL, Torch CPU, host LAL + CUDA, and DiffGW
   python tools/bench_inspiral_4arm_campaign.py \
     --track track2 \
     --frame-file docs/_include/H-H1_LOSC_CLN_4_V1-1187007040-2048.gwf \
     --bank-file inputs/bank_512_taylorf2.hdf \
     --arms original_cpu branch_cpu torch_cpu torch_cpu_diffgw torch_cuda_lal torch_cuda_diffgw \
     --affinity 8 \
