...
* **Bank**: **512 distinct BNS templates** (:math:`1.4 \le m_1 \le 2.5 \, M_\odot`,
  :math:`1.2 \le m_2 \le 1.4 \, M_\odot`, non-spinning).
  Bank SHA256: ``1d0145aa62b21ea907981da315b212e68fa28118196c402d077bdad4a58b212c``.
* **Approximant**: ``TaylorF2`` at 3.5PN phase order (``--order 7``), 30 Hz cutoff,
  4096 Hz sample rate.
* **Waveform Generation**:

  - **CPU arms** (``original_cpu``, ``branch_cpu``): Sequential
    evaluation on host CPU via LALSimulation (``XLALSimInspiralChooseFDWaveform``).
  - **Torch CPU LAL** (``torch_cpu``): Sequential host CPU evaluation via
    LALSimulation paired with PyTorch CPU matched filtering.
  - **Torch CPU DiffGW** (``torch_cpu_diffgw``): Batched evaluation on CPU via
    ``diffgw`` compiled with ``torch.compile``.
  - **Torch CUDA LAL** (``torch_cuda_lal``): Sequential host CPU evaluation via
    LALSimulation, transferring each waveform to GPU for matched filtering.
  - **Torch CUDA DiffGW** (``torch_cuda_diffgw``): On-device batched evaluation via
    ``diffgw`` compiled with ``torch.compile`` (with batch size 64).
...
.. code-block:: console
   ...
   # Standard CPU arms (original_cpu, branch_cpu):
   taskset -c 8 pycbc_inspiral \
     --verbose \
     --frame-files docs/_include/H-H1_LOSC_CLN_4_V1-1187007040-2048.gwf \
     --channel-name H1:LOSC-STRAIN \
   ...
   taskset -c 8 pycbc_inspiral \
     ...
     --disable-diffgw \
     --output triggers_track2_torch_cpu.hdf

   # Torch CPU DiffGW arm (torch_cpu_diffgw):
   taskset -c 8 pycbc_inspiral \
     --verbose \
     --frame-files docs/_include/H-H1_LOSC_CLN_4_V1-1187007040-2048.gwf \
     --channel-name H1:LOSC-STRAIN \
     ...
     --output triggers_track2_torch_cpu_diffgw.hdf

   # Torch CUDA LAL arm (torch_cuda_lal):
   taskset -c 8 pycbc_inspiral \
     --verbose \
     --frame-files docs/_include/H-H1_LOSC_CLN_4_V1-1187007040-2048.gwf \
     --channel-name H1:LOSC-STRAIN \
     ...
     --batch-size 64 \
     --disable-diffgw \
     --output triggers_track2_cuda_lal.hdf

   # Torch CUDA DiffGW arm (torch_cuda_diffgw):
   taskset -c 8 pycbc_inspiral \
     --verbose \
     --frame-files docs/_include/H-H1_LOSC_CLN_4_V1-1187007040-2048.gwf \
     --channel-name H1:LOSC-STRAIN \
