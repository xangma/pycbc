...
- Double precision (``float64``, ``complex128``) preserves bitwise agreement within
  numerical rounding tolerances of machine precision (:math:`\epsilon \approx 2.22 \times 10^{-16}`).
- Complex inner products use Cartesian components (:math:`(a_r b_r - a_i b_i) + i(a_r b_i + a_i b_r)`)
  to prevent truncation or precision drift.
- Marginalized likelihoods compute log-sum-exp reductions stably via ``jax.scipy.special.logsumexp``.
...
To execute the standardized benchmark evaluating ``original_standard``,
``branch_standard``, ``jax_cpu``, and ``jax_cuda`` across batch sizes
:math:`B \in \{1, 4, 8, 16, 32\}`, run the repository orchestrator:

.. code-block:: console

   export OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1
   taskset -c 8 python tools/bench_production_live_batch.py orchestrate \
     --root /path/to/checkout_root \
     --python /path/to/venv/bin/python \
     --routes original_standard branch_standard jax_cpu jax_cuda \
     --batches 1 4 8 16 32 \
     --replicates 3 \
     --samples 5 \
