...
On MPS, ``TaylorF2RedSpin`` and ``TaylorF2RedSpinTidal`` evaluate waveforms
below their mass-ratio-dependent phase-accuracy boundary with Torch float64
on the CPU, then copy the results to MPS as complex64. This precision fallback
applies to both regular-grid and arbitrary-frequency calls and retains the
native sequence interface. Those calls include host computation and transfers;
calls above the boundary retain MPS waveform evaluation.
