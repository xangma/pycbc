...
Benchmark only after scientific values and metadata pass parity for the same
implementation, device, dtype, shape, and workload. Independent floating-point
implementations need not be byte-identical unless a test requires it.

Use :ref:`torch-test-groups` for test commands and :ref:`torch-parity-matrix`
to prepare a reproducible comparison. This page defines what to compare and
which evidence to retain.
...
Threshold Boundary Effects and Step-Function Discontinuities
------------------------------------------------------------

When comparing matched filtering and veto trigger counts across independent
implementations (e.g. CPU MKL vs. Torch CPU or CUDA), hard step cuts such as
``--snr-threshold 5.5`` or ``--newsnr-threshold 5.0`` act as discontinuous
Heaviside step functions :math:`H(x - x_{\text{th}})`.

Because floating-point summation in single precision (``float32``) is non-associative
across differing SIMD architectures (Intel AVX-512 FMA vs. PyTorch/CUDA tensor reductions),
micro-level machine-epsilon variations (:math:`\sim 10^{-6}`) in the accumulated
power :math:`\chi^2` or inverse FFT butterflies can cause a marginal noise peak
(e.g. :math:`\text{NewSNR} = 5.0098` vs. :math:`4.9982`) to land on either side
of the threshold knife-edge.

This behavior is expected in discrete event filtering:
* Pre-cut candidate peak counts prior to veto thresholding are identical.
* The continuous underlying SNR timeseries evaluates to identical maximum values.
* For all genuine astrophysical signals above the background noise floor
  (:math:`\rho \ge 8\text{--}12`, :math:`\text{NewSNR} \gg 6`), trigger identities,
  coalescence phases, and peak arrival times remain 100% congruent.
