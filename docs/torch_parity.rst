...
stream the one-shot setup script to the remote host.  The selected root must not exist;
...
virtual environments based on the target GPU environment.  It verifies
...
.. code-block:: bash
   ...
   ssh <remote-host> "root=\$HOME/$ROOT_NAME; \
...
.. code-block:: bash
   ...
   ssh <remote-host> "PYCBC_PARITY_ROOT=\$HOME/$NEXT_ROOT_NAME \
       CURRENT_COMMIT=$CURRENT_COMMIT bash -s" \
       < tools/torch_parity/run_matrix.sh
