...
PLAN = {
    ...
    'replicates': 3, 'warmups': 2, 'samples': 5, 'seed': 7101,
    'physical_core_logical_cpu': 8, 'smt_sibling': 72, 'numerical_threads': 1,
    'qualification': 'Separate processes; compare every complex filter sample and structured triggers to same-revision standard CPU batch 1, using the same fixed bank and strain for every batch.',
...
def validate_result(result, route, batch, mode, reference=None, smoke=False):
    ...
    if mode == 'qualify':
        ...
        if len(pointwise) != blocks or any(
                not p['processed_once'] or p['samples'] != bank * size
                or p['failed_samples'] or p['nonfinite_actual'] or p['nonfinite_reference']
                or p['processing_counts'] != [1] * bank or len(p['rows']) != bank
                for p in pointwise):
...
class Campaign(control.Campaign):
    ...
    def worker(self, label, route, batch, mode, reference=None, smoke=False):
        ...
        command = ['taskset', '-c', '8', sys.executable, self.root / 'batch-worker.py',
                   ...
                   '--mode', mode, '--output-dir', target, '--seed', str(PLAN['seed']),
        ...
        validate_result(result, route, batch, mode, reference, smoke)
        return result
...
def run(campaign):
    ...
    campaign.capacity()
    campaign.update(phase='smoke-qualification')
    reference = ROOT / 'runs/smoke-b1-branch_standard'
    qualifications = {}
    for batch in (1, 8):
        for route in ROUTES:
            label = f'smoke-b{batch}-{route}'
            result = campaign.worker(label, route, batch, 'qualify',
                                     None if label == reference.name else reference, smoke=True)
            qualifications[label] = result
            control.save(ROOT / 'smoke-qualifications.json', qualifications)
            require_qualification(result)
    campaign.update(phase='qualification')
    reference = ROOT / 'runs/qual-b1-branch_standard'
    qualifications = {}
    failures = []
    for batch in BATCHES:
        for route in ROUTES:
            label = f'qual-b{batch}-{route}'
            result = campaign.worker(label, route, batch, 'qualify',
                                     None if label == reference.name else reference)
            qualifications[label] = result
            control.save(ROOT / 'qualifications.json', qualifications)
            if result.get('status') != 'pass':
                failures.append(label)
                if label == reference.name:
                    raise ValueError('Reference qualification failed')
    if failures:
