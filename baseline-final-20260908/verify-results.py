...
DEPENDENCY_DIAGNOSTIC = {
    'torch-environment-diagnostic.json': '7ca88cfbe69954d16ab8dd5585db26aadc8a77e411225bd7ebbd90ff1a5cb494',
    'inspect-torch-environment.py': '7e0ae47390539eb5b86b8e6879ab3a6576133ce68d760d222dd244bd895572f8',
}
TIMING_SLACK = {
...
def dependency_diagnostic(diag, dependencies, timing_status, dependency_hash, status_hash):
    ...
    return dict(status='RECONCILED_WITH_POST_RUN_DIAGNOSTIC',
        ...
        diagnostic_sha256=DEPENDENCY_DIAGNOSTIC['torch-environment-diagnostic.json'],
        diagnostic_script_sha256=DEPENDENCY_DIAGNOSTIC['inspect-torch-environment.py'],
...
class Verifier:
    ...
    def initialize(self):
        ...
        if any((self.root / name).exists() for name in DEPENDENCY_DIAGNOSTIC):
            for name, expected in DEPENDENCY_DIAGNOSTIC.items():
                require(digest(self.file(name)) == expected, 'Unreviewed dependency diagnostic: ' + name)
            require(self.continuation is not None, 'Dependency diagnostic requires the pinned timing continuation')
            self.dependency_reconciliation = dependency_diagnostic(
                self.read('torch-environment-diagnostic.json'), self.deps, self.timing_status,
    ...
    def case(self, arm, mode, repeat=None):
        ...
        if scheme.startswith('torch:'):
            ...
            if self.dependency_reconciliation is None:
                require(runtime['torch_version'] == packages['torch'], 'Torch dependency version mismatch')
            else:
                reconciliation = self.dependency_reconciliation
                require(runtime['torch_version'] == reconciliation['runtime_torch_version'] and
                        runtime['torch_cuda_version'] == reconciliation['runtime_cuda_version'] and
                        runtime['hostname'] == reconciliation['hostname'],
                        'Worker Torch version/host disagrees with independent diagnostic')
        else:
...
def self_test():
    ...
    def reconcile(value):
        return dependency_diagnostic(value, deps, timing, 'a' * 64, 'b' * 64)
    check(reconcile(diag)['original_manifest_unchanged'] and deps['packages']['torch'] == '2.1.1',
          'supported duplicate-installation diagnostic preserves original manifest')
