"""Matched frozen-baseline/final-proposal executable experiment."""
import copy
import datetime
import fcntl
import hashlib
...
def main():
    ...
    with LOCK.open() as lock:
        ...
        for arm in ('proposed-cpu', 'torch-cpu', 'torch-cuda'):
            name = 'original-vs-' + arm
            comparisons[name] = compare(qualified['original-cpu'], qualified[arm], name)
            conditioning_compare(qualified['original-cpu'], qualified[arm], name)
