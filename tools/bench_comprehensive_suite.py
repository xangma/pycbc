...
- Native TaylorF2 waveform batching against repeated scalar generation
- Torch CPU (Native Batch Correlate, FFTW/MKL Batching, Vector Peak Scan)
- Torch CUDA (2D tensor correlate and GPU-side peak scan)

Measures:
1. Scalar waveform generation throughput (templates/sec)
2. TaylorF2 batch generation throughput (templates/sec)
3. Pure Correlation + IFFT throughput (transforms/sec & GFlops)
4. Peak Finding / Clustering throughput (MSamples/sec)
5. Synthetic LiveBatchMatchedFilter microbenchmark latency and throughput
...
def _worker_benchmark(args_json_str: str) -> str:
    ...
    # -------------------------------------------------------------
    if workload in ("waveform", "waveform_batch"):
        approx = config.get("approximant", "TaylorF2")
        params = {
            "mass1": 30.0,
            "mass2": 25.0,
            "spin1z": 0.4,
        ...
        use_native_batch = (
            workload == "waveform_batch"
            and route in ("torch_cpu", "torch_cuda")
        )
        if workload == "waveform_batch" and approx != "TaylorF2":
            raise ValueError("native waveform batching supports only TaylorF2")
        ...
        with context:
            if use_native_batch:
                ...
                def generate_waveforms():
                    return pycbc.waveform.get_fd_waveform_batch(
                        approx,
                        **batch_params,
                    )
                ...
                def generate_waveforms():
                    ...
                    for _index in range(batch_size):
                        result = pycbc.waveform.get_fd_waveform(
                            approximant=approx,
                            **params,
                        )
                    return result
...
def run_full_suite(
    ...
    results.setdefault("waveform_batch", {})
    ...
    # -------------------------------------------------------------
    # 2. Native TaylorF2 Batch Generation Benchmark
    # -------------------------------------------------------------
    if "waveform_batch" in workloads:
        print("\n" + "-" * 90)
        print(" 2. TAYLORF2 BATCH GENERATION THROUGHPUT (Templates / Second)")
        print("-" * 90)
        print(
            f" {'Batch (B)':<10} | {'LAL scalar CPU':<14} | "
            f"{'Torch scalar CPU':<16} | {'Torch batch CPU':<15} | "
            f"{'Torch batch CUDA':<16} | {'CPU Batch Gain':<14}"
        )
        print("-" * 90)
        ...
        for b in batch_sizes:
            cfg_base = {
                ...
                "batch_size": b,
                "iterations": 8 if b < 128 else 4,
                "warmup": 3 if b < 128 else 2,
            }
            r_lal = _run_subprocess_worker(
                {**cfg_base, "route": "original_standard", "threads": 1},
                py_exe,
            )
            r_torch_scalar = _run_subprocess_worker(
                {**cfg_base, "route": "torch_cpu_scalar", "threads": 16},
                py_exe,
            )
            r_torch_batch = _run_subprocess_worker(
                {**cfg_base, "route": "torch_cpu", "threads": 16}, py_exe
            )
            rate_lal = 1.0 / r_lal["median"] if r_lal["median"] > 0 else 0
            rate_torch_scalar = (
                1.0 / r_torch_scalar["median"]
                if r_torch_scalar["median"] > 0
                else 0
            )
            rate_torch_batch = (
                1.0 / r_torch_batch["median"]
                if r_torch_batch["median"] > 0
                else 0
            )
            ...
            cpu_gain = (
                rate_torch_batch / rate_torch_scalar
                if rate_torch_scalar > 0
                else 0
            )
            print(
                f" {b:<10} | {rate_lal:8.1f} wf/s | "
                f"{rate_torch_scalar:8.1f} wf/s   | "
                f"{rate_torch_batch:8.1f} wf/s  | "
                f"{cuda_str:<16} | {cpu_gain:8.2f}x"
            )
            results["waveform_batch"][f"batch_{b}"] = {
                "batch_size": b,
                "lal_scalar_cpu": r_lal,
                "torch_scalar_cpu": r_torch_scalar,
                "torch_batch_cpu": r_torch_batch,
                "torch_batch_cuda": r_cuda,
            }

    # -------------------------------------------------------------
    # 3. Pure Matched Filter Correlate + IFFT Throughput
    # -------------------------------------------------------------
    if "correlate_ifft" in workloads:
        print("\n" + "-" * 90)
        print(
            " 3. PURE MATCHED FILTER CORRELATION + IFFT THROUGHPUT (N=131072, 64s at 2048Hz)"
        )
        print("-" * 90)
        print(
    ...
    # -------------------------------------------------------------
    # 4. Peak Finding & Thresholding Throughput
    # -------------------------------------------------------------
    if "peak_finding" in workloads:
        print("\n" + "-" * 90)
        print(" 4. PEAK FINDING & THRESHOLDING THROUGHPUT (MSamples / Second)")
        print("-" * 90)
        print(
    ...
    # -------------------------------------------------------------
    # 5. Synthetic LiveBatchMatchedFilter microbenchmark
    # -------------------------------------------------------------
    if "synthetic_live_batch" in workloads:
        print("\n" + "-" * 90)
        print(
            " 5. SYNTHETIC LIVEBATCH MICROBENCHMARK "
    ...
    # -------------------------------------------------------------
    # 6. Multi-Detector Network Response Benchmark
    # -------------------------------------------------------------
    if "detector_network" in workloads:
        print("\n" + "-" * 90)
        print(" 6. MULTI-DETECTOR NETWORK ANTENNA PATTERNS & DELAYS (H1, L1, V1)")
        print("-" * 90)
        print(
    ...
    # -------------------------------------------------------------
    # 7. Batched Relative Binning Likelihood Benchmark
    # -------------------------------------------------------------
    if "relbin_likelihood" in workloads:
        print("\n" + "-" * 90)
        print(" 7. BATCHED RELATIVE BINNING SUMMARY EVALUATION (relbin_torch)")
        print("-" * 90)
        print(
    ...
    # -------------------------------------------------------------
    # 8. MatchedFilterControl.full_matched_filter_and_cluster_symm
    # -------------------------------------------------------------
    if (
        ...
        print("\n" + "-" * 105)
        print(
            " 8. MATCHED_FILTER_AND_CLUSTER_SYMM "
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Batch sizes for waveform generation, correlate, peak finding, "
            "and synthetic live-batch microbenchmarks"
        ),
    )
    parser.add_argument(
        "--output",
