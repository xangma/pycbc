...
def require_device(requested, module=None):
    if module is None:
        import jax as module
    devices = module.devices()
    types = {d.platform for d in devices}
    requested_str = str(requested).lower()
    if requested_str == "cpu":
        if "cpu" not in types:
            raise DeviceUnavailable(f"Requested {requested}: CPU unavailable")
    elif requested_str.startswith("cuda") or requested_str.startswith("gpu"):
        if "gpu" not in types and "cuda" not in types:
            raise DeviceUnavailable(f"Requested {requested}: GPU/CUDA unavailable")
    else:
        if not any(d.platform == requested_str for d in devices):
            raise DeviceUnavailable(f"Requested {requested}: device unavailable")
    return str(requested)


def synchronize(device=None, module=None):
    if module is None:
        try:
            import jax as module

            module.effects_barrier()
        except Exception:
            pass
...
def search_snapshot(batches, num_templates):
    ...
    batch = batches[0]
    field = (
        (lambda key, default=None: batch.get(key, default))
        if isinstance(batch, dict)
        else (lambda key, default=None: getattr(batch, key, default))
    )
    ...
    for row in rows:
        ...
        for key, values in row.items():
            ...
            elif hasattr(values, "block_until_ready"):
                values = np.asarray(values.block_until_ready())
            else:
    ...
    if not snapshot:
        return {
            "template_id": np.empty(0, np.int64),
            "sample_idx": np.empty(0, np.int64),
...
def compare_search_snapshots(reference, actual):
    ...
    for name, expected in reference.items():
        ...
        if exact:
            ...
            atol, rtol = (1e-3, 0.0) if name == "snr" else (1e-6, 1e-6)
    ...
    return {
        "passed": True,
        "candidate_count": len(actual["template_id"]),
        "max_absolute_errors": errors,
        "output_hashes": {
            name: array_hash(value) for name, value in actual.items()
        },
    }
...
def loaded_implementation_identities():
    ...
    for name, module in list(sys.modules.items()):
        relevant = (
            name == "pycbc"
            or name.startswith("pycbc.")
            or name.split(".")[0].startswith("lal")
            or name.startswith(("numpy.", "scipy.fft", "pyfftw."))
        )
        path = getattr(module, "__file__", None)
        if relevant and path and any(
    ...
    for key in (
        ...
        if isinstance(name, str):
            fft.setdefault("ctypes_libraries", {})[key] = (
                ...
                else {"loader_name": name}
            )
    return {
        "loaded_extension_binaries": binaries,
        "mapped_reference_libraries": mapped,
        "reference_library_versions": reference_versions,
        "reference_fft": fft,
        "mapping_scope": (
            "/proc/self/maps reference-library mappings"
            if maps.is_file()
            else "Python extensions and explicit ctypes paths; process mappings unavailable"
        ),
    }


def compare_evidence_receipts(reference, actual):
    """Fail closed unless binary identities, mappings, and Git revisions match."""
    ...
    for key in ("loaded_extension_binaries", "mapped_reference_libraries"):
        ...
        for name, identity in reference[key].items():
            if identity != actual[key].get(name) or not identity.get("sha256"):
                raise ValueError(
                    f"parent/child binary differs or unavailable: {name}"
                )
...
def git_snapshot(root):
    ...
    try:
        ...
        for name in untracked.decode().split("\0"):
            ...
            if (
                not name
                or name.split("/")[0] in {"artifacts", "review-input", ".agents"}
            ):
                continue
...
def execution_provenance(repo_root=None):
    ...
    for name in (
        ...
        if name == "diffgw" and origin:
            try:
                ...
                repositories[name] = git_snapshot(diffgw_root)
    ...
    return {
        **loaded_implementation_identities(),
        "repositories": repositories,
        "imported_module_paths": modules,
