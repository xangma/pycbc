#!/usr/bin/env python3
"""Counterbalanced 4-arm pycbc_inspiral performance-evidence benchmark.

Evaluates the four standardized benchmark arms:
1. original_cpu: Baseline checkout (40e94792b3), --processing-scheme cpu:1
2. branch_cpu:   Candidate branch, --processing-scheme cpu:1
3. torch_cpu:    Candidate branch, --processing-scheme torch:cpu:1
4. torch_cuda:   Candidate branch, --processing-scheme torch:cuda:0
...
def _run_single_case(
    ...
    if arm in ("original_cpu", "branch_cpu"):
        scheme = "cpu:1"
    elif arm in ("torch_cpu", "torch_cpu_diffgw"):
        scheme = "torch:cpu:1"
    elif arm in ("torch_cuda_lal", "torch_cuda", "torch_cuda_diffgw"):
        scheme = "torch:cuda:0"
    else:
    ...
    if use_compressed_waveforms:
        ...
        if arm in ("torch_cpu", "torch_cuda_lal", "torch_cuda"):
            cli_args.append("--disable-diffgw")
            if "cuda" in arm:
                cli_args.extend(["--batch-size", str(batch_size)])
        elif arm in ("torch_cuda_diffgw", "torch_cpu_diffgw"):
            cli_args.extend(["--batch-size", str(batch_size), "--enable-diffgw", "--enable-diffgw-compile"])
    ...
    if arm in ("torch_cpu", "torch_cuda_lal"):
        env["PYCBC_TAYLORF2_NATIVE"] = "0"
        env["PYCBC_TORCH_NATIVE"] = "0"
    elif arm == "torch_cuda":
        env["PYCBC_TAYLORF2_NATIVE"] = "1"
    elif arm in ("torch_cuda_diffgw", "torch_cpu_diffgw"):
        env["PYCBC_TAYLORF2_NATIVE"] = "0"
        env["PYCBC_DIFFGW_COMPILE"] = "1"
...
def main():
    parser = argparse.ArgumentParser(
        description="Run 4-arm pycbc_inspiral reference campaign benchmark."
    )
    parser.add_argument(
        "--original-source",
        type=Path,
    ...
    sources = {
        ...
        "torch_cpu_diffgw": args.branch_source.resolve(),
        "torch_cuda_lal": args.branch_source.resolve(),
    ...
    print("=" * 80)
    print("STARTING 4-ARM PYCBC_INSPIRAL REFERENCE CAMPAIGN BENCHMARK")
    ...
    orderings = [
        ["original_cpu", "branch_cpu", "torch_cpu", "torch_cpu_diffgw", "torch_cuda_lal", "torch_cuda", "torch_cuda_diffgw"],
        ["torch_cuda_diffgw", "torch_cuda", "torch_cuda_lal", "torch_cpu_diffgw", "torch_cpu", "branch_cpu", "original_cpu"],
        ["branch_cpu", "torch_cuda_lal", "torch_cuda_diffgw", "original_cpu", "torch_cpu_diffgw", "torch_cuda", "torch_cpu"],
    ]
    ...
    for rep in range(args.replicates):
        ...
        for arm in order:
            ...
            print(
                f"[{run_count:2d}/{total_runs}] Running {arm:14s} (Rep {rep + 1}/{args.replicates}) ... ",
                end="",
                flush=True,
            )
    ...
    receipt = {
        ...
        "parity_analysis": parity_results,
        "summary": summaries,
    ...
    print("\n" + "=" * 90)
    print("PYCBC_INSPIRAL 4-ARM REFERENCE CAMPAIGN BENCHMARK SUMMARY")
    print("=" * 90)
    print(
        f"{'Arm':<14} | {'Wall Med [95% CI] (s)':<25} | {'Calc Time (s)':<14} | {'Calc Spdup':<10} | {'Wall Spdup':<10} | {'Trigs':<6}"
    )
    print("-" * 90)
    for arm in args.arms:
        s = summaries[arm]
        w_ci = s["wall_sec"]["median_ci95"]
        w_str = f"{s['wall_sec']['median']:6.2f} [{w_ci['low']:.2f}, {w_ci['high']:.2f}]"
        c_str = f"{s['calc_time_sec']['median']:6.2f}"
        c_spdup = f"{s.get('speedup_calc_vs_original', 1.0):6.2f}x"
        w_spdup = f"{s.get('speedup_wall_vs_original', 1.0):6.2f}x"
        trigs = f"{s['triggers_count']:5d}"
        print(
            f"{arm:<14} | {w_str:<25} | {c_str:<14} | {c_spdup:<10} | {w_spdup:<10} | {trigs:<6}"
        )
    print("-" * 90)

    print("\nPARITY VALIDATION (against original_cpu):")
    for arm, p in parity_results.items():
        status = "PASSED" if p.get("passed") else "FAILED"
        print(
            f"  - {arm:<12}: [{status}] max SNR diff={p.get('max_snr_diff', 0.0):.2e}, "
            f"rel SNR L2={p.get('relative_l2_snr', 0.0):.2e}, Triggers={p.get('trigger_count')}"
        )
    print("=" * 90)
