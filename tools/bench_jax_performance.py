...
The default comparison uses the current checkout's standard CPU and JAX CPU/CUDA
matched filters, all with host LAL waveforms. Optional diffgw arms measure pure
JAX waveform generation. This harness does not load a separate baseline checkout
or select a particular standard CPU FFT backend.
"""
...
def main():
    parser = argparse.ArgumentParser(description="PyCBC JAX Performance Benchmark Harness")
    parser.add_argument(
        "--device",
        type=str,
        default="auto",
    ...
    parser.add_argument(
        ...
        default=None,
        choices=ARM_LABELS,
        help="Current-checkout arms; defaults to branch_cpu, jax_cpu_lal, and available jax_cuda_lal",
    )
    parser.add_argument(
        "--skip-gpu",
        action="store_true",
    ...
    if args.arms is not None:
        ...
        selected_arms = ["branch_cpu", "jax_cpu_lal"]
        if gpu_dev is not None:
            selected_arms.append("jax_cuda_lal")

    print("=== PyCBC JAX Performance Benchmark ===")
    ...
    for n in args.lengths:
        ...
        # Calculate speedups vs the current checkout standard CPU, batch 1.
        base_arm = "branch_cpu"
        ...
        for a_key, a_data in track_arms.items():
            for b_str, b_data in a_data["batches"].items():
                ...
                if baseline_tps is not None and baseline_tps > 0:
                    b_data["speedup_vs_baseline"] = tps / baseline_tps
                else:
                    b_data.pop("speedup_vs_baseline", None)

        # Populate backward-compatible keys
        if "branch_cpu" in track_arms and "1" in track_arms["branch_cpu"]["batches"]:
