...
ROUTE_CONFIGS = {
    ...
    "jax_cpu": {
        "label": "JAX CPU (production defaults)",
        "backend": "jax",
        "device": "cpu",
        "routing_mode": "production_default",
        "experimental": False,
    ...
    "jax_cuda": {
        "label": "JAX CUDA (production defaults)",
        "backend": "jax",
        "device": "cuda",
        "routing_mode": "production_default",
        "experimental": False,
...
def _route_source(root: Path, route: str) -> Path:
    ...
    elif route.startswith("torch_cpu") or route.startswith("jax_cpu"):
        candidate = root / "branch_cpu"
        if not candidate.exists():
            candidate = root / "branch"
    elif route.startswith("torch_cuda") or route.startswith("jax_cuda"):
...
def _child(args: argparse.Namespace) -> None:
    ...
    is_torch_cuda = args.route.startswith("torch_cuda")
    is_torch_cpu = args.route.startswith("torch_cpu")
    is_torch = is_torch_cuda or is_torch_cpu

    is_jax_cuda = args.route.startswith("jax_cuda")
    is_jax_cpu = args.route.startswith("jax_cpu")
    is_jax = is_jax_cuda or is_jax_cpu

    is_cuda = is_torch_cuda or is_jax_cuda
    ...
    if is_torch:
        ...
        jax_module = jax
        if is_jax_cuda:
            context = scheme.JAXScheme(f"cuda:{args.cuda_device}")
        else:
    ...
    def _sync():
        if is_torch_cuda and torch_module is not None:
            torch_module.cuda.synchronize()
        elif is_jax and jax_module is not None:
            try:
    ...
    with context:
        ...
        elif hasattr(out_mem, "_data") and hasattr(out_mem._data, "data"):
            out_np = np.asarray(out_mem._data.data).reshape(batch_size, size).copy()
        elif hasattr(out_mem, "numpy"):
            out_np = out_mem.numpy().reshape(batch_size, size).copy()
        else:
            out_np = np.asarray(out_mem).reshape(batch_size, size).copy()
