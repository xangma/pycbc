...
optional JAX batching or explicitly enabled diffgw generation. Record process
wall time and executable HDF timers. Log-derived phase estimates are approximate
and do not constitute an independently instrumented six-phase decomposition.

The built-in comparison checks first-repetition trigger counts, times and SNR
against original_cpu when present, otherwise branch_cpu. It is a limited check,
not complete scientific qualification of trigger identities, vetoes or metadata.
"""
...
def compare_campaign_parity(raw_results):
    """Compare first-replicate triggers with the available standard CPU arm."""
    reference_arm = next(
        (arm for arm in ("original_cpu", "branch_cpu") if raw_results.get(arm)),
        None,
    )
    if reference_arm is None:
        return None, {}
    reference_hdf = Path(raw_results[reference_arm][0]["triggers_path"])
    results = {
        arm: compare_trigger_parity(reference_hdf, Path(runs[0]["triggers_path"]))
        for arm, runs in raw_results.items()
        if arm != reference_arm and runs
    }
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="CUDA template batch size for batched/diffgw arms (default: 64); JAX CPU batched uses 16",
    )
    parser.add_argument(
        "--enable-diffgw",
        action="store_true",
