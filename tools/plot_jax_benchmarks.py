...
ARM_CONFIG = {
    "original_cpu": {
        "label": "Original CPU (MKL)",
        "color": "#333333",
        "linestyle": "--",
        "marker": "o",
    },
    "branch_cpu": {
        "label": "Branch CPU (MKL)",
        "color": "#777777",
        "linestyle": ":",
        "marker": "s",
    },
...
def plot_throughput_scaling(data: Dict[str, Any], output_path: Path):
    """Dual-panel scaling figure for streaming and inspiral tracks."""
    fig, axes = plt.subplots(1, 2, figsize=(14, 6), sharey=False)

    tracks = [
        ("streaming_n131072", "Streaming Live Track (N = 131,072 = 2¹⁷, 2048 Hz)", axes[0]),
        ("inspiral_n2097152", "Offline Inspiral Track (N = 2,097,152 = 2²¹, 4096 Hz)", axes[1]),
    ]
    ...
    for track_key, track_title, ax in tracks:
        ...
        ax.set_ylabel("Calculation Rate (templates / sec)", fontsize=12, fontweight="bold")
...
def plot_speedup_matrix(data: Dict[str, Any], output_path: Path):
    ...
    for idx, b in enumerate(eval_batches):
        ...
        for arm in active_arms:
            ...
            if b_str in arms[arm].get("batches", {}):
                speedups.append(arms[arm]["batches"][b_str].get("speedup_vs_baseline", 1.0))
            else:
                speedups.append(0.0)
    ...
    ax.set_yscale("log")
    ax.set_ylabel("Speedup vs Standard CPU Baseline", fontsize=12, fontweight="bold")
    ax.set_title("Relative Acceleration Across Benchmark Arms (Streaming Track)", fontsize=14, fontweight="bold")
    ax.set_xticks(x)
...
def plot_latency_breakdown(data: Dict[str, Any], output_path: Path):
    ...
    ax.barh(y_pos, tr_times, left=wf_times, color="#c44e52", label="Host-to-Device Transfer")
    ...
    ax.set_title(f"Per-Template Latency Breakdown (Batch B={target_b}, Streaming Track)", fontsize=14, fontweight="bold")
    ax.set_yticks(y_pos)
