...
try:
    with checked.LOCK.open() as lock:
        ...
        baseline_log = (ROOT/'linux-baseline-mkl-tests.log').read_text()
        corrected_log = (ROOT/'linux-validation-v2/tests.log').read_text()
        failures = ['0 != -0.06937270748284567 within 1 places',
                    '131072 != 131071.93062729252 within 1 places']
        assert '2 failed, 13 passed' in baseline_log
        assert '2 failed, 32 passed' in corrected_log
        assert all(error in baseline_log and error in corrected_log for error in failures)
        save('test-gate.json', dict(status='no_new_failures', precision_tests_passed=19,
             corrected=dict(passed=32,failed=2), baseline_existing=dict(passed=13,failed=2),
             existing_failures=['TestMatchedFilter.test_perfect_match',
                                'TestMatchedFilter.test_perfect_match_offset'],
             assertion_messages=failures,
             baseline_log_sha256=sha(ROOT/'linux-baseline-mkl-tests.log'),
             corrected_log_sha256=sha(ROOT/'linux-validation-v2/tests.log')))
        state['completed'].append('tests_no_new_failures')
        for label, tail in [('replay',replay)]:
        ...
        expected_q = json.loads((BASE/'runs/qual-proposed-cpu/qualification.json').read_text())
        conditioning_exact = q['observations']['conditioned_strain'] == expected_q['observations']['conditioned_strain']
        geometry_exact = q['observations']['segment_geometry'] == expected_q['observations']['segment_geometry']
        assert len(fields) == 13 and all(fields.values()) and psd_exact
        assert conditioning_exact and geometry_exact
        ...
        save('comparison.json',dict(status='pass',comparison='CPU split versus frozen proposed CPU',
             trigger_fields_exact=fields,psd_exact=psd_exact,conditioned_strain_exact=conditioning_exact,geometry_exact=geometry_exact,source_and_input_pins_unchanged=unchanged,
