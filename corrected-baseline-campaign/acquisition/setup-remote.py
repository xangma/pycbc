...
REFERENCES = {'corrected': FROZEN / 'original', 'proposed': FROZEN / 'proposed'}
...
try:
    ...
    with LOCK.open() as lock:
        ...
        run('git', '-C', str(ROOT / 'repo'), 'fetch', str(ROOT / 'sources.bundle'),
            'refs/heads/codex/cpu-precision-corrections-20260908',
            'refs/heads/codex/cpu-base-restack-20260908-pr15')
        for name, head in CONFIG['source_commits'].items():
            ...
            build = json.loads((FROZEN / ('original-build.json' if name == 'corrected' else 'proposed-build.json')).read_text())
            ...
            save(ROOT / (name + '-build.json'), dict(commit=head, source=str(source),
                ...
                reference_build_sha256=sha(FROZEN / ('original-build.json' if name == 'corrected' else 'proposed-build.json'))))
