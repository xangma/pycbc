...
Integrated runtime and qualification
------------------------------------

The stack includes production descriptor reuse from ``abf2528b367e`` and the
guarded offline graph controller from ``fad7d8440bfd``. The
`sealed integrated qualification
<https://github.com/xangma/pycbc/tree/06c77a19432bce561f5cbeb6c2da1dedde98274b/stack-validation-20260908>`_
tests production source ``fad7d8440bfde083f2e94ee62a0017492dfc4013``.
Its runtime and executable files match published main
...
regressions. The other 52 regression cases skipped because MPS was unavailable.
...
match its own pinned reference byte for byte; this is not byte identity across
backends or of complete HDF files. Only independently checked source and
executable-path provenance substitutions were allowed, with raw failed metadata
verdicts retained.

The CUDA qualifier snapshots production outputs before running its eager oracle.
All 1920 replays pass both full and sparse output comparisons, with five
captures, output-retention checks and actual consumer use. Suppressed-replay
and pending-allocation negative controls
detect the intended failures. This is correctness evidence. The historical R
and G plots below measure their pinned prototype helpers; no timing campaign
has measured the integrated implementation.
...
The integrated qualification archive above has two separate offline checks.
From its restored ``stack-validation-20260908/`` directory:
...
The first checks the complete publication file inventory. The second needs
Python 3.11 or later, NumPy and h5py; it verifies frozen source and native test
records, replays the five HDF comparisons and checks the graph and negative
control records. Neither command reruns a scientific workload or timing job.
