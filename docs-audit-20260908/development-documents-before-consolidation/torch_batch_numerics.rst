...
folder. ``--campaign`` and ``PYCBC_BATCH_SWEEP_SCHEMA`` must point to the
restored inner ``torch-current-batch-sweep-20260907-r4/`` directory containing
``batch-campaign.py`` and ``runs/``, rather than the publication root:

.. code-block:: bash

   python tools/plot_torch_batch_sweep.py --campaign /path/to/restored-r4/torch-current-batch-sweep-20260907-r4 --output /path/to/new-plots
   python tools/plot_torch_batch_sweep.py --campaign /path/to/restored-r4/torch-current-batch-sweep-20260907-r4 --output /path/to/new-plots --verify-only
...
inputs, renderer and image hashes. Rendering requires a new or empty output
directory; it refuses to overwrite existing contents. ``--verify-only`` needs
only the Python standard library and checks an existing plot directory.
...
.. code-block:: bash
   ...
   PYCBC_BATCH_SWEEP_SCHEMA=/path/to/restored-r4/torch-current-batch-sweep-20260907-r4 pytest -q test/test_plot_torch_batch_sweep.py
