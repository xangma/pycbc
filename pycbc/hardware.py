        ...
        try:
            ...
            if nvcuda.cuInit(0) == 0:
                ...
                if nvcuda.cuDeviceGet(ctypes.byref(dev), int(device_id)) == 0:
                    val = ctypes.c_int()
                    if nvcuda.cuDeviceGetAttribute(ctypes.byref(val), 38, dev) == 0:
                        if val.value > 0:
...
def get_optimal_batch_tile_size(
    ...
    """
    if (
        "PYCBC_BATCH_TILE_SIZE" in os.environ
        and os.environ["PYCBC_BATCH_TILE_SIZE"].strip()
    ):
        try:
...
def get_cpu_cores_per_numa_node():
    ...
    try:
        shared_list = "/sys/devices/system/cpu/cpu0/cache/index3/shared_cpu_list"
        if not os.path.exists(shared_list):
            shared_list = "/sys/devices/system/cpu/cpu0/cache/index2/shared_cpu_list"
