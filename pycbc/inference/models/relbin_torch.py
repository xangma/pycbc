...
def polarization_phase(polarization, like):
    ...
    two_psi = 2.0 * angle
    return torch.complex(torch.cos(two_psi), -torch.sin(two_psi))
...
def polarized_antenna_response(fp, fc, pol_phase, like):
    ...
    if torch.is_complex(pol_phase):
        pol_phase = _as_tensor(pol_phase, like, like.dtype)
        pr = pol_phase.real
        pi = pol_phase.imag
    else:
        pol_phase = _as_tensor(pol_phase, like, real_dtype)
        two_psi = 2.0 * pol_phase
        pr = torch.cos(two_psi)
        pi = -torch.sin(two_psi)
...
def dominant_mode_projection(fp, fc, polarization, inclination, like):
    ...
    return torch.complex(fp_rot * plus, fc_rot * cosi)
...
def prepare_reference_data(waveform, data, size, offset, delta_f, time_shift):
    ...
    d_r = data_tensor.real if torch.is_complex(data_tensor) else data_tensor
    d_i = data_tensor.imag if torch.is_complex(data_tensor) else torch.zeros_like(d_r)
    shifted_data = torch.complex(
        d_r * cos_p + d_i * sin_p,
        d_i * cos_p - d_r * sin_p,
    )
    return (
...
def summary_product(h1, h2, psd, freqs, bins, delta_f):
    ...
    h1_r = h1.real if torch.is_complex(h1) else h1
    h1_i = h1.imag if torch.is_complex(h1) else 0.0
    h2_r = h2.real if torch.is_complex(h2) else h2
    h2_i = h2.imag if torch.is_complex(h2) else 0.0
    inv_psd = 1.0 / psd
    h12_r = (h1_r * h2_r + h1_i * h2_i) * inv_psd
    h12_i = (h1_r * h2_i - h1_i * h2_r) * inv_psd

    zero = torch.zeros(h12_r.shape[:-1] + (1,), dtype=real_dtype, device=like.device)
    prefix_r = torch.cat((zero, torch.cumsum(h12_r, dim=-1)), dim=-1)
    prefix_i = torch.cat((zero, torch.cumsum(h12_i, dim=-1)), dim=-1)
    weighted_prefix_r = torch.cat((zero, torch.cumsum(h12_r * freqs, dim=-1)), dim=-1)
    weighted_prefix_i = torch.cat((zero, torch.cumsum(h12_i * freqs, dim=-1)), dim=-1)
    ...
    a0 = torch.complex(a0_r, a0_i)
    a1 = torch.complex(a1_r, a1_i)
    return a0, a1
...
def _linearized_filter_cartesian(ratio_r, ratio_i, a0_r, a0_i, a1_r, a1_i):
    ...
    filt_r = (t1_r + t2_r).sum(dim=-1)
    filt_i = -(t1_i + t2_i).sum(dim=-1)
    return torch.complex(filt_r, filt_i)


def _linearized_norm_cartesian(ratio_r, ratio_i, b0, b1):
    """Calculate the linearized waveform norm in Cartesian form."""
    power = ratio_r.square() + ratio_i.square()
    power_lo = power[..., :-1]
    power_hi = power[..., 1:]
    return ((b0 - b1) * power_lo + b1 * power_hi).sum(dim=-1)
...
def _linearized_cross_cartesian(
    ...
    return torch.complex((t1_r + t2_r).sum(dim=-1), (t1_i + t2_i).sum(dim=-1))
...
def _linearized_filter(ratio, a0, a1):
    ...
    r_r = ratio.real if torch.is_complex(ratio) else ratio
    r_i = ratio.imag if torch.is_complex(ratio) else 0.0
    a0_r = a0.real if torch.is_complex(a0) else a0
    a0_i = a0.imag if torch.is_complex(a0) else 0.0
    a1_r = a1.real if torch.is_complex(a1) else a1
    a1_i = a1.imag if torch.is_complex(a1) else 0.0
...
def _linearized_cross(ratio, ratio2, a0, a1):
    ...
    r1_r = ratio.real if torch.is_complex(ratio) else ratio
    r1_i = ratio.imag if torch.is_complex(ratio) else 0.0
    r2_r = ratio2.real if torch.is_complex(ratio2) else ratio2
    r2_i = ratio2.imag if torch.is_complex(ratio2) else 0.0
    a0_r = a0.real if torch.is_complex(a0) else a0
    a0_i = a0.imag if torch.is_complex(a0) else 0.0
    a1_r = a1.real if torch.is_complex(a1) else a1
    a1_i = a1.imag if torch.is_complex(a1) else 0.0
...
def likelihood_parts(freqs, fp, fc, dtc, hp, hc, h00, a0, a1, b0, b1):
    ...
    shift_r = torch.cos(phase)
    shift_i = torch.sin(phase)
...
def likelihood_parts_det(freqs, dtc, channel, h00, a0, a1, b0, b1):
    ...
    shift_r = torch.cos(phase)
    shift_i = torch.sin(phase)
    ch_r = channel.real if torch.is_complex(channel) else channel
    ch_i = channel.imag if torch.is_complex(channel) else 0.0
...
def likelihood_parts_multi(
    ...
    *,
    ...
    shift_r = torch.cos(phase)
    shift_i = torch.sin(phase)
    shift2_r = torch.cos(phase2)
    shift2_i = torch.sin(phase2)
...
def likelihood_parts_det_multi(freqs, dtc, channel, h00, dtc2, channel2, h002, a0, a1):
    ...
    ch2_r = channel2.real if torch.is_complex(channel2) else channel2
    ch2_i = channel2.imag if torch.is_complex(channel2) else 0.0
...
def _time_shifted_filters_cartesian(
    ...
    if num_samples == 0:
        return ratio_r.new_empty((0,)), ratio_i.new_empty((0,))
    ...
    for start in range(0, num_samples, block_size):
        ...
        sample_indices = torch.arange(
            start, stop, device=ratio_r.device, dtype=freqs.dtype
        )
        ...
        filters_r.append((t1_r + t2_r).sum(dim=-1))
        filters_i.append(-(t1_i + t2_i).sum(dim=-1))

    return torch.cat(filters_r), torch.cat(filters_i)
...
def _time_shifted_filters(freqs, tstart, delta_t, num_samples, ratio, a0, a1):
    ...
    ratio_r = ratio.real if torch.is_complex(ratio) else ratio
    ratio_i = ratio.imag if torch.is_complex(ratio) else 0.0
    a0_r = a0.real if torch.is_complex(a0) else a0
    a0_i = a0.imag if torch.is_complex(a0) else 0.0
    a1_r = a1.real if torch.is_complex(a1) else a1
    a1_i = a1.imag if torch.is_complex(a1) else 0.0
...
def snr_predictor(freqs, tstart, delta_t, num_samples, hp, hc, h00, a0, a1, b0, b1):
    ...
    hp_r, hp_i = (hp.real, hp.imag) if torch.is_complex(hp) else (hp, 0.0)
    hc_r, hc_i = (hc.real, hc.imag) if torch.is_complex(hc) else (hc, 0.0)
    h00_r, h00_i = (h00.real, h00.imag) if torch.is_complex(h00) else (h00, 0.0)
    ...
    snr2 = (sh_r.square() + sh_i.square()) / (2.0 * hh) + (
        csh_r.square() + csh_i.square()
    ) / (2.0 * chh)
...
def snr_predictor_dom(freqs, tstart, delta_t, num_samples, hp, h00, a0, a1, b0, b1):
    ...
    return torch.complex(sh_r, sh_i), hh
