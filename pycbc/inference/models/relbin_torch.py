...
def _explicit_antenna_response(response, x0, x1, y0, y1, y2):
    ...
    dx0 = response[0, 0] * x0 + response[0, 1] * x1 + response[0, 2] * zero
    dx1 = response[1, 0] * x0 + response[1, 1] * x1 + response[1, 2] * zero
    dx2 = response[2, 0] * x0 + response[2, 1] * x1 + response[2, 2] * zero
...
def _zero_polarization_response_and_delay(
    ...
    if time_tensor is not None or time_is_array:
        ...
        if like.device.type != "cpu" or right_ascension.ndim or declination.ndim:
            right_ascension, declination, relative_time = torch.broadcast_tensors(
                right_ascension, declination, relative_time
            )
        phase_offsets = relative_time / float(detector.sday) * (2.0 * torch.pi)
    ...
    gha_start = (
        torch.as_tensor(gmst_start, device=like.device, dtype=dtype) - right_ascension
    )
    ...
    if (
        ...
        fplus, fcross = _explicit_antenna_response(response, x0, x1, y0, y1, y2)
    else:
...
def detector_response(detector, right_ascension, declination, times, like):
    ...
    if all(
        ...
        for name in ("response", "location", "sday", "gmst_estimate")
    ):
