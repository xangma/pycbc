...
def create_data_parser():
    ...
    dqgroup = parser.add_argument_group("Options for quering data quality (DQ)")
    dqgroup.add_argument(
        "--dq-segment-name",
        default="DATA",
        help='The status flag to query for data quality. Default is "DATA".',
    )
    dqgroup.add_argument(
        "--dq-source",
        choices=["any", "GWOSC", "dqsegdb"],
        default="any",
...
def check_validtimes(
    ...
    **kwargs,
    ...
    if MPI is not None and (
        ...
        sleep(120)
    validsegs = dq.query_flag(
        detector, segment_name, check_start, check_end, cache=True, **kwargs
    )
    use_start = gps_start
    use_end = gps_end
    # shift if necessary
    ...
    if (use_start, use_end) not in validsegs:
        raise NoValidDataError(
            "Could not find a continous valid segment in in detector {}".format(
                detector
            )
        )
...
def detectors_with_valid_data(
    ...
    **kwargs,
    ...
    for det in detectors:
        logging.info("Checking that %s has valid data in the requested segment", det)
        try:
            pad = pad_data[det]
            start, end = check_validtimes(
                det, gps_start_times[det] - pad, gps_end_times[det] + pad, **kwargs
            )
...
def data_opts_from_config(cp, section, filter_flow):
    ...
    for det in opts.instruments:
        ...
        if opts.psd_inverse_length[det] is not None:
            ...
            logging.info(
                "Padding %s analysis start and end times by %d "
    ...
    if opts.low_frequency_cutoff:
        ...
        low_freq_cutoff.update(
            {
                det: opts.low_frequency_cutoff[det]
                for det in opts.instruments
        ...
        if any(low_freq_cutoff[det] > filter_flow[det] for det in filter_flow):
            raise ValueError(
                "data conditioning low frequency cutoff must "
...
def data_from_cli(
    ...
    if instruments == []:
        raise NoValidDataError(
            "No valid data could be found in any of the requested instruments."
        )
