...
def inner(left, right):
    ...
    is_comp = (compiler is not None and compiler()) or (
        dynamo is not None and dynamo()
    )
    if left.device.type != "mps" and not is_comp:
        try:
            return torch.vdot(
...
def phase_reconstruction_values(sh, hh, sample_count=int(1e4)):
    ...
    sh_r = sh.real if torch.is_complex(sh) else sh
    sh_i = sh.imag if torch.is_complex(sh) else 0.0
