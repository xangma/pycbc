...
def _to_tensor(value, like, *, real=False):
    ...
    if real:
        ...
        dtype = (
            torch.complex128 if numpy.iscomplexobj(value) else torch.float64
        )
...
def fused_inner_hd_hh(h, d, weight=None):
    ...
    if is_batched:
        if h_tensor.device.type == "mps":
            real_acc_dtype = (
                h_tensor.real.dtype if dtype.is_complex else dtype
            )
            accumulation_dtype = dtype
        else:
        ...
        if dtype.is_complex:
            ...
            cplx_hd = torch.sum(
                h_acc * d_acc, dim=-1, dtype=accumulation_dtype
            )
    ...
    d_acc = d_tensor.to(
        dtype=accumulation_dtype, device=h_tensor.device
    ).reshape(-1)
    if dtype.is_complex:
        try:
            ...
            cplx_hd = torch.complex(real_hd, imag_hd)
        hh = torch.sum(
            torch.view_as_real(h_acc).square(), dtype=torch.float64
        )
    else:
...
def selected_values(values, indices, *, host=True):
    ...
    indices = torch.as_tensor(
        indices, device=tensor.device, dtype=torch.int64
    )
...
def draw_sample(loglr, size=None, *, host=True):
    ...
    if size and not host:
        ...
        uniforms = (
            ...
            if size else numpy.random.uniform()
        )
...
def rect_bivariate_spline_evaluator(interp):
    ...
    def evaluate(x, y, bounds_check=True):
        ...
        indices_x, basis_x = _bspline_basis(
            x_tensor, tensor_knots_x, degree_x
        )
        indices_y, basis_y = _bspline_basis(
...
def marginalize_likelihood(sh, hh, logw, phase, distance, skip_vector,
                           return_peak, return_complex, interpolator=None):
