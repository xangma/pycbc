        ...
        for m1, m2 in itertools.combinations(models, 2):
            ...
            for ifo in self.data:
                ...
                if any(_torch_tensor(value) is not None
                    ...
                    bins, fedge = active_edge_bins(
                        h1, h2, m1.f[ifo], edge)
                else:
                    keep = numpy.where(
                        (h1[edge] != 0) | (h2[edge] != 0))[0]
                    edge = edge[keep]
                    fedge = m1.f[ifo][edge]
                    bins = numpy.array([
                            (edge[i], edge[i + 1])
