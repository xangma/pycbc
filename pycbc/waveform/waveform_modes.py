...
def _formatdocstr(docstr):
    ...
    return docstr.format(
        **{
            _p[1]: getattr(parameters, _p[1])
            .docstr(prefix="    ", include_label=False)
            .lstrip(" ")
            for _p in Formatter().parse(docstr)
            if _p[1] is not None
        }
    )
...
def _formatdocstrlist(docstr, paramlist, skip_params=None):
    ...
    pl = "\n".join(
        [
            _p.docstr(prefix="    ", include_label=False)
            for _p in paramlist
    ...
    if hlms:
        ...
        if isinstance(first_hlm, FrequencySeries) and first_tensor is not None:
            ...
            if all(2 <= ell <= 4 and abs(emm) <= ell for ell, emm in modes):
                ...
                res_r = torch.matmul(yr, hr) - torch.matmul(yi, hi)
                res_i = torch.matmul(yr, hi) + torch.matmul(yi, hr)
                res_tensor = torch.complex(res_r, res_i)
...
def default_modes(approximant):
    ...
    if approximant in ["IMRPhenomXPHM", "IMRPhenomXHM"]:
        ...
        raise ValueError(
            "I don't know what the default modes are for approximant {}, sorry!".format(
                approximant
            )
        )
    return ma
...
_mode_waveform_td = {
    "EOBNRv2": get_lalsimulation_modes,
