...
def _abs_scalar_sph_pi_over2(l: int, m: int) -> float:
    ...
    LALSimIMREOBNewtonianMultipole.c:272-307 (GSL→SciPy)."""
    ...
    *,
    ...
    if l == 2 and m == 2:
        ...
        f21v3 = torch.where(
            ...
            torch.tensor(3.0 / 8.0, device=device, dtype=dtype) * chiA,
        )
        aux = (v * f21v1 + (v * v * v) * f21v3).to(aux.dtype)
        ...
        if waveform:
            ...
            aux = aux + (v ** 4) * f21v4 + (v ** 5) * f21v5 + (v ** 6) * f21v6 + (v ** 7) * cal21_t
        ...
        rho33v8l = 13.0 / 3.0
        rho33v10 = torch.tensor(0.0, device=device, dtype=dtype)
        rho33v10l = torch.tensor(0.0, device=device, dtype=dtype)
        if waveform:
            rho33v6 = rho33v6 + (-129509.0 / 25740.0 + 41.0 * _PI * _PI / 192.0) * eta - 274621.0 / 154440.0 * eta2 + 12011.0 / 46332.0 * eta3
            rho33v10 = torch.tensor(-903823148417327.0 / 30566888352000.0, device=device, dtype=dtype)
            rho33v10l = torch.tensor(87347.0 / 13860.0, device=device, dtype=dtype)
        ...
        f33v3 = torch.where(
            ...
            torch.tensor(0.375, device=device, dtype=dtype) * chiA,
        )
        f33v4 = torch.tensor(0.0, device=device, dtype=dtype)
        f33v5 = torch.tensor(0.0, device=device, dtype=dtype)
        f33v6 = torch.tensor(0.0, device=device, dtype=dtype)
        f33vh6 = torch.tensor(0.0, device=device, dtype=dtype)
        ...
        rho44v8 = torch.tensor(0.0, device=device, dtype=dtype)
        rho44v8l = torch.tensor(0.0, device=device, dtype=dtype)
        rho44v10 = torch.tensor(0.0, device=device, dtype=dtype)
        rho44v10l = torch.tensor(0.0, device=device, dtype=dtype)
        if waveform:
            chiS2 = chiS * chiS
            chiA2 = chiA * chiA
            rho44v4 = (
            ...
            rho44v8 = torch.tensor(-172066910136202271.0 / 19426955708160000.0, device=device, dtype=dtype)
            rho44v8l = torch.tensor(845198.0 / 190575.0, device=device, dtype=dtype)
            rho44v10 = torch.tensor(-17154485653213713419357.0 / 568432724020761600000.0, device=device, dtype=dtype)
            rho44v10l = torch.tensor(22324502267.0 / 3815311500.0, device=device, dtype=dtype)
        ...
        rho55v6 = torch.tensor(0.0, device=device, dtype=dtype if not waveform else dtype)
        rho55v6l = torch.tensor(0.0, device=device, dtype=dtype if not waveform else dtype)
        rho55v8 = torch.tensor(0.0, device=device, dtype=dtype)
        rho55v8l = torch.tensor(0.0, device=device, dtype=dtype)
        rho55v10 = torch.tensor(0.0, device=device, dtype=dtype)
        rho55v10l = torch.tensor(0.0, device=device, dtype=dtype)
        if waveform:
            rho55v6 = torch.tensor(190606537999247.0 / 11957879934000.0, device=device, dtype=dtype)
            rho55v6l = torch.tensor(-1546.0 / 429.0, device=device, dtype=dtype)
            rho55v8 = torch.tensor(-1213641959949291437.0 / 118143853747920000.0, device=device, dtype=dtype)
            rho55v8l = torch.tensor(376451.0 / 83655.0, device=device, dtype=dtype)
            rho55v10 = torch.tensor(-150082616449726042201261.0 / 4837990810977324000000.0, device=device, dtype=dtype)
            rho55v10l = torch.tensor(2592446431.0 / 456756300.0, device=device, dtype=dtype)
        rho = 1.0 + v * v * (
            rho55v2
            + v
            * (
        ...
        if waveform:
            ...
            aux = (v ** 3) * f55v3 + (v ** 4) * f55v4 + (v ** 5) * cal55_t
            aux = aux.to(complex_dtype)
    ...
    delta = torch.zeros_like(v, device=device, dtype=dtype)
    if l == 2 and m == 2:
        delta22vh3 = 7.0 / 3.0
...
def _safe_norm(x: torch.Tensor, eps: float = 1e-15) -> torch.Tensor:
    return _lal_scalar_sqrt(torch.clamp(_dot3(x, x), min=eps))
...
def _phi_hat(r_vec: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    phi_hat = phi_hat / torch.clamp(_safe_norm(phi_hat).unsqueeze(-1), min=1.0e-15)
    ...
    if aligned.any():
        ...
        sin_theta = torch.where(aligned, torch.tensor(1.0, device=r_vec.device, dtype=r_vec.dtype), sin_theta)
...
def _precessing_spherical_initial_conditions(
    ...
    *,
    ...
    try:
        ...
        root_entry_hcoeffs = dict(params.hcoeffs)
        gsl_root = None
        if omega_target.device.type == "cpu" and omega_target.dtype == torch.float64:
            gsl_root = _gsl_multiroot_hybrids(
                residual,
                guess,
                epsabs=1.0e-9,
                max_iter=10000,
            )
        if gsl_root is not None:
            sol_x, final_res = gsl_root
            sol_success = True
            sol_message = "gsl_multiroot_fsolver_hybrids"
        else:
            params.hcoeffs = root_entry_hcoeffs
            params.tortoise = 0
            # Fallback for environments without GSL. LAL stops the GSL hybrid
            # solve with gsl_multiroot_test_residual(..., 1e-9); using a tighter
            # SciPy tolerance over-solves to a slightly different point.
            sol = scipy.optimize.root(residual, guess, method="hybr", tol=1.0e-9, options={"maxfev": 80})
            sol_x = sol.x
            final_res = getattr(sol, "fun", [math.inf, math.inf, math.inf])
...
def _tail_factor_mag(l: int, m: int, omega: torch.Tensor, H: torch.Tensor):
    """Absolute |T_lm| matching LALSimIMRSpinEOBFactorizedWaveformPrec.c:675-709."""
    k = torch.abs(torch.tensor(float(m), device=omega.device, dtype=omega.dtype) * omega)
    hathatk = torch.clamp(H * k, min=1e-12)
    # prefactor sqrt(4 pi k / (1 - exp(-4 pi k)))
    pref = torch.sqrt((4.0 * _PI * hathatk) / torch.clamp(1.0 - torch.exp(-4.0 * _PI * hathatk), min=1e-16))
    # factorial term
...
def non_keplerian_vphi(
    ...
    *,
    ...
    """Non-Keplerian vPhi using the LAL (rΩ/r)^3 coefficient from dH/dpφ at pr=0.

    Mirrors XLALSimIMRSpinPrecEOBNonKeplerCoeff + follow-up scaling in
    LALSimIMRSpinEOBFactorizedWaveformPrec (vPhi = r * cbrt(coeff) * Omega).
    """

    pr0 = torch.zeros_like(r)
    if r_vec is None or p_vec is None:
        aligned_gauge = (
...
def _orbital_basis_from_L_phi(
    ...
    L_mag = _safe_norm(L_vec)
    Lhat = L_vec / torch.clamp(L_mag.unsqueeze(-1), min=1.0e-15)
...
def _tortoise_matrices(r_vec: torch.Tensor, csi: torch.Tensor, dcsi: torch.Tensor):
    ...
    for i in range(3):
        for j in range(i + 1):
            ...
            invT[..., i, j] = inv_tij
            invT[..., j, i] = inv_tij
    dT = torch.zeros((*r_vec.shape[:-1], 3, 3, 3), device=r_vec.device, dtype=r_vec.dtype)
    for i in range(3):
        for j in range(3):
            for k in range(3):
...
def _dH_dx_cartesian_autograd(
    ...
    """Autograd port of LAL's exact fixed-P Cartesian d(H/eta)/dX block."""

    old_tortoise = getattr(params, "tortoise", 2)
    params.tortoise = 2
    try:
        r_vec_var = r_vec.detach().clone().requires_grad_(True)
        p_vec = p_non_tortoise.detach().clone()
        r = _safe_norm(r_vec_var)
        n_hat = r_vec_var / r
        pr = _dot3(p_vec, n_hat)
        phi = torch.zeros((), device=r_vec.device, dtype=r_vec.dtype)
        L_vec = _cross3(r_vec_var, p_vec)
