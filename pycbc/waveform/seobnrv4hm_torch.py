...
Set ``PYCBC_SEOBNRV4HM_NATIVE=0`` to opt out for this model, or set
``PYCBC_TORCH_NATIVE_PORTS=0`` (or ``PYCBC_TORCH_NATIVE=0``) to disable
default-native waveform ports globally.

Apple MPS uses single precision. Requests starting below the conservative
dimensionless-frequency boundary ``M f_start = 1e-3`` retain the LAL path;
sequence requests use their lowest supplied frequency for this guard. Higher-
frequency MPS requests and all supported CPU/CUDA requests use the native
evaluator.
...
def _native_device_supported(params, *, sequence: bool) -> bool:
    """Bound MPS float32 phase error at very low starting frequencies."""
