...
"""Declarative registry for JAX-native waveform implementations.

This module deliberately has no dependency on jax or lalsimulation. Native
implementation modules are imported only after a matching waveform is called
under :class:`~pycbc.scheme.JAXScheme` with its feature switch enabled.
"""
...
from pycbc.waveform.jax_switches import jax_native_override
...
def _validate_ports(ports):
    ...
    if len(set(names)) != len(names):
        raise RuntimeError("duplicate JAX-native waveform registry entry")

    for port in ports:
        if not _declared_interfaces(port):
            raise RuntimeError(
                f"JAX-native waveform {port.approximant!r} has no interface"
            )
        for interface, (generator_field, supported_field) in _INTERFACE_FIELDS.items():
            ...
            if (generator is None) != (supported is None):
                raise RuntimeError(
                    f"JAX-native waveform {port.approximant!r} has an "
...
JAX_NATIVE_WAVEFORMS: Mapping[str, JAXNativeWaveform] = MappingProxyType(
...
def native_approximants(interface):
    ...
    return tuple(
        name
        for name, port in JAX_NATIVE_WAVEFORMS.items()
        if getattr(port, generator_field) is not None
    )


def try_jax_native_waveform(interface, params):
    """Run an enabled and supported native port, or return ``None``.

    The caller owns scheme selection. Parameter-support predicates remain
    authoritative, and environment overrides control whether
    ...
    port = JAX_NATIVE_WAVEFORMS.get(approximant)
    if port is None:
        return None
    ...
    override = jax_native_override(port.component_flag)
...
__all__ = (
    "JAX_NATIVE_WAVEFORMS",
    "JAXNativeWaveform",
    "native_approximants",
    "try_jax_native_waveform",
)
