...
from .imrphenomxhm_mode33_torch import (
    ...
    _collocation_targets,
    _evaluate_pruned_amplitude,
    _evaluate_pruned_phase,
    _get_scripted_boundary_lane_executor,
    _inspiral_boundary as _shared_inspiral_boundary,
    _mark_scripted_boundary_lane_failed,
    _poly,
    _qualified_uniform_grid_common,
    _scripted_boundary_lane_supported,
...
_SCRIPTED_BOUNDARY_LANE_ENV = (
    "PYCBC_IMRPHENOMXHM_MODE44_SCRIPTED_BOUNDARY_LANE"
)
_NATIVE_CPU_BOUNDARY_ENV = (
    "PYCBC_IMRPHENOMXHM_MODE44_NATIVE_CPU_BOUNDARY"
)
_REGION_PRUNING_ENV = "PYCBC_IMRPHENOMXHM_MODE44_REGION_PRUNING"
_REGION_PRUNING_MIN_SAMPLES = 512
_REGION_PRUNING_PHASE_MIN_SAMPLES = 1280


def _region_pruning_enabled():
    """Return the strict, off-by-default mode-44 piecewise switch."""
...
def _prepare_uniform_region_plan(
    ...
    *,
    ...
    """Prepare exact regular-grid mode-44 region starts in Python."""
...
def _scripted_boundary_lane_enabled():
    """Return the strict, off-by-default mode-44 scalar-lane switch."""
...
def _native_mode44_inspiral_boundary(
    function,
    point,
    like,
    parameters,
    ...
    try:
        from . import _imrphenomxhm_mode44_native as native

        return native.try_native_boundary(
            lambda: _shared_inspiral_boundary(function, point, like),
            point,
            like,
            parameters,
        )
...
def _inspiral_boundary(
    ...
    *,
    ...
    """Evaluate mode 44's exact fourth-order inspiral boundary stencil."""
    ...
    if (
        ...
        native_result = _native_mode44_inspiral_boundary(
            function,
            point,
            like,
    ...
    if not _scripted_boundary_lane_enabled():
        return _shared_inspiral_boundary(function, point, like)
    ...
    # An enabled mode-44 lane always fails closed to the established scalar
    # stencil, independently of the separate experimental bulk-stencil gate.
    derivative = (
...
def _mode44_state(
    ...
    *,
    ...
    return _Mode44State(
        base=base,
        f_ring_44=(
            _qnm_fring_44(base.final_spin) / final_mass
            if ringdown_frequency is None
            else ringdown_frequency
        ),
        f_damp_44=(
            _qnm_fdamp_44(base.final_spin) / final_mass
...
_phase_44 = _legacy_staged_solver(_phase_44_staged)
...
_amplitude_44_2022 = _legacy_staged_solver(_amplitude_44_2022_staged)
...
def _amplitude_44(
    ...
    if amplitude_release == 122022:
        return _amplitude_44_2022(mf, state, region_indices)

    from .imrphenomxhm_mode44_2019_torch import amplitude_44_2019

    return amplitude_44_2019(mf, state)
