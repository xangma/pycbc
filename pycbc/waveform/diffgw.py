...
def _get_diffgw_generator():
    """Import diffgw or fall back to torchwave."""
    if importlib.util.find_spec('diffgw') is not None:
        try:
            import diffgw
            return diffgw.get_fd_waveform
        except Exception:
            pass
    import torchwave
    return torchwave.get_fd_waveform
...
def generate_batch(bank, indices, device='cpu', dtype=None, delta_f=None):
    ...
    if native:
        diffgw_gen = _get_diffgw_generator()
        provider_name = getattr(bank, 'waveform_provider_name', 'diffgw')
