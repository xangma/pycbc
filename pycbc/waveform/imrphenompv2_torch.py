...
merger-time alignment, NRTidal corrections, and polarization assembly execute
on the active Torch device.

Supported requests use the native evaluator by default under ``TorchScheme``.
Set ``PYCBC_IMRPHENOMPV2_NATIVE=0`` to opt out for this family, or set
``PYCBC_TORCH_NATIVE_PORTS=0`` (or ``PYCBC_TORCH_NATIVE=0``) to disable
default-native ports whose component switch is unset. Unsupported options
continue to fall back to lalsimulation. On Apple MPS, very-low-frequency
requests retain the LAL path to bound single-precision phase error.
"""
...
def phenomp_source_frame_parameters(
    mass1,
    mass2,
    f_ref,
    coa_phase,
    ...
    *,
    ...
    ``orbital_angular_momentum`` selects the PN prescription used to orient
    the total-angular-momentum frame. PhenomPv2 uses the default 2PN
    expression; PhenomPv3 supplies its spinning 3PN evaluator for precessing
    systems.
    """
...
def _native_device_supported(params, *, sequence):
    ...
    try:
        ...
        if sequence:
            ...
            # can therefore evaluate below a non-bin-aligned f_lower.
...
def _build_model(inputs):
    ...
    fixed_frequencies = torch.linspace(
        fixed_start,
        fixed_stop,
        10,
        dtype=inputs.real_dtype,
    ...
    if inputs.tidal_version is not None:
        fixed_phase += _tidal_phase(inputs, fixed_frequencies)
    fixed_phase = -fixed_phase
    spline_coefficients = _natural_cubic_coeff(
        fixed_frequencies, fixed_phase
    )
...
def _assemble_twisted_polarizations(
    ...
    *,
    ...
    """Assemble PhenomP polarizations from co-precessing (2, 2) modes.

    ``h_phenom`` has the usual symmetry between the positive- and
    negative-m modes.  If supplied, ``h_phenom_antisymmetric`` is twisted
    with the opposite polarization symmetry before the common frame
    ...
    if h_phenom_antisymmetric is not None:
        hp_antisymmetric_sum = torch.zeros_like(h_phenom)
        hc_antisymmetric_sum = torch.zeros_like(h_phenom)
    for index, emm in enumerate(range(-2, 3)):
        ...
        hc_sum += 1j * (t2m - tm2m)
        if h_phenom_antisymmetric is not None:
            hp_antisymmetric_sum += t2m - tm2m
            hc_antisymmetric_sum += 1j * (t2m + tm2m)

    epsilon_factor = torch.exp(-2j * epsilon) / 2.0
    factor = epsilon_factor * h_phenom
    plus = factor * hp_sum
    cross = factor * hc_sum
    if h_phenom_antisymmetric is not None:
        factor = epsilon_factor * h_phenom_antisymmetric
        plus += factor * hp_antisymmetric_sum
        cross += factor * hc_antisymmetric_sum
...
def imrphenompv2_fd_torch(**params):
    ...
    npoints = _lal_next_power_of_two(layout_max / delta_f) + 1
