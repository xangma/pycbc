...
def _torch_native_spa(
    ...
    """
    f_dtype = torch.float64 if dtype_out == torch.complex128 else torch.float32
    ...
    frequencies = (torch.arange(n, device=device, dtype=table_dtype) + kmin) * delta_f
    pi_m_root = torch.pow(
    ...
    k_idx = torch.arange(kmin + 1, kmin + n + 1, device=device, dtype=table_dtype)
    ...
    ph = ph * (pfaN_t / v5) - torch.tensor(math.pi / 4.0, dtype=f_dtype, device=device)
    ...
    sinp = (
        torch.tensor(0.225, device=device, dtype=f_dtype)
        * (sinp * torch.abs(sinp) - sinp)
        + sinp
    )
...
def _torch_native_spa_sequence_frequencies(
    ...
    v = (pi_m_root.to(table_dtype) * torch.pow(frequency_table, 1.0 / 3.0)).to(f32)
    logv = (torch.log(frequency_table) / 3.0 + log_pi_m_root.to(table_dtype)).to(f32)
    amp = (
    ...
    phase = (phase + coefficient(pfa5) + coefficient(pfl5) * logv) * v
    ...
    cosp = 1.273239545 * cosine_phase - 0.405284735 * cosine_phase.abs() * cosine_phase
...
def _cpu_reference(
    ...
    amp_freqs = _np.arange(kmin + 1, kmin + n + 1, dtype=_np.float64) * delta_f
