...
Wigner rotation, merger-time alignment, and polarization assembly run on the
active Torch device. Only scalar coefficient setup remains on the host.

Supported requests use the native evaluator by default under ``TorchScheme``.
Set ``PYCBC_IMRPHENOMP_NATIVE=0`` to opt out for this model, or set
...
continue to use lalsimulation. On Apple MPS, low-frequency requests retain the
LAL path to bound single-precision phase error.
"""
...
_PI = math.pi
_F_CUT = 0.15
_MPS_MIN_START_MF = 7.5e-4
_MPS_Q_START_MF_SCALE = 1.0e-4
...
def _native_device_supported(params, *, sequence):
    ...
    if not all(
        ...
        for value in (total_mass, mass_ratio, start_frequency)
    ):
        return False
    start_mf = total_mass * _MTSUN_SI * start_frequency
    minimum_mf = max(
        _MPS_MIN_START_MF,
        _MPS_Q_START_MF_SCALE * mass_ratio,
    )
    return math.isfinite(start_mf) and start_mf >= minimum_mf
