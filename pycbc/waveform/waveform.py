...
def _lalsim_fd_waveform(**p):
    ...
    if (
        ...
        from .imrphenomc_torch import (
            imrphenomc_fd_torch,
            imrphenomc_native_supported,
        )

        if imrphenomc_native_supported(p):
            return imrphenomc_fd_torch(**p)
    ...
    if (
        ...
        from .imrphenomd_torch import (
            imrphenomd_fd_torch,
            imrphenomd_native_supported,
        )

        if imrphenomd_native_supported(p):
    ...
    if (
        ...
        from .imrphenomp_torch import (
            imrphenomp_fd_torch,
            imrphenomp_native_supported,
        )

        if imrphenomp_native_supported(p):
            return imrphenomp_fd_torch(**p)
    ...
    if (
        ...
        from .imrphenompv2_torch import (
            imrphenompv2_fd_torch,
            imrphenompv2_native_supported,
        )

        if imrphenompv2_native_supported(p):
            return imrphenompv2_fd_torch(**p)
    ...
    if (
        ...
        if imrphenomxhm_fd_native_supported(p):
            return imrphenomxhm_fd_torch(**p)

    if (
        p.get("approximant") == "IMRPhenomXPHM"
        and using_torch
        and torch_native_enabled("PYCBC_IMRPHENOMXPHM_NATIVE", default=False)
    ):
        from .imrphenomxphm_torch import (
            imrphenomxphm_fd_torch,
            imrphenomxphm_native_supported,
        )

        if imrphenomxphm_native_supported(p):
            return imrphenomxphm_fd_torch(**p)
    ...
    if (
        ...
        if eobnrv2_native_supported(p):
            return eobnrv2_fd_torch(**p)
...
def _lalsim_fd_sequence(**p):
    ...
    if (
        ...
        if imrphenomc_sequence_native_supported(p):
            return imrphenomc_fd_sequence_torch(**p)
    ...
    if (
        ...
        if imrphenomp_sequence_native_supported(p):
            return imrphenomp_fd_sequence_torch(**p)
    ...
    if (
        ...
        if imrphenompv2_sequence_native_supported(p):
            return imrphenompv2_fd_sequence_torch(**p)
    ...
    if (
        ...
        if imrphenomxphm_sequence_native_supported(p):
            return imrphenomxphm_fd_sequence_torch(**p)
