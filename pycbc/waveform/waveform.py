...
default_args = (
    parameters.fd_waveform_params.default_dict() + parameters.td_waveform_params
).default_dict()
...
cuda_td = dict(
    list(_lalsim_td_approximants.items()) + list(_cuda_td_approximants.items())
)
cuda_fd = dict(
    list(_lalsim_fd_approximants.items()) + list(_cuda_fd_approximants.items())
)
...
get_fd_waveform_sequence.__doc__ = get_fd_waveform_sequence.__doc__.format(
    params=parameters.fd_waveform_sequence_params.docstr(
        prefix="    ", include_label=False
    )
)
...
get_td_waveform.__doc__ = get_td_waveform.__doc__.format(
    params=parameters.td_waveform_params.docstr(prefix="    ", include_label=False)
)
...
get_fd_waveform.__doc__ = get_fd_waveform.__doc__.format(
    params=parameters.fd_waveform_params.docstr(prefix="    ", include_label=False)
)
...
def _base_get_td_waveform_from_fd(template=None, rwrap=None, **params):
    ...
    if rwrap is None:
        ...
        if mass_spin_params.issubset(set(nparams.keys())):
            m_final, spin_final = get_final_from_initial(
                mass1=nparams["mass1"],
                mass2=nparams["mass2"],
                spin1z=nparams["spin1z"],
                spin2z=nparams["spin2z"],
            )
    ...
    if nparams["approximant"] not in _filter_time_lengths:
        raise ValueError(
            "Approximant %s _filter_time_lengths function \
                         not available"
            % (nparams["approximant"])
        )
    ...
    while full_duration < duration * 1.5:
        ...
        if "t_obs_start" in nparams and full_duration >= nparams["t_obs_start"]:
            break
    ...
    fudge_duration = (max(0, full_duration) + 0.1 + rwrap) * 1.5
    ...
    if nparams["approximant"] not in fd_det:
        ...
        for ifo in wfs.keys():
            ...
            wfs[ifo] = wfutils.fd_to_td(
                wfs[ifo],
                delta_t=kwds["delta_t"],
                left_window=(nparams["f_lower"], kwds["f_lower"]),
            )
        return wfs
...
def seobnrv2_final_frequency(**kwds):
    return pnutils.get_final_freq(
        "SEOBNRv2", kwds["mass1"], kwds["mass2"], kwds["spin1z"], kwds["spin2z"]
    )
...
if "PYCBC_WAVEFORM" in os.environ:
    ...
    for mod in mods:
        ...
        mhandle.add_me(
            cpu_fd=cpu_fd, cpu_td=cpu_td, filter_time_lengths=_filter_time_lengths
        )
...
def get_waveform_filter(out, template=None, **kwargs):
    ...
    if input_params["approximant"] in fd_approximants(_scheme.mgr.state):
        ...
        hp, _ = wav_gen[input_params["approximant"]](
            duration=duration, return_hc=False, **input_params
        )
        ...
        if "taper" in input_params.keys() and input_params["taper"] is not None:
...
def td_waveform_to_fd_waveform(waveform, out=None, length=None, buffer_length=100):
    ...
    if out is None:
        if length is None:
            N = pnutils.nearest_larger_binary_number(len(waveform) + buffer_length)
            n = int(N // 2) + 1
        else:
            n = length
            N = (n - 1) * 2
        ...
        n = len(out)
        N = (n - 1) * 2
    delta_f = 1.0 / (N * waveform.delta_t)
...
def get_two_pol_waveform_filter(outplus, outcross, template, **kwargs):
    ...
    if input_params["approximant"] in fd_approximants(_scheme.mgr.state):
        ...
        if "taper" in input_params.keys() and input_params["taper"] is not None:
            hp = hp.taper_timeseries(
                location=input_params["taper"],
                tapermethod=input_params.get("taper_method", "lal"),
                taper_window=input_params.get("taper_window"),
                return_lal=False,
            )
            hc = hc.taper_timeseries(
                location=input_params["taper"],
