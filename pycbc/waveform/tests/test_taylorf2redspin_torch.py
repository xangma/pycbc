...
def test_taylorf2redspin_matches_lalsimulation(
    ...
    monkeypatch.setenv(_native_flag(approximant), "1")
...
def test_taylorf2redspin_unsupported_options_reach_lal(
    monkeypatch,
    preserve_scheme,
):
    import pycbc.waveform.taylorf2redspin_torch as native_mod
...
def test_taylorf2redspin_sequence_conventions(monkeypatch, preserve_scheme):
    monkeypatch.setenv("PYCBC_TAYLORF2REDSPINTIDAL_NATIVE", "1")
    _activate_scheme(_scheme.TorchScheme("cpu"))
...
def test_sequence_unsupported_options_reach_lal(
    monkeypatch,
    preserve_scheme,
):
    import pycbc.waveform.taylorf2redspin_torch as native_mod
