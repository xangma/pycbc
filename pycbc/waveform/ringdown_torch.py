...
def td_damped_sinusoid(
    ...
    decay = torch.exp(damping)
    phase1 = omegalm + phi
    cos1 = torch.cos(phase1)
    sin1 = torch.sin(phase1)
    xr, xi = (xlm.real, xlm.imag) if torch.is_complex(xlm) else (xlm, 0.0)
    ...
    yr, yi = (xlnm.real, xlnm.imag) if torch.is_complex(xlnm) else (xlnm, 0.0)
...
def fd_damped_sinusoid(
    ...
    denom_r = (
        1.0 - 4.0 * _common.pi_sq * (freqs * freqs - f_0 * f_0) * (tau * tau)
    )
    ...
    if t_0 != 0:
        ...
        shift_r = torch.cos(shift_phase)
        shift_i = torch.sin(shift_phase)
    ...
    return torch.complex(hp_r, hp_i), torch.complex(hc_r, hc_i)
...
def multimode_base(input_params, domain, freq_tau_approximant=False):
    ...
    if domain == "td":
        ...
        sample_grid = torch.arange(
            len(outplus), device=device, dtype=dtype
