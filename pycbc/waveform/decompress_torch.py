...
calculations are done in float64 even when the output is float32 to better
match the CPU path (which accumulates in double); the final values are cast
to the output dtype. Comments mark the deliberate precision choices.
"""
...
def _work_dtype():
    """Use float64/complex128 for intermediate math for closer CPU parity."""
    return torch.float64, torch.complex128
...
def _decomp_main_loop_torch(degree, amp, phase, sample_frequencies, output,
    ...
    work_real, work_complex = _work_dtype()
