...
Torch device. Small phenomenological fits and the disruption-radius root
solve remain host-side scalar work.

Activation
----------
- Per-model flag: ``PYCBC_IMRPHENOMNSBH_NATIVE=1``
...
def imrphenomnsbh_native_supported(params) -> bool:
    """Return whether ``params`` are covered by the native implementation."""
...
def imrphenomnsbh_sequence_native_supported(params) -> bool:
    ...
    return imrphenomnsbh_native_supported(params)
...
def _imrphenomnsbh_inputs(params, *, sequence=False):
    ...
    if not imrphenomnsbh_native_supported(params):
        raise ValueError(
