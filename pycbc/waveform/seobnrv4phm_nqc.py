...
def solve_nqc_coeffs(eta: float, chiS: float, chiA: float, *, mode_l=2, mode_m=2, eob_peak=None):
    """Compute NQC coefficients using an in-place linear solve (torch/NumPy friendly).
    ...
    if eob_peak is not None:
        ...
        try:
            a_sol = np.linalg.solve(qMatrix, rhs_a)
        except np.linalg.LinAlgError:
            a_sol = np.linalg.lstsq(qMatrix, rhs_a, rcond=None)[0]
        ...
        pMatrix = np.array([[-p1dot, -p2dot], [-p1ddot, -p2ddot]], dtype=float)
        rhs_b = np.array([peak_omega_nr - omega_e, peak_omegadot_nr - omegadot_e], dtype=float)
        try:
            b_sol = np.linalg.solve(pMatrix, rhs_b)
        except np.linalg.LinAlgError:
            b_sol = np.linalg.lstsq(pMatrix, rhs_b, rcond=None)[0]
