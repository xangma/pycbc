...
On MPS, inputs below a mass-ratio-scaled dimensionless frequency of 1e-3 are
evaluated with Torch float64 on the CPU, then returned to MPS as complex64
arrays. This uses the TaylorF2 phase bound with a stricter reduced-spin margin.
...
from pycbc.constants import C_SI, GAMMA, MTSUN_SI, PC_SI
from pycbc.constants import PI as _PI
from pycbc.types import Array as PyCBCArray
from pycbc.types import FrequencySeries
from pycbc.types.array_torch import TorchArrayData

from .taylorf2_torch import _native_device_supported as _phase_device_supported
...
def taylorf2redspin_native_supported(params) -> bool:
    """Return whether the Torch port supports these features.

    Low-frequency MPS inputs use its CPU precision fallback, including for
    sequences, which LAL does not implement for these approximants.
    """
...
def _validated_inputs(params, *, sequence=False) -> _Inputs:
    if not taylorf2redspin_native_supported(params):
        raise ValueError(
            "TaylorF2RedSpin parameters are not supported by the native Torch path"
        )
    ...
    if approximant == "TaylorF2RedSpin":
        if abs(spin1z) > 1.0 or abs(spin2z) > 1.0:
            raise ValueError("TaylorF2RedSpin component spins must be between -1 and 1")
    ...
    # RedSpin does not subtract a phase at f_ref. Its lowest evaluated
    # frequency sets the same leading inspiral-phase roundoff bound as
    # TaylorF2. Keep sequence support by using Torch CPU precision rather
    # than falling back to LAL's unsupported sequence interface.
    if not _phase_device_supported(
        dict(params, f_ref=0.0), sequence=sequence, minimum_phase_mf=1.0e-3
    ):
    ...
    return _Inputs(
        ...
        device=device,
        output_device=state.torch_device,
...
def _coefficients(inputs: _Inputs) -> _Coefficients:
    ...
    psi4_spin = (
        63845.0 * (-81.0 + 4.0 * eta) * chi * chi / (8.0 * eta_factor * eta_factor)
    )
    psi5_spin = (
        -565.0
    ...
    alpha4_spin = (
        12769.0 * chi * chi * (-81.0 + 4.0 * eta) / (32.0 * eta_factor * eta_factor)
    )
    alpha5_spin = (
        -113.0
        * chi
    ...
    psi4 = (
        15293365.0 / 508032.0 + 27145.0 * eta / 504.0 + 3085.0 * eta2 / 72.0 + psi4_spin
    )
    ...
    *,
    ...
    amplitude = (
        ...
        * (
            ...
            + (
                coefficients.alpha6
                + coefficients.alpha6_log * (GAMMA + math.log(4.0) + log_velocity)
            )
            * v6
...
def _output_storage(samples, inputs):
    """Return CPU precision-fallback results to the requested device."""
    dtype = (
        torch.complex64 if inputs.output_device.type == "mps" else inputs.complex_dtype
    )
    return TorchArrayData(samples.to(device=inputs.output_device, dtype=dtype))
...
def taylorf2redspin_fd_torch(**params):
    ...
    if delta_f <= 0.0 or f_lower <= 0.0:
        raise ValueError("TaylorF2RedSpin delta_f and f_lower must be positive")
    if f_final < 0.0:
    ...
    if first_bin < length:
        ...
        raw[first_bin:] = _model_samples(
            ...
            coefficients,
            frequencies,
            # At integer bins, -1/df contributes whole phase cycles. Keep
            # only the GPS-rounding residual to avoid large MPS arguments.
            time_shift=epoch + 1.0 / delta_f,
        )
    plus, cross = _polarizations(inputs, raw)
    return (
        FrequencySeries(
            _output_storage(plus, inputs), delta_f=delta_f, epoch=epoch, copy=False
        ),
        FrequencySeries(
            _output_storage(cross, inputs), delta_f=delta_f, epoch=epoch, copy=False
        ),
    )
...
def _sequence_frequencies(sample_points, inputs: _Inputs) -> torch.Tensor:
    ...
    if isinstance(values, torch.Tensor) and values.device != inputs.device:
        # Transfer before widening: MPS cannot cast to float64 on-device.
        values = values.to(device=inputs.device)
    ...
    if frequencies.ndim != 1 or frequencies.numel() == 0:
        raise ValueError("TaylorF2RedSpin sample_points must be a non-empty vector")
...
def taylorf2redspin_fd_sequence_torch(**params):
    ...
    return (
        PyCBCArray(_output_storage(plus, inputs), copy=False),
        PyCBCArray(_output_storage(cross, inputs), copy=False),
    )
