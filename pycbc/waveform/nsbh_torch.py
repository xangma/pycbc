...
Scalar fit setup remains on the host, including a float64 Torch companion
solve for the disruption polynomial. The frequency-dependent correction is
evaluated on the Torch device of the supplied frequency tensor.
"""
...
def nsbh_xi_tide(mass_ratio: float, spin: float, mu: float) -> float:
    ...
    candidates = roots.real[
        (torch.abs(roots.imag) < 1.0e-5) & (roots.real > 0.0)
    ]
    if candidates.numel() == 0:
        return 0.0
    return float(torch.max(candidates**2))
