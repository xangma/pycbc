...
"""Fail-closed native CPU boundary helper for IMRPhenomXHM mode 33."""
...
_NATIVE_BOUNDARY_ENV = "PYCBC_IMRPHENOMXHM_MODE33_NATIVE_CPU_BOUNDARY"
_NATIVE_CFLAGS = (
    "-O3",
    "-fno-fast-math",
    "-ffp-contract=off",
...
# Mode 33 and mode 44 have the same seven-coefficient PN/pseudo-PN stencil.
# The extension takes the mode-specific global factor as an explicit scalar,
# so sharing this exact source avoids maintaining two arithmetic programs.
...
_NATIVE_EXTENSION_NAME = f"pycbc_mode33_native_boundary_{_NATIVE_CACHE_KEY}"
_GLOBAL_FACTOR = (
    (2.0 / 3.0) ** (-7.0 / 6.0) * 0.75 * math.sqrt(5.0 / 7.0)
)
_PN_DOMINANT_FACTOR = (2.0 / 3.0) ** (-7.0 / 6.0)
...
def native_boundary_enabled():
    """Return the strict, off-by-default native mode-33 switch."""
...
def _arguments_supported(point, like, parameters):
    """Validate the exact request-local mode-33 boundary schema."""
    ...
    if (
        ...
        or fcut <= 2.0e-9
        or point != fcut
        or pn_dominant != amp_norm * _PN_DOMINANT_FACTOR
...
def _build_extension():
    ...
    if hashlib.sha256(source).hexdigest() != _NATIVE_SOURCE_SHA256:
        raise RuntimeError("mode-33 native source digest mismatch")
...
def _after_fork_child():
    """Discard all inherited loader and qualification state in a child."""
...
def _native_values(extension, point, parameters):
    ...
    if not _plain_owned_tensor(
        output,
        dtype=torch.float64,
        shape=torch.Size((2,)),
    ):
        raise RuntimeError("mode-33 native extension returned an invalid tensor")
    if not _tensor_values_finite(output):
        raise RuntimeError("mode-33 native extension returned non-finite values")
    return output.unbind()


def _raw_equal(left, right):
    """Compare both returned scalar tensors' metadata and raw bytes."""
    ...
    try:
        return all(
            ...
            and reference.layout is candidate.layout is torch.strided
            and reference.device == candidate.device
            and reference.device.type == "cpu"
            and reference.dtype is candidate.dtype is torch.float64
...
def try_native_boundary(eager_boundary, point, like, parameters):
    """Return exact native values, the first eager canary, or ``None``.

    The first supported call in each process evaluates both paths with the
    same inputs, compares both scalar tensors' metadata and raw bytes, and
    returns the original eager objects. Any load, execution, or parity failure
    disables the helper for the remainder of that process.
    """
    ...
    with _STATE_LOCK:
        ...
        if not _QUALIFIED:
            ...
            if not _raw_equal(reference, candidate):
                _mark_failed_locked(
                    RuntimeError("mode-33 native boundary parity mismatch")
                )
                return reference
            _QUALIFIED = True
            return reference
