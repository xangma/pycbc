...
def nltides_fourier_phase_difference(
    frequencies, delta_f, f0, amplitude, n, m1, m2
):
    ...
    below *= -phi_of_f_factor * (f0 / f_ref) ** (n - 3.)
    above_frequencies = frequencies[kmin:kmax]
    above = -phi_of_f_factor * (above_frequencies / f_ref) ** (n - 3.)

    below += (
        2.0 * PI * frequencies[:kmin] * t_of_f_factor
        * (f0 / f_ref) ** (n - 4.)
    )
    above += (
        2.0 * PI * above_frequencies * t_of_f_factor
        * (above_frequencies / f_ref) ** (n - 4.)
    )
...
def nonlinear_tidal_spa(**kwds):
    ...
    phase_difference = nltides_fourier_phase_difference(
        frequencies, hp.delta_f, kwds['f0'], kwds['amplitude'], kwds['n'],
