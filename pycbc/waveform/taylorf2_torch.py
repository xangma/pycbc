...
def taylorf2_fd_batch(**params):
    """Generate a batch of TaylorF2 frequency-domain waveforms directly as 2D tensors.
    ...
    f_final : float, optional
        Upper frequency cutoff in Hertz. If not provided, computed from ISCO.
    spin1z : float or Tensor, optional
        Dimensionless aligned spin of primary. Default 0.
    spin2z : float or Tensor, optional
        Dimensionless aligned spin of secondary. Default 0.
    ...
    coa_phase : float or Tensor, optional
        Coalescence phase in radians. Default 0.0.
    ...
    device = getattr(state, "torch_device", torch.device("cpu"))
    real_dtype = (
        torch.float32 if getattr(device, "type", "cpu") == "mps" else torch.float64
    )
    complex_dtype = (
        torch.complex64 if real_dtype == torch.float32 else torch.complex128
    )
    ...
    def _to_tensor_batch(val, default_val=0.0):
        if val is None:
            val = default_val
    ...
    m1 = _to_tensor_batch(m1_in)
    m2 = _to_tensor_batch(m2_in)
    s1z = _to_tensor_batch(params.get("spin1z", 0.0))
    s2z = _to_tensor_batch(params.get("spin2z", 0.0))
    dist = _to_tensor_batch(params.get("distance", 1.0), 1.0)
    incl = _to_tensor_batch(params.get("inclination", 0.0))
    coa_phase = _to_tensor_batch(params.get("coa_phase", 0.0))
    ...
    for i in range(batch_size):
        ...
        v_coeffs.append(p.v)
        vlogv_coeffs.append(p.vlogv)
        vlogvsq_coeffs.append(p.vlogvsq)

    v_mat = torch.as_tensor(_np.array(v_coeffs), dtype=real_dtype, device=device)
    vlog_mat = torch.as_tensor(_np.array(vlogv_coeffs), dtype=real_dtype, device=device)
    vlogsq_mat = torch.as_tensor(_np.array(vlogvsq_coeffs), dtype=real_dtype, device=device)
    ...
    f_final = float(params.get("f_final", 0.0))
    if f_final == 0.0:
        f_max = float(torch.max(f_isco).item())
    else:
        f_max = f_final
    ...
    freqs = torch.arange(first_bin, length, dtype=real_dtype, device=device) * delta_f

    freqs_2d = freqs.unsqueeze(0)
    pi_mass_2d = pi_mass.unsqueeze(1)
    velocity = torch.pow(pi_mass_2d * freqs_2d, 1.0 / 3.0)
    log_v = torch.log(velocity)
    log_v_sq = log_v * log_v

    res = v_mat[:, 15:16] + vlog_mat[:, 15:16] * log_v + vlogsq_mat[:, 15:16] * log_v_sq
    for idx in range(14, -1, -1):
        term = v_mat[:, idx:idx+1] + vlog_mat[:, idx:idx+1] * log_v + vlogsq_mat[:, idx:idx+1] * log_v_sq
        res = torch.addcmul(term, res, velocity)

    v2 = velocity * velocity
    phi = res / (v2 * v2 * velocity)

    epoch = -1.0 / delta_f
    phase = (
        phi
        + (2.0 * math.pi * epoch * freqs_2d)
        - (2.0 * coa_phase.unsqueeze(1))
        - (math.pi / 4.0)
    )
    ...
    amp_factor = amp0 * torch.sqrt(5.0 / (32.0 * eta)).unsqueeze(1)
    amplitude = amp_factor * torch.pow(velocity, -3.5)

    samples = torch.polar(amplitude, -phase).to(complex_dtype)

    cos_i = torch.cos(incl).unsqueeze(1)
    plus_factor = 0.5 * (1.0 + cos_i * cos_i)
    cross_factor = torch.complex(
        torch.zeros_like(cos_i),
        -cos_i,
    )

    hp_active = samples * plus_factor
    hc_active = samples * cross_factor

    hp = torch.zeros((batch_size, length), dtype=complex_dtype, device=device)
    hc = torch.zeros((batch_size, length), dtype=complex_dtype, device=device)
    hp[:, first_bin:] = hp_active
    hc[:, first_bin:] = hc_active
    return hp, hc
