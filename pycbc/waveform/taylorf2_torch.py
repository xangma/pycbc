...
def _native_device_supported(
    params, *, sequence, minimum_phase_mf=_MPS_MIN_EQUAL_MASS_PHASE_MF
):
    ...
    minimum_phase_mf = minimum_phase_mf * (0.25 / symmetric_mass_ratio) ** (3.0 / 5.0)
...
def _evaluate_phase_polynomial(
    ...
    for index in range(_MAX_PN_ORDER - 2, -1, -1):
        if taylorf2_log_terms:
            ...
            term = (
                coeff[index] + coeff_log[index] * log_v + coeff_log_sq[index] * log_v_sq
            )
