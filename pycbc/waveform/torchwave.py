...
def _reason(bank, params, device):
    if bank.enable_torchwave is not True:
        return 'TorchWave requires explicit enable_torchwave=True'
    if importlib.util.find_spec('torchwave') is None:
        return 'TorchWave is not installed'
    if device.split(':')[0] not in ('cpu', 'cuda'):
        return 'float64 TorchWave generation requires CPU or CUDA'
...
def batch_key(bank, indices):
    ...
    return (_IDENTITY, provider_identity(), bank.enable_torchwave,
...
def generate_batch(bank, indices, device='cpu', dtype=None, delta_f=None):
    ...
    if native:
        import torchwave
        params = [requests[i][0] for i in native]
        kwargs = {
            key: torch.tensor([p[key] for p in params], dtype=torch.float64,
                              device=device)
            for key in _PHYSICAL - {'coa_phase'}
        }
        kwargs['phic'] = torch.tensor(
            [p['coa_phase'] + np.pi / 2 for p in params],
            dtype=torch.float64, device=device)
        # PyCBC/LAL's explicit f_final replaces the TaylorF2 ISCO cutoff.
        # Include floor(f_final / df); start at ceil(f_lower / df).
        freqs = torch.arange(flen, device=device, dtype=torch.float64)
        freqs *= params[0]['delta_f']
        hp, _ = torchwave.get_fd_waveform(
        ...
        output[native] = hp.to(dtype=dtype)
        for position in native:
