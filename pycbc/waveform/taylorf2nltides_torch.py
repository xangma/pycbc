...
from .taylorf2_torch import (
    ...
    _taylorf2_samples,
    _taylorf2_sequence_frequencies,
...
def _native_features_supported(params):
    """Return whether the nonlinear-tide options are implemented here."""
...
def taylorf2nltides_native_supported(params):
    """Return whether regular-grid TaylorF2NLTides is native."""

    return _native_features_supported(params) and taylorf2_native_supported(
        _base_params(params)
    )


def taylorf2nltides_sequence_native_supported(params):
    """Return whether arbitrary-frequency TaylorF2NLTides is native."""

    return _native_features_supported(params) and taylorf2_sequence_native_supported(
        _base_params(params)
    )


def _taylorf2nltides_sequence_frequencies(sample_points, inputs):
    """Validate nonlinear-tide sample points on the active Torch device."""
    import torch

    frequencies = _taylorf2_sequence_frequencies(sample_points, inputs)
    if not bool(torch.all(torch.isfinite(frequencies))):
        raise ValueError("TaylorF2NLTides sample_points must be finite")
    if bool(torch.any(frequencies <= 0.0)):
        raise ValueError("TaylorF2NLTides sample_points must be positive")
    return frequencies
...
def _body_phase(frequencies, amplitude, spectral_index, turn_on, mass_fraction):
    ...
    if spectral_index == 3.0:
        ...
        phase = coefficient * (torch.pow(frequencies, exponent) - turn_on**exponent)
...
def _nonlinear_tide_phase(frequencies, inputs, params):
    ...
    chirp_mass = (inputs.mass1 * inputs.mass2) ** 0.6 / total_mass**0.2
    coefficient = 50.0 * 2.0 ** (2.0 / 3.0) / 3072.0
    coefficient *= (1.0 / (chirp_mass * MTSUN_SI * 100.0 * math.pi)) ** (10.0 / 3.0)
...
def taylorf2nltides_fd_torch(**params):
    ...
    if not taylorf2nltides_native_supported(params):
        raise ValueError(
            "TaylorF2NLTides parameters are not supported by the native Torch path"
        )
    ...
    if delta_f <= 0.0 or f_lower <= 0.0:
        raise ValueError("TaylorF2NLTides delta_f and f_lower must be positive")

    pi_mass = math.pi * (inputs.mass1 + inputs.mass2) * MTSUN_SI
    f_max = f_final or 1.0 / (6.0**1.5 * pi_mass)
    if f_max <= f_lower:
        raise ValueError("TaylorF2NLTides ending frequency must exceed f_lower")

    length = int(f_max / delta_f + 1.0)
    first_bin = int(math.ceil(f_lower / delta_f))
    if first_bin >= length:
        raise ValueError("TaylorF2NLTides frequency range contains no sampled bins")
...
def taylorf2nltides_fd_sequence_torch(**params):
    """Evaluate ``TaylorF2NLTides`` at arbitrary frequencies with Torch.

    LAL does not expose this model through its arbitrary-frequency sequence
    API, so this is a native extension of the regular-grid implementation.
    """
    import torch
    ...
    if not taylorf2nltides_sequence_native_supported(params):
        raise ValueError(
            "TaylorF2NLTides sequence parameters are not supported by the "
            "native Torch path"
        )
    ...
    frequencies = _taylorf2nltides_sequence_frequencies(
    ...
    plus, cross = _taylorf2_polarizations(samples * correction, inputs)
    return (
