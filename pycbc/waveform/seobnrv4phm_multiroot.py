...
Some macOS GSL builds leave their CBLAS symbols for runtime lookup, so the
solver arithmetic changes according to whether SciPy or Torch was imported
first.  This module ports the small GSL control algorithm to Python and calls
the companion GSL CBLAS library explicitly, making that arithmetic stable.
...
def _load_cblas(candidates: Iterable[str]) -> _GslCblas:
    global _CBLAS
    ...
    for path in candidates:
        try:
            ...
            continue
        return _CBLAS
    raise GslCblasUnavailable("no GSL CBLAS library found")
