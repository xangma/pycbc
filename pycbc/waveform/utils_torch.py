...
def fused_detector_strain_fd_torch(
    ...
    hp_r = hp_tensor.real if hp_tensor.is_complex() else hp_tensor
    hp_i = hp_tensor.imag if hp_tensor.is_complex() else torch.zeros_like(hp_r)
    hc_r = hc_tensor.real if hc_tensor.is_complex() else hc_tensor
    hc_i = hc_tensor.imag if hc_tensor.is_complex() else torch.zeros_like(hc_r)
    ...
    shift_r = torch.cos(theta)
    shift_i = torch.sin(theta)
    ...
    return torch.complex(
        torch.cat((head_r, tail_r), dim=-1),
        torch.cat((head_i, tail_i), dim=-1),
    )
