...
NRTIDAL_V3_APPROXIMANTS = frozenset(
    {
...
def nrtidal_quadrupole_from_lambda(lambda_value: float) -> float:
    ...
    if isinstance(lambda_value, torch.Tensor):
        if not bool(torch.all(torch.isfinite(lambda_value) & (lambda_value >= 0.0)).item()):
            raise ValueError("tidal deformability must be finite and non-negative")
        clamped_lambda = torch.clamp(lambda_value, min=0.5)
        log_lambda = torch.log(clamped_lambda)
        quad = torch.exp(
            0.1940
            + 0.0936 * log_lambda
            + 0.0474 * log_lambda**2
        ...
        return torch.where(lambda_value < 0.5, torch.ones_like(quad), quad)
...
def nrtidal_octupole_from_quadrupole(quadrupole: float) -> float:
    ...
    if isinstance(quadrupole, torch.Tensor):
        if not bool(torch.all(torch.isfinite(quadrupole) & (quadrupole > 0.0)).item()):
            raise ValueError("spin-induced quadrupole must be finite and positive")
        log_quadrupole = torch.log(quadrupole)
        return torch.exp(
            0.003131
