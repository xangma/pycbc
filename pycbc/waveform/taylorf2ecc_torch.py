...
def taylorf2ecc_native_supported(params):
    """Return whether ``params`` are covered by native TaylorF2Ecc."""
    try:
    ...
    # Reuse the TaylorF2 capability checks after removing the two options
    # that distinguish TaylorF2Ecc from the circular model.
    circular_params = dict(params)
    circular_params["eccentricity"] = 0.0
    circular_params["eccentricity_order"] = -1
    return taylorf2_native_supported(circular_params)
...
def _eccentric_phase_polynomial(velocity, velocity0, eccentricity, eta, order):
    ...
    if order >= 6:
        ...
        phase = phase + (
            (
                coeff_v6
                + 53.6803271 / 3.9564000 * torch.log(16.0 * v**2)
            )
            * v**6
            ...
            + (
                coeff_v06
                - 33.17 / 2.52 * torch.log(16.0 * v0**2)
            )
            * v0**6
        )
