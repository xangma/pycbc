...
def _native_features_supported(params):
    ...
    if not math.isfinite(eccentricity) or not 0.0 <= eccentricity < 1.0:
        return False
    if _as_order(params.get("eccentricity_order", -1)) not in (_ECCENTRICITY_ORDERS):
        return False
    return True
...
def _taylorf2ecc_sequence_frequencies(sample_points, inputs):
    ...
    if frequencies.ndim != 1 or frequencies.numel() == 0:
        raise ValueError("TaylorF2Ecc sample_points must be a non-empty vector")
...
def _eccentric_phase_bracket(
    ...
    *,
    ...
    if order >= 3:
        phase = phase + (
            (-28.19123 / 2.82600 * math.pi) * v**3 + (37.7 / 7.2 * math.pi) * v0**3
        )
    if order >= 4:
        phase = phase + (
            (
    ...
    if order >= 5:
        phase = phase + (
            ...
            * v**2
            * v0**3
            + (76.4881 / 9.0720 * math.pi - 94.9457 / 2.2680 * math.pi * eta) * v0**5
        )
    if order >= 6:
        coeff_v6 = (
            ...
            + (2991.72861614477 / 6.89135247360 - 15.075413 / 1.446912 * math.pi**2)
            * eta
        ...
        phase = phase + (
            (coeff_v6 + 53.6803271 / 3.9564000 * log(16.0 * v**2)) * v**6
            + coeff_v4_v02 * v**4 * v0**2
            + coeff_v3_v03 * v**3 * v0**3
            + coeff_v2_v04 * v**2 * v0**4
            + (coeff_v06 - 33.17 / 2.52 * log(16.0 * v0**2)) * v0**6
        )
...
def _native_device_supported(params, *, sequence):
    ...
    try:
        ...
        return False
    return math.isfinite(phase_scale) and phase_scale <= _MPS_MAX_ECCENTRIC_PHASE
...
def _taylorf2ecc_samples(
    ...
    *,
    ...
    if inputs.f_ref == 0.0:
        reference_phase = torch.zeros((), dtype=inputs.real_dtype, device=inputs.device)
    else:
...
def taylorf2ecc_fd_sequence_torch(**params):
    ...
    if not taylorf2ecc_sequence_native_supported(params):
        raise ValueError(
            "TaylorF2Ecc sequence parameters are not supported by the native Torch path"
        )
