...
def spin_weighted_spherical_harmonic(
    ...
    *,
    ...
    for index in range(2 * ell + 1):
        ...
        if min(denominator_indices) < 0:
            continue
        denominator = math.prod(math.factorial(value) for value in denominator_indices)
        coefficient = (-1) ** (emm - wigner_m + index) * prefactor / denominator
        cos_power = 2 * ell + wigner_m - emm - 2 * index
        sin_power = emm - wigner_m + 2 * index
        amplitude = (
            amplitude + coefficient * cos_powers[cos_power] * sin_powers[sin_power]
        )
...
def _selected_spin_minus_two_values(theta, phi, modes):
    ...
    for ell, emm in modes:
        amplitude = zero
        for coefficient, cos_power, sin_power in _SPIN_MINUS_TWO_TERMS[ell, emm]:
            amplitude = (
                amplitude + coefficient * cos_powers[cos_power] * sin_powers[sin_power]
            )
        phase = emm * phi
        harmonics.append(
            torch.complex(amplitude * torch.cos(phase), amplitude * torch.sin(phase))
        )
...
def selected_spin_minus_two_spherical_harmonics(
    ...
    *,
    ...
    if len(normalized_modes) != mode_count or any(
        ...
        raise ValueError("selected harmonics require valid ell 2 through 4 modes")
