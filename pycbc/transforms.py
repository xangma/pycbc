...
def _torch_expression_context(maps, input_names):
    ...
    def tensor_function(torch_name):
        function = getattr(torch, torch_name)

        def evaluate(*args):
            operand = next(
                (arg for arg in args if isinstance(arg, torch.Tensor)), reference
            )
            # Scalar-only subexpressions use the input precision and device.
            # Leave tensor operands intact so their graphs and promotion
            # rules survive; do not truncate floats against integer inputs.
            dtype = (
                operand.dtype
                if operand.is_floating_point() or operand.is_complex()
                else None
            )
            return function(
                *(
                    ...
                    else torch.as_tensor(arg, device=operand.device, dtype=dtype)
                    for arg in args
                )
            )
    ...
    context.update(
        {
            name: tensor_function(torch_name)
            for name, torch_name in _TORCH_EXPRESSION_FUNCTIONS.items()
            if hasattr(torch, torch_name)
        }
    )
    context.update(
        {name: getattr(conversions, name) for name in _TORCH_EXPRESSION_CONVERSIONS}
    )
    context.update(
        {name: getattr(cosmology, name) for name in _TORCH_EXPRESSION_COSMOLOGY}
    )
...
class LambdaFromTOVFile(BaseTransform):
    ...
    def _lambda_from_tov_torch(self, m_src):
        ...
        result = torch.where(values > mass_data[-1], torch.zeros_like(result), result)
        return result.reshape(shape)
