...
class LiveSingle(object):
    ...
    def check(self, trigs, data_reader):
        ...
        cutall_trigs = {k: trigsc[k][sngl_idx] for k in trigs}
        ...
        ifar = self.calculate_ifar(rank[i], trigsc["template_duration"][i])
        if ifar is None:
            return None

        # fill in a new candidate event
        candidate = {
            f"foreground/{self.ifo}/{k}": cut_trigs[k][sngl_idx][i] for k in trigs
        }
