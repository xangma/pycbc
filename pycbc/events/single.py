...
class LiveSingle(object):
    def __init__(
        ...
        **kwargs,
        ...
        self.stat_calculator = stat_class(
            sngl_ranking, stat_files, ifos=[ifo], **stat_extras
        )
    ...
    def verify_args(args, parser, ifos):
        ...
        if any(sngl_opts) and not all(sngl_opts):
            parser.error(
                f"Single detector trigger options ({sngl_opts_str}) "
                "must either all be given or none."
            )

        if args.enable_single_detector_upload and not args.enable_gracedb_upload:
            parser.error(
                "--enable-single-detector-upload requires "
                "--enable-gracedb-upload to be set."
            )

        sngl_optional_opts = [
            args.single_fixed_ifar,
            args.single_fit_file,
            args.single_maximum_ifar,
        ]
        sngl_optional_opts_str = (
            "--single-fixed-ifar, --single-fit-file,--single-maximum-ifar"
        )
        if any(sngl_optional_opts) and not all(sngl_opts):
            parser.error(
                "Optional singles options "
        ...
        for ifo in ifos:
            ...
            if not args.sngl_ifar_est_dist[ifo] == "fixed":
                if not args.single_fit_file:
                    ...
                    parser.error(
                        f"Single detector trigger fits file must be "
                ...
                if ifo in args.single_fixed_ifar:
                    parser.error(
                        f"Value {args.single_fixed_ifar[ifo]} given "
    ...
    def from_cli(cls, args, ifo):
        ...
        return cls(
            ifo,
            ranking_threshold=args.single_ranking_threshold[ifo],
    ...
    def check(self, trigs, data_reader):
        ...
        if tensors is not None:
            ...
            valid_idx = (
                (trigs["template_duration"] > self.thresholds["duration"])
                & (trig_chisq < self.thresholds["reduced_chisq"])
                & (trig_snr > self.thresholds["ranking"])
            )
        ...
        ifar = self.calculate_ifar(
            selected_rank, _scalar_at(cutall_trigs["template_duration"], i)
        )
        if ifar is None:
            return None
        ...
        candidate = {
            f"foreground/{self.ifo}/{k}": _scalar_at(cutall_trigs[k], i) for k in trigs
        }
    ...
    def calculate_ifar(self, sngl_ranking, duration):
        ...
        rate_louder = (
            rate * fits.cum_fit("exponential", [sngl_ranking], coeff, thresh)[0]
        )

        # apply a trials factor of the number of duration bins
        rate_louder *= len(rates)

        return min(conv.sec_to_year(1.0 / rate_louder), self.maximum_ifar)
    ...
    def start_refresh_thread(self):
        ...
        thread = threading.Thread(
            target=self.refresh_statistic, daemon=True, name="Stat refresh " + self.ifo
        )
    ...
    def refresh_statistic(self):
        ...
        while True:
            ...
            if since_stat_refresh > self.statistic_refresh_rate:
                self.time_stat_refreshed = time.time()
                logger.info("Checking %s statistic for updated files", self.ifo)
                with self.stat_calculator_lock:
