...
if _TRITON_AVAILABLE:
    ...
    def _triton_symm_mask_tensor_kernel(
        ...
        left_val = tl.load(max_vals_ptr + left_offsets, mask=left_mask, other=-1e30)
        left_ok = tl.where(offsets > 0, curr > left_val, True)

        # Right neighbor
        right_offsets = offsets + 1
        right_mask = mask & (offsets < nb - 1)
        right_val = tl.load(max_vals_ptr + right_offsets, mask=right_mask, other=-1e30)
        right_ok = tl.where(offsets < nb - 1, curr >= right_val, True)

        # First element special: curr[0] > curr[1] when nb > 1
        first_special = tl.where((offsets == 0) & (nb > 1), curr > right_val, True)
...
_l2_size = (
    opt.get_l2_cache_size()
    if hasattr(opt, "get_l2_cache_size")
    else getattr(opt, "LEVEL2_CACHE_SIZE", None)
)
...
_TRUSTED_NATIVE_RESULT_ENV = "PYCBC_TORCH_CPU_THRESHOLD_TRUSTED_ARRAYS"
...
def _threshold_compile_requested():
    ...
    if not _environment_flag("PYCBC_TORCH_COMPILE", default=False):
        return False
    return _environment_flag("PYCBC_TORCH_COMPILE_THRESHOLD", default=True)


def _threshold_compile_configuration():
    """Return the explicitly attributable compiler configuration."""
    backend = os.environ.get("PYCBC_TORCH_COMPILE_BACKEND", "inductor").strip()
...
def _findchirp_cluster_indices(times, values, window_length):
    ...
    if span <= max(2 * count, 10_000_000):
        ...
        pooled = F.max_pool1d(padded, kernel_size=window_length + 1, stride=1).view(-1)
        ...
        for start in range(0, count, chunk_size):
            ...
            fwd_max[start:end] = (
                torch.where(valid, clean_mags[c_indices], float("-inf"))
                .max(dim=1)
                .values
            )
...
def _findchirp_cluster_positions(times, values, window_length):
    ...
    if device.type == "cpu":
        ...
        k = findchirp_cluster_over_window_cython(
            times_np, mags_np, int(window_length), indices_np, count
        )
        return torch.from_numpy(indices_np[: k + 1].astype(np.int64))

    return _findchirp_cluster_indices(time_tensor, value_tensor, window_length)
...
def _cluster_candidates(mag_sq, window):
    ...
    if num_blocks == 0:
        return (
            torch.empty(0, device=mag_sq.device, dtype=mag_sq.dtype),
    ...
    if pad:
        pad_val = torch.full(
            (pad,), float("-inf"), device=mag_sq.device, dtype=mag_sq.dtype
        )
    ...
    if mag_sq.device.type != "cpu" or is_compiling():
    ...
    block_idx.add_(torch.arange(num_blocks, device=mag_sq.device), alpha=window)
    return block_max, block_idx
...
def _fixed_shape_threshold_core(tensor, threshold_sq, window):
    ...
    if tensor.is_complex():
        ...
        mag_sq = tensor.square().sum(dim=-1)
    else:
        mag_sq = tensor.square()
...
def _compiled_threshold_core(
    ...
    if compiler is None:
        raise RuntimeError("PYCBC_TORCH_COMPILE_THRESHOLD requires torch.compile")
    ...
    def fixed_shape_core(tensor_real, threshold_sq):
        return _fixed_shape_threshold_core(tensor_real, threshold_sq, window)
...
def _run_eligible_compiled_threshold_core(tensor, threshold_sq, window):
    ...
    verify = _environment_flag("PYCBC_TORCH_COMPILE_VERIFY", default=False)
    tensor_real = torch.view_as_real(tensor) if tensor.is_complex() else tensor
    ...
    compiled_max, compiled_idx, compiled_keep = compiled(tensor_real, threshold_sq)
...
def _run_threshold_core(tensor, threshold_sq, window, *, single_series):
    ...
    return _run_eligible_compiled_threshold_core(tensor, threshold_sq, window)
...
def _symmetric_cluster(max_vals, max_idx, threshold_sq):
    ...
    if nb == 0:
        return (
            torch.empty(0, device=max_vals.device, dtype=max_vals.dtype),
...
class TorchThresholdCluster(_BaseThresholdCluster):
    ...
    def __init__(self, series):
        ...
        self.series = (
            self._source if self._source.ndim == 1 else self._source.reshape(-1)
        )
        ...
        native_cpu_candidate = (
            ...
            and (_TORCH_IS_INFERENCE is None or not _TORCH_IS_INFERENCE(self.series))
        ...
        if (
            ...
            self._magnitude_component = (
                torch.empty_like(self._magnitude) if self.series.is_complex() else None
            )
        else:
    ...
    def _native_cpu_threshold_and_cluster(self, threshold, window):
        ...
        if self._native_values is None or self._native_values.size < candidate_count:
    ...
    def threshold_and_cluster(self, threshold, window):
        ...
        if (
            ...
            return self._native_cpu_threshold_and_cluster(native_threshold, window)
        ...
        use_scratch = (
            ...
            and (
                (
                    ...
                    and self._magnitude_component.dtype == self.series.real.dtype
                )
                or (not self.series.is_complex() and self._magnitude_component is None)
            )
            and (_TORCH_IS_INFERENCE is None or not _TORCH_IS_INFERENCE(self.series))
            and (
                _TORCH_IS_INFERENCE is None or not _TORCH_IS_INFERENCE(self._magnitude)
            )
            and (
