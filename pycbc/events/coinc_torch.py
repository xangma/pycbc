...
def _as_torch_tensor(value):
    ...
    return backend_array(value, "torch") if is_backend(value, "torch") else None
...
def _wrap_coincidence_result(t1, t2, *values):
    ...
    if not isinstance(t1, Array) and not isinstance(t2, Array):
        return values
    return tuple(Array(TorchArrayData(value), copy=False) for value in values)
...
def time_coincidence(t1, t2, window, slide_step=0):
    ...
    def _as_time_tensor(value, tensor):
        ...
        if tensor.ndim != 1 or not tensor.is_floating_point():
            raise TypeError(
                "coincidence time arrays must be one-dimensional floating point values"
            )
        return tensor
...
def cluster_coincs(stat, time1, time2, timeslide_id, slide, window, **kwargs):
    ...
    if not time1_tensor.is_floating_point() or not time2_tensor.is_floating_point():
...
def cluster_coincs_multiifo(stat, time_coincs, timeslide_id, slide, window, **kwargs):
    ...
    time_avg = event_anchor - event_anchor[:1] + relative_sum / num_ifos.to(times.dtype)
...
def _cluster_window_maxima(stat, left, right, method):
    ...
    tree = torch.full((2 * tree_size,), sentinel, dtype=torch.int64, device=stat.device)
    ...
    best = torch.full((length,), sentinel, dtype=torch.int64, device=stat.device)
...
def cluster_over_time(stat, time, window, method="python", argmax=numpy.argmax):
    ...
    positions = torch.arange(length, dtype=torch.int64, device=reference.device)
    ...
    visited = torch.zeros(length + 1, dtype=torch.bool, device=reference.device)
