...
def _torch_veto_vectors(times, start, end):
    ...
    for value, tensor in zip(values, tensors):
        ...
        if (
            ...
            raise TypeError(
                "veto time arrays must be one-dimensional numeric values"
            )
...
def _torch_indices_within_times(times, start, end):
    ...
    within = (segment_id >= 0) & (
        sorted_times < end_t[safe_segment_id]
    )
    return order[within]
...
def _torch_indices_outside_times(times, start, end):
    ...
    keep[exclude] = False
    return torch.arange(
        len(times), dtype=torch.int64, device=exclude.device
    )[keep]
...
def _torch_complement_indices(times, exclude):
    ...
    if exclude_t is None:
        exclude_t = torch.as_tensor(
            exclude, dtype=torch.int64, device=reference.device
        )
    ...
    return torch.arange(
        len(times), dtype=torch.int64, device=reference.device
    )[keep]
...
def start_end_from_segments(segment_file):
    ...
    """
    _, lsctables, ligolw_utils = _ligolw_modules(
        "reading LIGO-LW segment files"
    )
...
def indices_within_times(times, start, end):
    ...
    if torch_indices is not None:
        return _wrap_torch_veto_result(
            (times, start, end), torch_indices
        )
...
def select_segments_by_definer(segment_file, segment_name=None, ifo=None):
    ...
    """
    ligolw, _, ligolw_utils = _ligolw_modules(
        "selecting LIGO-LW segment definitions"
    )
    from pycbc.io.ligolw import LIGOLWContentHandler as h
...
def indices_within_segments(times, segment_files, ifo=None, segment_name=None):
    ...
    if tensor is not None:
        indices = _wrap_torch_veto_result(
            (times, idx), tensor.sort().values
        )
    else:
...
def get_segment_definer_comments(xml_file, include_version=True):
    ...
    _, lsctables, ligolw_utils = _ligolw_modules(
        "reading LIGO-LW segment definitions"
    )
    from pycbc.io.ligolw import LIGOLWContentHandler as h
