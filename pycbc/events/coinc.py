...
def _torch_time_tensor(value):
    """Return a Torch tensor without importing Torch for NumPy callers."""
...
def _wrap_torch_coincidence_result(t1, t2, *values):
...
def _wrap_torch_cluster_result(inputs, value):
...
def _torch_cluster_vectors(values):
    """Move mixed clustering vectors beside the first Torch input."""
    tensors = [_torch_time_tensor(value) for value in values]
...
def _torch_time_coincidence(t1, t2, window, slide_step):
    """Device implementation of :func:`time_coincidence`."""
    tensor1 = _torch_time_tensor(t1)
    tensor2 = _torch_time_tensor(t2)
    ...
    if slide_step:
        fold2 = torch.cat(
            (fold2 - slide_step, fold2, fold2 + slide_step)
        )
    ...
    if idx1.numel() and sort2.numel():
        ...
        idx2 = torch.empty(
            0, dtype=torch.int64, device=reference.device
        )
    ...
    return _wrap_torch_coincidence_result(t1, t2, idx1, idx2, slide)
...
def time_coincidence(t1, t2, window, slide_step=0):
    ...
    """
    torch_result = _torch_time_coincidence(t1, t2, window, slide_step)
...
def _torch_cluster_coincs(
        stat, time1, time2, timeslide_id, slide, window, kwargs):
    """Device implementation of :func:`cluster_coincs`."""
    inputs = (stat, time1, time2, timeslide_id)
    tensors = _torch_cluster_vectors(inputs)
    if tensors is None:
        return None
    ...
    if time1_tensor.numel() == 0 or time2_tensor.numel() == 0:
        empty = torch.empty(
            0, dtype=torch.int64, device=stat_tensor.device
        )
        return _wrap_torch_cluster_result(inputs, empty)
    ...
    return _wrap_torch_cluster_result(inputs, result)
...
def cluster_coincs(stat, time1, time2, timeslide_id, slide, window, **kwargs):
    ...
    """
    torch_result = _torch_cluster_coincs(
        stat, time1, time2, timeslide_id, slide, window, kwargs
    )
    if torch_result is not None:
        return torch_result
...
def _torch_cluster_coincs_multiifo(
        stat, time_coincs, timeslide_id, slide, window, kwargs):
    """Device implementation of :func:`cluster_coincs_multiifo`."""
...
def cluster_coincs_multiifo(stat, time_coincs, timeslide_id, slide, window,
    ...
    cindex: numpy.ndarray or Torch-backed array
        ...
        inputs produce indices on the same device.
    """
    torch_result = _torch_cluster_coincs_multiifo(
        stat, time_coincs, timeslide_id, slide, window, kwargs
    )
    if torch_result is not None:
        return torch_result
...
def _torch_cluster_better(first, second, stat, sentinel, nan_high=True):
...
def _torch_cluster_window_maxima(stat, left, right, method):
    ...
    width = tree_size
    while width > 1:
        tree[width // 2:width] = _torch_cluster_better(
...
def _torch_cluster_over_time(stat, time, window, method, argmax):
    """Device implementation of :func:`cluster_over_time`."""
    stat_tensor = _torch_time_tensor(stat)
    time_tensor = _torch_time_tensor(time)
    if stat_tensor is None and time_tensor is None:
        return None
    ...
    if length == 0:
        ...
        return _wrap_torch_coincidence_result(stat, time, empty)[0]
    ...
    maxima = _torch_cluster_window_maxima(
        sorted_stat, left, right, method
    )
    ...
    return _wrap_torch_coincidence_result(stat, time, result)[0]
...
def cluster_over_time(stat, time, window, method='python',
    ...
    torch_result = _torch_cluster_over_time(
        stat, time, window, method, argmax
    )
    if torch_result is not None:
        return torch_result
...
class LiveCoincTimeslideBackgroundEstimator(object):
    ...
    def __init__(self, num_templates, analysis_block, background_statistic,
        ...
        self.dets = {
            ifo: _detector(ifo, "live coincidence detector timing")
            for ifo in ifos
        }
