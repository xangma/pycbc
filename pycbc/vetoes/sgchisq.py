...
def _cached_gpu_sg_tile(
    ...
    tile_slice = tile_tensor[kmin:kmax].to(device=device, dtype=complex_dtype)
...
def _torch_sgchisq_values(
    ...
    length = len(snrv)
    output = torch.ones(length, device=stilde_tensor.device, dtype=torch.float32)
    ...
    snr_values = _as_torch_tensor(snrv, stilde_tensor, dtype=complex_dtype)
    bchisq_values = _as_torch_tensor(bchisq, stilde_tensor, dtype=real_dtype)
    dof_values = _as_torch_tensor(bchisq_dof, stilde_tensor, dtype=real_dtype)
    index_values = _as_torch_tensor(indices, stilde_tensor, dtype=torch.long)
