...
def _chisq_zeros(corr, length):
    ...
    return _torch_array(
        torch.zeros(length, device=corr_tensor.device, dtype=torch.float32)
    )
...
def _copy_chisq_masked(output, mask, values):
    ...
    values_tensor = _as_torch_tensor(values, output_tensor, dtype=output_tensor.dtype)
...
def power_chisq_bins_from_sigmasq_series(sigmasq_series, num_bins, kmin, kmax):
    ...
    if tensor is not None:
        ...
        # Match NumPy's integer-arange promotion, so a rounded threshold
        # cannot move an edge across a flat single-precision power tail.
        edge_dtype = torch.float32 if tensor.device.type == "mps" else torch.float64
        sigmasq = tensor[kmax - 1].to(dtype=edge_dtype)
        edge_vec = (
            torch.arange(num_bins, dtype=edge_dtype, device=tensor.device)
            * sigmasq
            / num_bins
        )
        bins = torch.searchsorted(tensor[kmin:kmax], edge_vec, right=True) + kmin
...
def power_chisq_bins(
    htilde, num_bins, psd, low_frequency_cutoff=None, high_frequency_cutoff=None
):
    """Returns bins of equal power for use with the chisq functions
...
def power_chisq_from_precomputed(
    corr, snr, snr_norm, bins, indices=None, return_bins=False
):
...
def fastest_power_chisq_at_points(corr, snr, snrv, snr_norm, bins, indices):
    ...
    if isinstance(pycbc.scheme.mgr.state, pycbc.scheme.CPUScheme):
        ...
        return power_chisq_at_points_from_precomputed(
            corr, snrv, snr_norm, bins, indices
        )
    else:
...
def power_chisq(
    ...
    total_snr, corr, tnorm = matched_filter_core(
        htilde, stilde, psd, low_frequency_cutoff, high_frequency_cutoff, corr_out=corra
    )

    return power_chisq_from_precomputed(
        corr, total_snr, tnorm, bins, return_bins=return_bins
    )
...
class SingleDetPowerChisq(object):
    ...
    def cached_chisq_bins(self, template, psd):
        ...
        if (
            key not in template._bin_cache
            or id(template.params) not in psd._chisq_cached_key
        ):
        ...
        if self.do:
            ...
            if self.snr_threshold:
                if corr_tensor is not None:
                    ...
                    above = torch.abs(snrv_tensor * snr_norm) > self.snr_threshold
            ...
            if num_above > 0:
                ...
                dof = (len(bins) - 1) * 2 - 2
                _chisq = power_chisq_at_points_from_precomputed(
                    corr, above_snrv, snr_norm, bins, above_indices
                )
            ...
            return chisq_out, _chisq_dof_array(chisq_out, dof, len(indices))
        else:
            return None, None
...
class SingleDetSkyMaxPowerChisq(SingleDetPowerChisq):
    ...
    def values(
        ...
        """
        if _torch_tensor(corr_plus) is not None:
            return self._values_torch(
                corr_plus,
                corr_cross,
                snrv,
                psd,
        ...
        if self.do:
            ...
            if num_above > 0:
                chisq = []
                curr_tmplt_mult_fac = 0.0
                curr_corr_mult_fac = 0.0
                if self.template_mem is None or (
                    not len(self.template_mem) == len(template_plus)
                ):
                    self.template_mem = zeros(
                        len(template_plus), dtype=complex_same_precision_as(corr_plus)
                    )
                if self.corr_mem is None or (not len(self.corr_mem) == len(corr_plus)):
                    self.corr_mem = zeros(
                        len(corr_plus), dtype=complex_same_precision_as(corr_plus)
                    )
    ...
    def _values_torch(
        ...
        if self.do:
            ...
            if num_above > 0:
                ...
                try:
                    for lidx in range(num_above):
                        if torch_backed:
                            above_local_indices = above_indices[lidx : lidx + 1]
                            ...
                            above_local_indices = numpy.array([above_indices[lidx]])
                            above_local_snr = numpy.array([above_snrv[lidx]])
                            local_u_val = above_u_vals[lidx]
                            template = template_cross.multiply_and_add(
                                template_plus,
            ...
            return rchisq, _chisq_dof_array(rchisq, dof, len(indices))
        else:
            return None, None
