...
if _HAS_TRITON:
    ...
    def _triton_pointwise_chisq_bin_kernel(
        ...
        for offset in range(0, num_k, BLOCK_SIZE):
            ...
            c_re = tl.load(corr_real_ptr + k_indices * stride_corr, mask=mask, other=0.0)
            c_im = tl.load(corr_imag_ptr + k_indices * stride_corr, mask=mask, other=0.0)
...
def _point_tensor(corr, points):
    ...
    if isinstance(point_data, np.ndarray) and point_data.dtype.kind == "u":
        point_data = point_data.astype(np.int64)
    return torch.as_tensor(point_data, device=device, dtype=point_dtype)
...
def _cuda_exact_point_chisq(cuda_mod, corr, points, bin_edges, snr, snr_norm):
    ...
    snr_sq = torch.view_as_real(snr_dev).square().sum(dim=-1)
...
def _accelerator_batched_bin_sums(corr, pts, bins):
    ...
    if _HAS_TRITON and corr.device.type == "cuda" and corr.dtype == torch.complex64:
        ...
        corr_flat = torch.view_as_real(corr.contiguous()).flatten()
...
def power_chisq_at_points_from_precomputed(corr, snr, snr_norm, bins, indices):
    ...
    if snr_t.is_complex():
        snr_term = torch.view_as_real(snr_t).square().sum(dim=-1)
    else:
        snr_term = snr_t.square()
