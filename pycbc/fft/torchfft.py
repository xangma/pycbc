...
_MKL_DIRECT_IFFT_SIZES = frozenset({32768, 65536, 131072, 262144, 524288, 1048576})
...
def _create_mkl_cpu_ifft_plan(size, source, target, nthreads=None):
    ...
    try:
        ...
        if nthreads is None:
            nthreads = torch.get_num_threads()
        return _MKLCPUDirectIFFTPlan(mkl, size, source, target, nthreads)
    except (
        ImportError,
        OSError,
        AttributeError,
