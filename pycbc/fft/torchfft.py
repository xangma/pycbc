...
def _is_retained_measured_ifft(size, forward, nthreads, measure_level):
...
class _MKLCPUDirectIFFTPlan:
    """Own a pointer-bound MKL IFFT, optionally with a float64 workspace."""

    def __init__(self, mkl, size, source, target, nthreads=None, *, promote=False):
        if nthreads is None:
        ...
        create = mkl.mkl_descriptor["double" if promote else "single"]
        create.argtypes = [
        ...
        try:
            status = create(ctypes.byref(descriptor), mkl.DFTI_COMPLEX, size)
            ...
            set_value.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_int]
    ...
    def can_execute(self, source, target):
        ...
        return (
            ...
            and (_TORCH_IS_INFERENCE is None or not _TORCH_IS_INFERENCE(source))
            and (_TORCH_IS_INFERENCE is None or not _TORCH_IS_INFERENCE(target))
    ...
    def execute(self, source, target):
        ...
        if self._inplace:
            status = self._execute(self._descriptor, self._native_source.data_ptr())
        else:
        ...
        if self._promote:
            target.copy_(self._native_target)
...
class _FFTWCPUWorkPlan:
    ...
    def __init__(self, fftw, size, forward, measure_level):
        ...
        with _FFTW_PLANNING_LOCK:
            ...
            direction = fftw.FFTW_FORWARD if forward else fftw.FFTW_BACKWARD
            flags = fftw.get_flag(
                measure_level,
    ...
    def execute(self, source, target):
        ...
        self._execute(self._plan, self._workspace.ptr, self._workspace.ptr)
...
class _FFTWCPUDirectPlan:
    ...
    def __init__(
        ...
        if measure_level != 0 and not _is_retained_measured_ifft(
            ...
            raise ValueError("direct FFTW planning mode is not precision-qualified")
        ...
        if self._use_retained_workspace:
            ...
            self._scratch_output_tensor = torch.from_numpy(self._scratch_output)
        ...
        if aligned:
            ...
            if alignment_of(self._source_ptr) != alignment_of(
                scratch_input_ptr
            ) or alignment_of(self._target_ptr) != alignment_of(scratch_output_ptr):
                raise ValueError("Torch and legacy FFTW alignment classes differ")
        direction = fftw.FFTW_FORWARD if forward else fftw.FFTW_BACKWARD
    ...
    def execute(self, source, target):
        if getattr(self, "_use_retained_workspace", False):
            ...
            return
        self._execute(self._plan, source.data_ptr(), target.data_ptr())
...
def _create_mkl_cpu_ifft_plan(size, source, target, nthreads=None, *, promote=False):
    ...
    try:
        ...
        return _MKLCPUDirectIFFTPlan(
            mkl, size, source, target, nthreads, promote=promote
        )
    except (
        ImportError,
        OSError,
...
def _setup_fftw_cpu_batch_plan(fftobj, forward):
    ...
    if _can_use_fftw_cpu_batch(fftobj):
        fftobj._fftw_batch_plan = _create_fftw_cpu_batch_plan(fftobj, forward)
...
def _setup_fftw_cpu_plan(fftobj, forward):
    ...
    if _can_use_fftw_cpu(fftobj) and fftobj.size not in _TORCH_FFT_PREFERRED_CPU_SIZES:
        fin = fftobj.invec._data.tensor
        fout = fftobj.outvec._data.tensor
        direct_sizes = _FFTW_DIRECT_SIZES if forward else _FFTW_DIRECT_IFFT_SIZES
        direct = (
            _FFTW_DIRECT_PLATFORM_SUPPORTED
...
def _shares_storage(invec, outvec):
    ...
    return _tensors_overlap(invec._data.tensor, outvec._data.tensor)
...
class _TorchPromotedBatchPlan:
    ...
    def __init__(self, source, target):
        ...
        workspace_device = (
            torch.device("cpu") if self._device.type == "mps" else self._device
        )
        element_budget = _PROMOTED_BATCH_MAX_ELEMENTS[self._device.type]
        promoted_rows_enabled = self._device.type == "cuda" and _environment_flag(
            _CUDA_PROMOTED_ROWS_GATE, default=False
        )
        if promoted_rows_enabled:
            element_budget = _CUDA_PROMOTED_BATCH_MAX_ELEMENTS
        row_elements = max(source.shape[-1], target.shape[-1])
        self.rows = min(source.shape[0], max(1, element_budget // row_elements))
    ...
    def execute(self, source, target, transform, size, inverse):
        ...
        if _tensors_overlap(source, target) and source.data_ptr() != target.data_ptr():
...
def _execute_promoted_torch_batch_plan(fftobj, source, target, transform, inverse):
...
def _execute_batched_fft(fftobj):
    ...
    if _shares_storage(fftobj.invec, fftobj.outvec):
        fout.copy_(transform(transform_input, n=fftobj.size, dim=-1))
    else:
        transform(transform_input, n=fftobj.size, dim=-1, out=fout)
...
class IFFT(_BaseIFFT):
    ...
    def __init__(self, invec, outvec, nbatch=1, size=None):
        ...
        if self._fftw_batch_plan is None and not _can_use_direct_batch_ifft(self):
            _setup_promoted_torch_batch_plan(self)
        else:
            self._promoted_batch_plan = None
