...
def detect_loud_glitches(
    ...
    # estimate the PSD
    psd = pycbc.psd.welch(
        strain[corrupt_length : (len(strain) - corrupt_length)],
    ...
    if high_freq_cutoff:
        ...
        norm = strain.sample_rate / 2.0 - low_freq_cutoff
...
def from_cli(opt, dyn_range_fac=1, precision="single", inj_filter_rejector=None):
    ...
    if opt.frame_cache or opt.frame_files or opt.frame_type or opt.hdf_store:
        ...
        if opt.fake_strain_from_file:
            ...
            strain_psd = pycbc.psd.from_txt(
                opt.fake_strain_from_file, plen, pdf, fake_flow, is_asd_file=True
            )
        elif opt.fake_strain != "zeroNoise":
            logger.info("Making PSD for strain")
            strain_psd = pycbc.psd.from_string(
                opt.fake_strain, plen, pdf, fake_flow, **fake_extra_args
            )

        if opt.fake_strain == "zeroNoise":
            logger.info("Making zero-noise time series")
            strain = TimeSeries(
                pycbc.types.zeros(duration * fake_rate),
    ...
    if opt.injection_frame_files or opt.injection_frame_type:
        if opt.injection_frame_files and opt.injection_frame_type:
            err_msg = (
                "You cannot supply both injection-frame-files and injection-frame-type"
            )
            raise ValueError(err_msg)
    ...
    if not opt.channel_name and (opt.injection_file or opt.sgburst_injection_file):
        raise ValueError(
            "Please provide channel names with the format "
    ...
    if injector is not None:
        ...
        injections = injector.apply(
            strain,
            opt.channel_name.split(":")[0],
...
def from_cli_single_ifo(opt, ifo, inj_filter_rejector=None, **kwargs):
    ...
    return from_cli(single_det_opt, inj_filter_rejector=inj_filter_rejector, **kwargs)
...
def from_cli_multi_ifos(opt, ifos, inj_filter_rejector_dict=None, **kwargs):
    ...
    for ifo in ifos:
        strain[ifo] = from_cli_single_ifo(
            opt, ifo, inj_filter_rejector_dict[ifo], **kwargs
        )
    return strain
...
def insert_strain_option_group(parser, gps_times=True):
    ...
    data_reading_group = parser.add_argument_group(
    ...
    # Required options
    if gps_times:
        data_reading_group.add_argument(
            "--gps-start-time",
    ...
    data_reading_group.add_argument(
        "--strain-high-pass", type=float, help="High pass frequency"
    )
    data_reading_group.add_argument(
        "--strain-low-pass", type=float, help="Low pass frequency"
    )
    data_reading_group.add_argument(
        "--pad-data",
        default=8,
        help="Extra padding to remove highpass corruption (integer seconds, default 8)",
        type=int,
    )
    data_reading_group.add_argument(
        "--taper-data",
    ...
    data_reading_group.add_argument(
        "--frame-files", type=str, nargs="+", help="list of frame files"
    )
    # Read from hdf store file
    data_reading_group.add_argument(
        "--hdf-store", type=str, help="Store of time series data in hdf format"
    )
    ...
    data_reading_group.add_argument(
        ...
        type=float,
        default=1,
        help="Divide injections by this factor before adding to the strain data",
    )
    data_reading_group.add_argument(
    ...
    data_reading_group.add_argument(
        "--injection-f-ref",
        type=float,
        help="Reference frequency in Hz for creating CBC injections from an XML file",
    )
    data_reading_group.add_argument(
        "--injection-f-final",
        type=float,
        help="Override the f_final field of a CBC XML injection file (frequency in Hz)",
    )
    # Options for getting injection from frame files
    data_reading_group.add_argument(
        "--injection-channel-name",
        type=str,
    ...
    data_reading_group.add_argument(
        ...
        metavar="SECONDS",
        default=5.0,
        help="Length of clustering window for detecting glitches for autogating.",
    )
    data_reading_group.add_argument(
        "--autogating-width",
        type=float,
    ...
    data_reading_group.add_argument(
        "--zpk-k",
        type=float,
        help="(optional) Zero-pole-gain (zpk) filter strain. Transfer function gain",
    )
...
def insert_strain_option_group_multi_ifo(parser, gps_times=True):
    ...
    data_reading_group_multi = parser.add_argument_group(
        "Options for obtaining h(t)",
        "These options are used for generating h(t) either by "
        "reading from a file or by generating it. This is only "
        "needed if the PSD is to be estimated from the data, ie. "
...
def gate_data(data, gate_params):
    ...
    def inverted_tukey(M, n_pad):
        ...
        padarr = 0.5 * (1.0 + numpy.cos(numpy.pi * numpy.arange(n_pad) / n_pad))
...
class StrainSegments(object):
    ...
    def __init__(
        ...
        data_start = (trigger_start - segment_start_pad) - int(strain.start_time)
        ...
        if filter_inj_only and hasattr(strain, "injections"):
            ...
            inj_idx = [
                (float(time) - float(strain.start_time)) * strain.sample_rate
                for time in end_times
            ]
        ...
        for seg, ana in zip(self.segment_slices, self.analyze_slices):
            ...
            if filter_inj_only and hasattr(strain, "injections"):
                ...
                for inj_id in inj_idx:
                    if inj_id < (cum_end + inj_window) and inj_id > (
                        cum_start - inj_window
                    ):
                        analyze_this = True
    ...
    def insert_segment_option_group(cls, parser):
        ...
        segment_group.add_argument(
            "--segment-end-pad",
            type=int,
            help="The time in seconds to ignore at the end of each segment in seconds.",
        )
        segment_group.add_argument(
            "--allow-zero-padding",
            action="store_true",
        ...
        segment_group.add_argument(
            ...
            default=None,
            type=float,
            help="""If using --filter-inj-only then
    ...
    def from_cli_single_ifo(cls, opt, strain, ifo):
        ...
        return cls(
            strain,
            segment_length=opt.segment_length[ifo],
    ...
    def from_cli_multi_ifos(cls, opt, strain_dict, ifos):
        ...
        strain_segments = {}
        for ifo in ifos:
            strain_segments[ifo] = cls.from_cli_single_ifo(opt, strain_dict[ifo], ifo)
...
def _create_memory_and_engine_for_class_based_fft(
    ...
    vectilde = FrequencySeries(
        zeros(npoints_freq, dtype=complex_same_precision_as(vec)),
        delta_f=delta_f_tmp,
        copy=False,
    )
    if ifft:
...
def execute_cached_fft(
    invec_data, normalize_by_rate=True, ifft=False, copy_output=True, uid=0
):
    ...
    invec, outvec, fft_class = create_memory_and_engine_for_class_based_fft(
        npoints_time, dtype, delta_t=delta_t, ifft=ifft, uid=uid
    )
...
class StrainBuffer(pycbc.frame.DataBuffer):
    def __init__(
        ...
        increment_update_cache: {str, None}, Optional
            ...
            filesystem.
        """
        super(StrainBuffer, self).__init__(
            frame_src,
            channel_name,
            start_time,
        ...
        if state_channel is not None:
            ...
            logger.info(
                "State channel %s interpreted as bitmask %s = good",
                state_channel,
                bin(valid_mask),
            )
        ...
        if data_quality_channel is not None:
            ...
            if (
                len(self.data_quality_flags) == 1
                and self.data_quality_flags[0] == "veto_nonzero"
            ):
                sb_kwargs["valid_on_zero"] = True
                logger.info(
                    "DQ channel %s interpreted as zero = good", data_quality_channel
                )
            else:
            ...
            self.dq = pycbc.frame.StatusBuffer(
                frame_src, data_quality_channel, start_time, **sb_kwargs
            )
        ...
        if idq_channel is not None:
            ...
            self.idq = pycbc.frame.iDQBuffer(
                frame_src,
                idq_channel,
                idq_state_channel,
                idq_threshold,
        ...
        self.strain = TimeSeries(
            zeros(strain_len, dtype=numpy.float32),
        ...
        highpass_samples, self.beta = kaiserord(
            self.highpass_reduction,
    ...
    def end_time(self):
        ...
        return float(
            self.strain.start_time
            + (len(self.strain) - self.total_corruption) / self.sample_rate
        )
    ...
    def add_hard_count(self):
        ...
        self.wait_duration = int(
            numpy.ceil(self.total_corruption / self.sample_rate + self.psd_duration)
        )
        self.invalidate_psd()
    ...
    def recalculate_psd(self):
        ...
        psd = pycbc.psd.welch(
            self.strain[s:e], seg_len=seg_len, seg_stride=seg_len // 2
        )

        psd.dist = (
            spa_distance(psd, 1.4, 1.4, self.low_frequency_cutoff) * pycbc.DYN_RANGE_FAC
        )

        # If the new psd is similar to the old one, don't replace it
        if self.psd and self.psd_recalculate_difference:
            if (
                abs(self.psd.dist - psd.dist) / self.psd.dist
                < self.psd_recalculate_difference
            ):
                logger.info(
    ...
    def near_hwinj(self):
        ...
        if not self.state.is_extent_valid(
            self.start_time, self.blocksize, pycbc.frame.NO_HWINJ
        ):
            return True
        return False
    ...
    def advance(self, blocksize, timeout=10):
        ...
        if self.state and self.state.advance(blocksize) is False:
            ...
            if self.idq:
                self.idq.null_advance(blocksize)
            logger.info("%s time has invalid data, resetting buffer", self.detector)
            return False
        ...
        strain = pycbc.filter.highpass_fir(
            strain, self.highpass_frequency, self.highpass_samples, beta=self.beta
        )
        ...
        if self.taper_immediate_strain:
            ...
            strain = gate_data(
                strain, [(strain.start_time, 0.0, self.autogating_taper)]
            )
    ...
    def from_cli(cls, ifo, args):
        ...
        if (
            args.state_channel
            and ifo in args.state_channel
            and args.analyze_flags
            and ifo in args.analyze_flags
        ):
        ...
        if (
            args.data_quality_channel
            and ifo in args.data_quality_channel
            and args.data_quality_flags
        ...
        if args.frame_type:
            frame_src = pycbc.frame.frame_paths(
                args.frame_type[ifo], args.start_time, args.end_time, site=ifo[0]
            )
        else:
