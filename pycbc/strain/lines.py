...
def _torch_binned_inner_product(data1, data2, bin_size):
    """Return binned line products and their median on the Torch device."""
    ...
    return inner_prod, inner_median


def _torch_avg_inner_product(data1, data2, bin_size):
    """Calculate binned line inner products on the active Torch device."""
    inner_prod, inner_median = _torch_binned_inner_product(
        data1, data2, bin_size
    )

    # This public result is documented as Python containers and scalars.
...
def _torch_line_scalar(value, *, dtype, device):
    """Keep an internal line parameter scalar on the active Torch device."""
    if not isinstance(value, torch.Tensor):
        return torch.tensor(float(value), dtype=dtype, device=device)
    if value.numel() != 1 or value.is_complex():
        raise TypeError("line amplitude and phase must be real scalars")
...
def line_model(freq, data, tref, amp=1, phi=0):
    ...
    if _is_torch_series(data):
        ...
        amplitude = _torch_line_scalar(amp, dtype=dtype, device=device)
        phase = _torch_line_scalar(phi, dtype=dtype, device=device)
        alpha = 2 * torch.pi * float(freq) * times + phase
        values = torch.polar(
            amplitude.expand_as(alpha), alpha
        )
        return TimeSeries(
...
def matching_line(freq, data, tref, bin_size=1):
    ...
    if _is_torch_series(data):
        _, inner_median = _torch_binned_inner_product(
            data, template_line, bin_size
        )
        amp = torch.abs(inner_median)
        phi = torch.angle(inner_median)
    else:
        _, amp, phi = avg_inner_product(
            data, template_line, bin_size=bin_size
        )
