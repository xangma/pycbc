...
def check_same_len_precision(a, b):
    ...
    """
    if len(a) != len(b):
        msg = "lengths do not match ({} vs {})".format(len(a), len(b))
        raise ValueError(msg)
    if a.precision != b.precision:
        msg = "precisions do not match ({} vs {})".format(a.precision, b.precision)
        raise TypeError(msg)
...
class Array(object):
        ...
        if copy:
            ...
            if dtype is not None:
                ...
                if dtype.kind != "c" and initial_array.dtype.kind == "c":
                    raise TypeError(
                        str(initial_array.dtype) + " cannot be cast as " + str(dtype)
                    )
    ...
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        ...
        if backend_ufunc is not None and all(
            not isinstance(value, Array) or value._scheme is active_scheme
            for value in inputs
        ):
            backend_inputs = [
                value._data if isinstance(value, Array) else value for value in inputs
            ]
    ...
    def __array_function__(self, func, types, args, kwargs):
        ...
        if not all(
            issubclass(array_type, (Array, _numpy.ndarray)) for array_type in types
        ):
            ...
            if (
                ...
                if result is not NotImplemented:
                    ...
                    if func in (
                        ...
                        if isinstance(result, tuple):
                            return tuple(
                                ...
                                for original, value in zip(args, result, strict=True)
                            )
                        if (
                            len(args) == 1
                    ...
                    if func is _numpy.broadcast_arrays:
                        return tuple(
                            original
                            if isinstance(original, Array) and value is original._data
                            else Array(value, copy=False)
                            for original, value in zip(args, result, strict=True)
                        )
                    return Array(result, copy=False)
    ...
    def clip(self, min=None, max=None, out=None, **kwargs):
        ...
        host_bounds = [
            bound.numpy() if isinstance(bound, Array) else bound for bound in (min, max)
        ]
    ...
    def _backend_variance(self, axis, dtype, out, ddof, keepdims, where, mean):
        ...
        if (
            ...
            or where is not True
            or mean is not None
            or not isinstance(ddof, (int, float, _numpy.integer, _numpy.floating))
        ):
            return NotImplemented
    ...
    def save(self, path, group=None):
        ...
        if ext == ".npy":
            ...
            if self.kind == "real":
                ...
                output = _numpy.vstack((self.numpy().real, self.numpy().imag)).T
