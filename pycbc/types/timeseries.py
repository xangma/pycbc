...
def _torch_at_time_host_scalar(
    ...
    if extrapolate is not None:
        if not (_numpy.isscalar(extrapolate) and _numpy.isreal(extrapolate)):
    ...
    scalar_dtype = (
        _numpy.float32 if tensor.real.dtype == torch.float32 else _numpy.float64
    )
...
def _torch_taper_peak(amplitude, start, end, step):
    ...
    run_starts = torch.where(new_run, scan_order, torch.zeros_like(scan_order))
    run_starts = torch.cummax(run_starts, dim=0).values
    visited = scan_candidates & ((scan_order - run_starts).remainder(2) == 0)
...
def _torch_apply_taper_side(result, indices, boundary, length, step, valid):
    ...
    safe_distance = torch.where(interior, distance, torch.ones_like(distance))
    ...
    result = result * torch.where(interior, taper, torch.ones_like(taper))
...
def _torch_lal_taper(tensor, location):
    ...
    nonzero = result != 0
    start = torch.where(nonzero, indices, torch.full_like(indices, count)).amin()
    end = torch.where(nonzero, indices, torch.full_like(indices, -1)).amax()
    ...
    if location != "end":
        ...
        result = _torch_apply_taper_side(result, indices, start, peak - start, 1, valid)

    if location in ("end", "startend"):
        peak = _torch_taper_peak(amplitude, end, midpoint, -1)
        result = _torch_apply_taper_side(result, indices, end, end - peak, -1, valid)
...
def _torch_constant_taper_parameters(count, delta_t, epoch, taper_window):
    ...
    window_ns = int(window_gps.gpsSeconds) * nanoseconds + int(
        window_gps.gpsNanoSeconds
    )

    last_index = max(count - 1, 0)
    if last_index * nanoseconds > int64_max:
        return None
    max_center_ns = (last_index * nanoseconds + integer_rate - 1) // integer_rate
...
def _torch_constant_taper(
    tensor, location, sample_rate, window_ns, window_length, padding_length
):
    ...
    indices = torch.arange(count, device=tensor.device, dtype=torch.int64)
    nonzero = tensor != 0
    valid = nonzero.any()
    first = torch.where(nonzero, indices, torch.full_like(indices, count)).amin()
    last = torch.where(nonzero, indices, torch.full_like(indices, -1)).amax()
    ...
    for boundary in boundaries:
        ...
        offset = torch.div(product_ns, 1_000_000_000, rounding_mode="trunc")

        window_index = indices - offset
        inside = valid & (window_index >= 0) & (window_index < window_length)
        if padding_length:
            ...
            padding_value = 0.5 * (
                1.0
                + torch.cos(torch.pi * padding_index.to(tensor.dtype) / padding_length)
            )
        ...
        multiplier = torch.where(inside, selected, torch.ones_like(tensor))
...
class TimeSeries(Array):
    ...
    def _typecheck(self, other):
        if isinstance(other, TimeSeries):
            if not self.sample_rate_close(other):
                raise ValueError(
                    "different delta_t, {} vs {}".format(self.delta_t, other.delta_t)
                )
            if not self.epoch_close(other):
                raise ValueError(
                    "different epoch, {} vs {}".format(
    ...
    def _getslice(self, index):
        ...
        if index.start is None or self._epoch is None:
            ...
            if index.start < 0:
                raise ValueError(
                    ("Negative start index ({}) not supported").format(index.start)
                )
        ...
        return TimeSeries(
            Array._getslice(self, index), new_delta_t, new_epoch, copy=False
        )
    ...
    def time_slice(self, start, end, mode="floor"):
        ...
        if start < self.start_time:
            raise ValueError(
                "Time series does not contain a time as early as %s" % start
            )
    ...
    end_time = property(get_end_time, doc="Time series end time.")
    ...
    def at_time(self, time, nearest_sample=False, interpolate=None, extrapolate=None):
        ...
        if tensor is not None:
            ...
            query_tensor = getattr(getattr(time, "_data", None), "tensor", None)
            ...
            if (
                ...
                scalar_result = _torch_at_time_host_scalar(
                    self, tensor, time, nearest_sample, interpolate, extrapolate
                )
            ...
            if query_tensor is not None:
                ...
                coordinate_dtype = (
                    tensor.real.dtype if tensor.device.type == "mps" else torch.float64
                )
                ...
                if tensor.device.type == "mps":
                    ...
                    values = _numpy.asarray(values, dtype=_numpy.float64) - float(
                        self.start_time
                    )
                    relative_times = torch.as_tensor(
                        values, device=tensor.device, dtype=tensor.real.dtype
                    ).reshape(-1)
                else:
                    relative_times = torch.as_tensor(
            ...
            if extrapolate is not None:
                if _numpy.isscalar(extrapolate) and _numpy.isreal(extrapolate):
                    ...
                    keep = (relative_times >= self.delta_t * facl) & (
                        relative_times < self.duration - self.delta_t * facr
                    )
            ...
            if fill_value is not None:
                ...
                old = ans
                ans = torch.full(
                    (size,), fill_value, device=tensor.device, dtype=tensor.dtype
                )
                ans[keep] = old
        ...
        if super(TimeSeries, self).almost_equal_elem(other, tol=tol, relative=relative):
            if relative:
                return (
                    ...
                    and abs(self._delta_t - other._delta_t) <= dtol * self._delta_t
                )
            else:
                return (
                    self._epoch == other._epoch
                    and abs(self._delta_t - other._delta_t) <= dtol
                )
        else:
            return False
    ...
    def lal(self):
        ...
        if self._data.dtype == _numpy.float32:
            lal_data = _lal.CreateREAL4TimeSeries(
                "", ep, 0, self.delta_t, _lal.SecondUnit, len(self)
            )
        elif self._data.dtype == _numpy.float64:
            lal_data = _lal.CreateREAL8TimeSeries(
                "", ep, 0, self.delta_t, _lal.SecondUnit, len(self)
            )
        elif self._data.dtype == _numpy.complex64:
            lal_data = _lal.CreateCOMPLEX8TimeSeries(
                "", ep, 0, self.delta_t, _lal.SecondUnit, len(self)
            )
        elif self._data.dtype == _numpy.complex128:
            lal_data = _lal.CreateCOMPLEX16TimeSeries(
                "", ep, 0, self.delta_t, _lal.SecondUnit, len(self)
            )
    ...
    def psd(self, segment_duration, **kwds):
        ...
        return welch(self, seg_len=seg_len, seg_stride=seg_stride, **kwds)

    # map between tapering string in sim_inspiral table or inspiral
    # code option and lalsimulation constants

    def taper_timeseries(
        self, location=None, tapermethod="lal", return_lal=False, taper_window=None
    ):
        """
        ...
        if location is None:
            raise ValueError(
                "Must specify a tapering method (function was calledwith location=None)"
            )
        if location not in taper_locations:
            raise ValueError(
                "Unknown location %s, valid locations are %s"
                % (location, ", ".join(taper_locations))
            )
        if tsdata.dtype not in (float32, float64):
            raise TypeError(
                "Strain dtype must be float32 or float64, not " + str(tsdata.dtype)
            )
        if tapermethod == "lal":
            ...
            if tensor is not None and not return_lal:
                ...
                tapered = _torch_lal_taper(tensor, taper_locations[location])
            ...
            if return_lal:
                ...
                return TimeSeries(
                    ts_lal.data.data[:], delta_t=ts_lal.deltaT, epoch=ts_lal.epoch
                )
        elif tapermethod == "constant":
            # constant window tapering
            if taper_window is None:
                raise ValueError(
                    "If taper_method is 'constant', taper_window must be set"
                )
            ...
            if taper_location != "none":
                ...
                if tensor is not None:
                    ...
                    if parameters is not None and not tensor.requires_grad:
                        _torch_constant_taper(tensor, taper_location, *parameters)
                        return tsdata
                    import torch

                    nonzero = torch.nonzero(tensor != 0, as_tuple=False).flatten()
                    if nonzero.numel() == 0:
                        return tsdata
            ...
            raise ValueError(
                "Unknown tapering method %s, valid methods are lal and constant"
                % (tapermethod)
            )
    ...
    def gate(
        ...
        if method == "taper":
            ...
            if invpsd is None:
                ...
                invpsd = 1.0 / self.filter_psd(self.duration / 32, self.delta_f, 0)
            ...
            if offset == 0:
                if paint_method == "toeplitz":
                    ...
                    return gate_and_paint_matmul(
                        data, lindex, rindex, invpsd, invmat=paint_invmat, copy=False
                    )
                else:
    ...
    def filter_psd(self, segment_duration, delta_f, flow):
        ...
        return inverse_spectrum_truncation(
            p, samples, low_frequency_cutoff=flow, trunc_method="hann"
        )
    ...
    def qtransform(
        ...
        """
        from pycbc.filter.qtransform import qplane, qtiling
        ...
        if use_torch:
            ...
            fft_input = self
            if self._data.tensor.device.type != "mps" and self.dtype == _numpy.dtype(
                _numpy.float32
            ):
            ...
            grid_dtype = torch.float32 if device.type == "mps" else torch.float64
        ...
        if delta_f or delta_t or logfsteps:
            if return_complex:
                ...
                interp_phase = interp2d(freqs, times, _numpy.angle(q_plane), kx=1, ky=1)
            else:
                interp = interp2d(freqs, times, q_plane, kx=1, ky=1)

        if delta_t:
            times = _numpy.arange(float(self.start_time), float(self.end_time), delta_t)
        if delta_f:
            freqs = _numpy.arange(int(frange[0]), int(frange[1]), delta_f)
        if logfsteps:
            freqs = _numpy.logspace(
                _numpy.log10(frange[0]), _numpy.log10(frange[1]), logfsteps
            )
        ...
        if ext == ".npy":
            ...
            if self.kind == "real":
                ...
                output = _numpy.vstack(
                    (self.sample_times.numpy(), self.numpy().real, self.numpy().imag)
                ).T
    ...
    def to_frequencyseries(self, delta_f=None):
        ...
        if tlen < len(self):
            raise ValueError(
                "The value of delta_f (%s) would be "
        ...
        f = FrequencySeries(
            zeros(flen, dtype=complex_same_precision_as(self)), delta_f=delta_f
        )
        fft(tmp, f)
        f._delta_f = delta_f
        return f
    ...
    def inject(self, other, copy=True):
        ...
        if copy:
            ...
            ts = self
        # Other is disjoint
        if (other.start_time >= ts.end_time) or (ts.start_time > other.end_time):
            return ts
    ...
    def match(
        ...
        """
        return self.to_frequencyseries().match(
            other,
            psd=psd,
            low_frequency_cutoff=low_frequency_cutoff,
            ...
            if type in ("constant", "c"):
                ...
                if len(tensor) == 1:
                    ...
                    positions = torch.arange(
                        n, dtype=tensor.real.dtype, device=tensor.device
                    )
                    positions -= (n - 1) / 2
                    norm_sq = (n * (n * n - 1)) / 12.0
                    slope = torch.sum(positions * tensor) / norm_sq
                    result = tensor - (tensor.mean() + slope * positions)
            else:
...
def load_timeseries(path, group=None):
    ...
    if ext == ".npy":
        ...
        with h5py.File(path, "r") as f:
            data = f[key][:]
            series = TimeSeries(
                data, delta_t=f[key].attrs["delta_t"], epoch=f[key].attrs["start_time"]
            )
        return series
    else:
    ...
    raise ValueError(
        "File has %s dimensions, cannot convert to TimeSeries, \
