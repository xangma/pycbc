...
"""NumPy compatibility for the PyCBC Torch array backend.

This module owns the comparatively large ``__array_function__`` and
``__array_ufunc__`` implementation.  The storage object and PyCBC scheme
operations remain in :mod:`pycbc.types.array_torch` so backend discovery and
public imports do not depend on this module's internal organization.
"""
...
def _is_torch_array_data(value):
    """Identify PyCBC Torch storage through the shared backend protocol."""
    return backend_name(value) == "torch" and isinstance(
        getattr(value, "tensor", None), torch.Tensor
    )
...
def _torch_sign(tensor):
    """Preserve NaNs, which Torch's real sign operation converts to zero."""
    return torch.sign(tensor).masked_fill(torch.isnan(tensor), float("nan"))
...
def _device_matches_active(tensor):
    ...
    return device.type == active.type and (
        active.index is None or device.index == active.index
    )
...
def _comparison_tensors(tensor_a, dtype_a, other):
    ...
    if isinstance(other, torch.Tensor):
        ...
        if np.isscalar(other):
            ...
            target_np = np.result_type(np.dtype(dtype_a), other_values.dtype)
...
def _binary_ufunc_tensors(inputs, reference):
    ...
    for operand in inputs:
        if _is_torch_array_data(operand):
...
def _comparison_ufunc_result(inputs, reference, operation):
    ...
    try:
        if tuple(torch.broadcast_shapes(*shapes)) != reference.shape:
            return None
        if _is_torch_array_data(inputs[0]):
...
def _bitwise_ufunc_tensors(inputs, reference):
    ...
    try:
        ...
        execution_torch = torch.int64 if target_np.kind == "u" else target_torch
        if tuple(torch.broadcast_shapes(*shapes)) != reference.shape:
            return None
        tensors = []
        for operand in inputs:
...
class _NumpyMixin:
    """Implement NumPy protocols for Torch-backed PyCBC array storage."""
    ...
    def numpy_variance(self, axes, dtype=None, ddof=0, keepdims=False):
        ...
        try:
            ...
            if denominator < 0:
                denominator = 0
            if axes:
                result = torch.sum(squared, dim=axes, keepdim=keepdims) / denominator
            else:
    ...
    def numpy_arg_reduce(self, operation, axis=None, keepdims=False):
        ...
        if reduction_size == 0:
            raise ValueError(f"attempt to get arg{operation} of an empty sequence")
        ...
        if result_shape and result_size == 0:
            return self._wrap(
                torch.empty(result_shape, dtype=torch.int64, device=self.tensor.device)
            )
    ...
    def numpy_searchsorted(self, values, side="left", sorter=None):
        ...
        if not isinstance(side, str):
            raise TypeError(f"search side must be str, not {type(side).__name__}")
        if side not in ("left", "right"):
            raise ValueError(f"search side must be 'left' or 'right' (got {side!r})")
        if (
        ...
        if sorter is not None:
            if _is_torch_array_data(sorter):
            ...
            if sorter_dtype.kind not in "iu":
                return NotImplemented
            if sorter_tensor.ndim != 1 or sorter_tensor.numel() != sequence.numel():
                return NotImplemented
        ...
        if _is_torch_array_data(values):
            if values.tensor.device != sequence.device:
                return NotImplemented
            values = values.tensor
        elif isinstance(values, torch.Tensor) and values.device != sequence.device:
            return NotImplemented
        ...
        try:
            if sequence.is_floating_point():
                ...
                finite_count = sequence.numel() - torch.count_nonzero(sequence_nan)
    ...
    def numpy_argsort(self, axis=-1, kind=None, order=None, stable=None):
        ...
        stable_sort = (
            bool(stable) if stable is not None else kind in ("stable", "mergesort")
        )
        try:
    ...
    def numpy_nonzero(self):
        ...
        if not _device_matches_active(self.tensor) or (
            self.tensor.device.type == "mps" and self.tensor.is_complex()
        ):
            return NotImplemented
        try:
    ...
    def _numpy_index_locations(self, function, args, kwargs):
        ...
        if (
            options
            or not _is_torch_array_data(array)
            or array is not self
            or not _device_matches_active(array.tensor)
            or (array.tensor.device.type == "mps" and array.tensor.is_complex())
        ):
            return NotImplemented
    ...
    def _numpy_where(self, args, kwargs):
        ...
        try:
            ...
            if _is_torch_array_data(condition):
    ...
    def _numpy_diff(self, args, kwargs):
        ...
        for name, value in zip(parameters, args[1:], strict=False):
            if name in options:
                return NotImplemented
        ...
        array = args[0]
        if not _is_torch_array_data(array):
            return NotImplemented
        n = parameters["n"]
        try:
            if n == 0:
        ...
        try:
            ...
            for _ in range(n):
                if target_np.kind == "b":
                    tensor = torch.ne(tensor[slices_after], tensor[slices_before])
                else:
                    tensor = tensor[slices_after] - tensor[slices_before]
    ...
    def _numpy_diag(self, function, args, kwargs):
        ...
        array = parameters["v"]
        if (
            not _is_torch_array_data(array)
            or not _device_matches_active(array.tensor)
            or array.tensor.device != self.tensor.device
        ):
            return NotImplemented
    ...
    def _numpy_triangle(self, function, args, kwargs):
        ...
        try:
            tri_k = parameters["k"] if function is np.tril else parameters["k"] - 1
    ...
    def _numpy_trace(self, args, kwargs):
        ...
        parameters = {}
        for index, name in enumerate(("a", "offset", "axis1", "axis2", "dtype", "out")):
            if index < len(args):
                if name in options:
                    return NotImplemented
                parameters[name] = args[index]
        ...
        try:
            ...
            if target_dtype.kind in "biu":
                ...
                result = torch.sum(tensor.to(dtype=target_torch), dim=-1)
        except (OverflowError, TypeError, ValueError, RuntimeError):
            return NotImplemented
        ...
        if axis is None:
            ...
            for original_axis in axes:
                ...
                if normalized_axis < 0 or normalized_axis >= tensor.ndim:
                    raise np.exceptions.AxisError(original_axis, ndim=tensor.ndim)
    ...
    def _numpy_rot90(self, args, kwargs):
        ...
        if (
            axes[0] >= array.ndim
            or axes[0] < -array.ndim
            or axes[1] >= array.ndim
            or axes[1] < -array.ndim
        ):
            raise ValueError(
    ...
    def _numpy_expand_dims(self, args, kwargs):
        ...
        for original_axis in axes:
            ...
            if normalized_axis < 0 or normalized_axis >= output_ndim:
                raise np.exceptions.AxisError(original_axis, ndim=output_ndim)
            if normalized_axis in normalized_axes:
                raise ValueError("repeated axis")
            normalized_axes.append(normalized_axis)
    ...
    def _normalize_movement_axes(axis, ndim, name):
        ...
        for original_axis in axes:
            ...
            if normalized_axis < 0 or normalized_axis >= ndim:
                raise np.exceptions.AxisError(original_axis, ndim=ndim, msg_prefix=name)
            if normalized_axis in normalized_axes:
    ...
    def _numpy_moveaxis(self, args, kwargs):
        ...
        dimensions = [
            dimension for dimension in range(array.ndim) if dimension not in source
        ]
    ...
    def _numpy_rollaxis(self, args, kwargs):
        ...
        try:
            ...
            return NotImplemented
        normalized_axis = (
            original_axis + array.ndim if original_axis < 0 else original_axis
        )
    ...
    def _numpy_atleast_nd(self, function, args, kwargs):
        ...
        if kwargs or not args or not all(_is_torch_array_data(array) for array in args):
            return NotImplemented
        if not all(
        ...
        for array in args:
            ...
            if tensor.ndim >= minimum_ndim:
                ...
                result = (
                    tensor.reshape(1, 1) if tensor.ndim == 0 else tensor.unsqueeze(0)
                )
            elif tensor.ndim == 0:
            ...
            results.append(array if result is tensor else array._wrap(result))
    ...
    def _numpy_broadcast(self, function, args, kwargs):
        ...
        if function is np.broadcast_arrays:
            if (
                options
                or not args
                or not all(_is_torch_array_data(array) for array in args)
            ):
                return NotImplemented
            if not all(
                _device_matches_active(array.tensor)
                ...
                return NotImplemented
            try:
                tensors = torch.broadcast_tensors(*(array.tensor for array in args))
            except RuntimeError:
                return NotImplemented
            return tuple(
                array if tensor is array.tensor else array._wrap(tensor)
                for array, tensor in zip(args, tensors, strict=True)
            )
        ...
        if any(dimension < 0 for dimension in normalized_shape):
            return NotImplemented
        try:
            return array._wrap(torch.broadcast_to(array.tensor, normalized_shape))
        except RuntimeError:
            return NotImplemented
    ...
    def _numpy_close(self, function, args, kwargs):
        ...
        parameters = {}
        for index, name in enumerate(("a", "b", "rtol", "atol", "equal_nan")):
            if index < len(args):
                if name in options:
                    return NotImplemented
                parameters[name] = args[index]
        ...
        arrays = [operand for operand in operands if _is_torch_array_data(operand)]
        if not arrays:
            return NotImplemented
        ...
        for name in ("rtol", "atol"):
            value = parameters[name]
            if not np.isscalar(value) or np.asarray(value).dtype.kind not in "biuf":
                return NotImplemented
            try:
                value = float(value)
            except (OverflowError, TypeError, ValueError):
        ...
        try:
            ...
            target_np = np.result_type(*dtype_inputs)
            target_torch = _ensure_supported(device, _torch_dtype(target_np))
            tensors = []
            for operand in operands:
                if _is_torch_array_data(operand):
                    tensor = operand.tensor.to(dtype=target_torch)
    ...
    def _numpy_array_equality(self, function, args, kwargs):
        ...
        right = parameters["a2"]
        if (
            not _is_torch_array_data(left)
            or not _is_torch_array_data(right)
        ...
        try:
            ...
            if equal_nan and (
                left_tensor.is_floating_point() or left_tensor.is_complex()
            ):
                result = result | (torch.isnan(left_tensor) & torch.isnan(right_tensor))
            return bool(torch.all(result).item())
        except (OverflowError, TypeError, ValueError, RuntimeError):
            return NotImplemented
    ...
    def _numpy_average(self, args, kwargs):
        ...
        parameters = {}
        for index, name in enumerate(("a", "axis", "weights", "returned")):
            if index < len(args):
                if name in options:
                    return NotImplemented
                parameters[name] = args[index]
        ...
        try:
            axes = _normalized_reduction_axes(axis, array.tensor.ndim)
        ...
        if weights is None:
            output_dtype = (
                np.dtype(np.float64) if input_dtype.kind in "biu" else input_dtype
            )
            try:
            ...
            if axes:
                ...
                result = torch.mean(execution, dim=axes, keepdim=keepdims)
            else:
            ...
            if _is_torch_array_data(weights):
                if (
            ...
            if axes:
                scale = torch.sum(weight_tensor, dim=axes, keepdim=keepdims)
                numerator = torch.sum(
    ...
    def _numpy_median(self, args, kwargs):
        ...
        input_dtype = np.dtype(array.dtype)
        output_dtype = input_dtype if input_dtype.kind == "f" else np.dtype(np.float64)
        try:
            output_torch = _ensure_supported(tensor.device, _torch_dtype(output_dtype))
        except TypeError:
            return NotImplemented
        ...
        if not axes and not flatten_all:
            ...
            kept_axes = tuple(
                dimension for dimension in range(tensor.ndim) if dimension not in axes
            )
            ...
            if reduction_size == 0:
                warnings.warn("Mean of empty slice.", RuntimeWarning, stacklevel=4)
                result = torch.full(
                    kept_shape,
                    torch.nan,
                    dtype=output_torch,
    ...
    def _numpy_ptp(self, args, kwargs):
        ...
        if out is not None:
            if not _is_torch_array_data(out):
                return NotImplemented
            if (
        ...
        try:
            keepdims = bool(operator.index(parameters["keepdims"]))
            axes = _normalized_reduction_axes(parameters["axis"], tensor.ndim)
        except (IndexError, TypeError, ValueError):
            return NotImplemented
        ...
        try:
            ...
            if not axes:
                ...
                maximum = torch.amax(reduction_input, dim=axes, keepdim=keepdims)
                minimum = torch.amin(reduction_input, dim=axes, keepdim=keepdims)
            ...
            if out is None:
                ...
                subtraction_dtype = np.result_type(output_dtype, input_dtype)
                if not np.can_cast(
                    subtraction_dtype, output_dtype, casting="same_kind"
                ):
                    raise TypeError(
                ...
                result = maximum.to(dtype=subtraction_torch_dtype) - minimum.to(
    ...
    def _numpy_ravel(self, args, kwargs):
        ...
        if not args or len(args) > 2:
            return NotImplemented
        array = args[0]
        if not _is_torch_array_data(array) or array is not self:
            return NotImplemented
        if len(args) == 2:
            if "order" in options:
                return NotImplemented
    ...
    def _numpy_copy(self, args, kwargs):
        ...
        if args:
            ...
            try:
                ...
                return NotImplemented
        for name, value in zip(("order", "subok"), args[1:], strict=False):
            if name in options:
                return NotImplemented
            parameters[name] = value
        for name in tuple(parameters):
        ...
        if order == "A":
            order = (
                "F"
                if (not tensor.is_contiguous() and self._is_fortran_contiguous(tensor))
                else "C"
            )
        if order == "C":
            result = tensor.clone() if tensor.is_contiguous() else tensor.contiguous()
        elif order == "F":
            if tensor.numel() == 0:
    ...
    def _numpy_like_order(order):
        ...
        if order not in ("C", "F", "A", "K"):
            raise ValueError(
                f"order must be one of 'C', 'F', 'A', or 'K' (got {order!r})"
            )
        return order
    ...
    def _empty_like_ordered(self, source, shape, dtype, order):
        ...
        if order == "A":
            order = (
                "F"
                if (not source.is_contiguous() and self._is_fortran_contiguous(source))
                else "C"
            )

        if not shape:
            return torch.empty(shape, dtype=dtype, device=source.device)
        ...
        if order == "F":
            ...
            for axis in reversed(permutation):
                ...
                stride *= shape[axis]
            strides = tuple(strides)
        return torch.empty_strided(shape, strides, dtype=dtype, device=source.device)
    ...
    def _numpy_like_creator(self, function, args, kwargs):
        ...
        names = (
            ...
            if is_full
            else ("prototype", "dtype", "order", "subok", "shape")
        )
        ...
        shape = self._numpy_like_shape(parameters["shape"], array.shape)
        ...
        result = self._empty_like_ordered(array.tensor, shape, torch_dtype, order)
        ...
        if _is_torch_array_data(fill_value):
            if (
    ...
    def _numpy_tile(self, args, kwargs):
        ...
        if _is_torch_array_data(reps) or isinstance(reps, torch.Tensor):
            return NotImplemented
        try:
            ...
            try:
                ...
                if isinstance(reps, (str, bytes)) or not hasattr(reps, "__iter__"):
        ...
        interleaved_shape = tuple(value for size in shape for value in (1, size))
        expanded_shape = tuple(
            value
            for repetition, size in zip(repetitions, shape, strict=True)
    ...
    def _numpy_pad(self, args, kwargs):
        ...
        if _is_torch_array_data(pad_width) or isinstance(pad_width, torch.Tensor):
            return NotImplemented
        try:
        ...
        if width_array.dtype.kind not in "iu":
            return NotImplemented
        try:
            widths = np.broadcast_to(width_array, (array.tensor.ndim, 2))
        except ValueError:
            return NotImplemented
        if np.any(widths < 0):
            return NotImplemented
        widths = tuple(
            (operator.index(pair[0]), operator.index(pair[1])) for pair in widths
        )

        if mode == "constant":
            if _is_torch_array_data(constant_values) or isinstance(
                constant_values, torch.Tensor
            ):
                return NotImplemented
            try:
                constant_array = np.asarray(constant_values)
                constants = np.broadcast_to(constant_array, (array.tensor.ndim, 2))
            except (TypeError, ValueError):
                return NotImplemented
            if constant_array.dtype.kind not in "biufc":
                return NotImplemented
            if constant_array.dtype.kind == "c" and not array.tensor.is_complex():
                return NotImplemented
            try:
                cast_constants = []
    ...
    def _numpy_boolean_selection(self, function, args, kwargs):
        ...
        if not _is_torch_array_data(array) or not _device_matches_active(array.tensor):
            return NotImplemented
        ...
        if function is np.extract:
            ...
            if axis is None:
                ...
                try:
                    ...
                    return NotImplemented
                dimension = (
                    original_axis + source.ndim if original_axis < 0 else original_axis
                )
                if dimension < 0 or dimension >= source.ndim:
                    raise np.exceptions.AxisError(original_axis, ndim=source.ndim)
        ...
        try:
            ...
            result = torch.index_select(selection_source, dimension, indices)
            if source.dtype == _TORCH_UINT32:
                result = result.to(dtype=_TORCH_UINT32)
        except IndexError:
            raise
    ...
    def _numpy_append(self, args, kwargs):
        ...
        if axis is None:
            ...
            if dimension < 0 or dimension >= array.tensor.ndim:
                raise np.exceptions.AxisError(original_axis, ndim=array.tensor.ndim)
            left = array.tensor
            right = values_tensor
    ...
    def _numpy_resize(self, args, kwargs):
        ...
        repetitions = (new_size + flattened.numel() - 1) // flattened.numel()
    ...
    def _numpy_delete(self, args, kwargs):
        ...
        if isinstance(obj, slice):
            ...
            single_value = isinstance(obj, (int, np.integer)) and not isinstance(
                obj, (bool, np.bool_)
            )
            host_indices = False
            original_obj = obj
            if single_value:
                ...
                index_kind = "i"
            elif _is_torch_array_data(obj):
                if obj.tensor.device != source.device or not _device_matches_active(
                    obj.tensor
                ):
                    return NotImplemented
                ...
                try:
                    ...
                    return NotImplemented
                if index_values.size == 0 and not isinstance(original_obj, np.ndarray):
            ...
            if single_value:
                ...
                try:
                    ...
                    return NotImplemented
                delete_indices = torch.nonzero(delete_mask, as_tuple=False).reshape(-1)
        ...
        delete_mask = torch.zeros(size, dtype=torch.bool, device=source.device)
        ...
        try:
            ...
            result = torch.index_select(selection_source, dimension, keep_indices)
            if source.dtype == _TORCH_UINT32:
                result = result.to(dtype=_TORCH_UINT32)
        except RuntimeError:
            return NotImplemented
    ...
    def _numpy_putmask(self, args, kwargs):
        ...
        try:
            ...
            if _is_torch_array_data(values):
                ...
                values_tensor = values.tensor.to(dtype=target_torch).reshape(-1)
            ...
            if values_tensor.numel() == 0 or array.tensor.numel() == 0:
                return None
            indices = torch.arange(array.tensor.numel(), device=device).remainder(
                values_tensor.numel()
            )
            execution_dtype = (
                torch.int64 if target_torch == _TORCH_UINT32 else target_torch
            )
            replacements = values_tensor.to(dtype=execution_dtype)[indices]
            flattened = array.tensor.reshape(-1).to(dtype=execution_dtype)
    ...
    def _numpy_complex_component(self, function, args, kwargs):
        ...
        if _is_torch_array_data(degrees) or isinstance(degrees, torch.Tensor):
            return NotImplemented
        try:
    ...
    def _numpy_unwrap(self, args, kwargs):
        ...
        if discont_array.ndim != 0 or discont_array.dtype.kind not in "biuf":
            return NotImplemented

        try:
            dtype_probe = np.empty((), dtype=input_dtype)
            target_dtype = np.dtype(np.result_type(dtype_probe, period))
            comparison_dtype = np.dtype(np.result_type(dtype_probe, discont))
            target_torch = _ensure_supported(
                array.tensor.device, _torch_dtype(target_dtype)
            )
        ...
        delta_mod = (
            torch.remainder(delta_target - interval_low, period_tensor) + interval_low
        )
        ...
        small_delta = torch.abs(delta).to(dtype=comparison_torch) < discont_tensor
        ...
        suffix = source_target.narrow(axis, 1, source.shape[axis] - 1) + torch.cumsum(
            correction, dim=axis
        )
    ...
    def _numpy_take_along_axis(self, args, kwargs):
        ...
        if _is_torch_array_data(indices):
            ...
            try:
                ...
                index_tensor = torch.as_tensor(index_values, device=array.tensor.device)
            except (OverflowError, TypeError, ValueError, RuntimeError):
                return NotImplemented
        ...
        try:
            ...
            if execution.is_complex():
                result = torch.complex(
                    torch.take_along_dim(execution.real, index_tensor, dim=dimension),
                    torch.take_along_dim(execution.imag, index_tensor, dim=dimension),
                )
            else:
                result = torch.take_along_dim(execution, index_tensor, dim=dimension)
            if source.dtype == _TORCH_UINT32:
                result = result.to(dtype=_TORCH_UINT32)
    ...
    def _numpy_unique(self, args, kwargs):
        ...
        parameters = {}
        for index, name in enumerate(
            ("ar", "return_index", "return_inverse", "return_counts", "axis")
        ):
            if index < len(args):
                if name in options:
                    return NotImplemented
    ...
    def _numpy_histogram_weights(self, value, shape):
        ...
        if _is_torch_array_data(value):
            ...
            torch_dtype = _ensure_supported(self.tensor.device, _torch_dtype(dtype))
    ...
    def _numpy_histogram_explicit_edges(self, bins):
        ...
        if dtype.kind == "f" and bool(torch.any(torch.isnan(edges)).item()):
            ...
            raise TypeError
        execution = edges.to(dtype=torch.int64) if dtype.kind in "bu" else edges
    ...
    def _numpy_histogram_generated_edges(self, tensor, dtype, count, limits):
        ...
        if limits is None:
            if tensor.numel() == 0:
                ...
                extrema = tensor.to(dtype=torch.int64) if dtype.kind in "bu" else tensor
        ...
        if not (np.isfinite(first_edge) and np.isfinite(last_edge)):
            raise ValueError(
                f"{range_label} range of [{first_edge}, {last_edge}] is not finite"
            )
        if first_edge == last_edge:
        ...
        edge_torch = _ensure_supported(self.tensor.device, _torch_dtype(edge_dtype))
    ...
    def _numpy_histogram(self, args, kwargs):
        ...
        parameters = {}
        for index, name in enumerate(("a", "bins", "range", "density", "weights")):
            if index < len(args):
                if name in options:
                    return NotImplemented
                parameters[name] = args[index]
        ...
        if isinstance(bins, str):
            return NotImplemented
        if _is_torch_array_data(bins) or isinstance(bins, torch.Tensor):
            explicit = True
        else:
            try:
        ...
        try:
            if explicit:
                edges, edge_dtype = self._numpy_histogram_explicit_edges(bins)
            else:
        ...
        if bin_count == 0:
            histogram = torch.zeros((0,), dtype=histogram_torch, device=tensor.device)
        else:
            try:
                comparison_dtype = np.dtype(np.result_type(input_dtype, edge_dtype))
                ...
                valid = (values >= boundaries[0]) & (values <= boundaries[-1])
                indices = torch.searchsorted(boundaries, values, right=True) - 1
                indices = torch.where(
                    values == boundaries[-1],
                ...
                if weights is None:
                    ...
                    execution_dtype = (
                        torch.int64 if histogram_dtype.kind == "u" else histogram_torch
                    )
        ...
        if density:
            density_torch = (
                torch.float32 if tensor.device.type == "mps" else torch.float64
            )
    ...
    def _numpy_digitize(self, args, kwargs):
        ...
        try:
            ...
            try:
                ...
                device_dtype = bins_dtype if bins_tensor is not None else x_dtype
            ...
            def materialize(tensor, host):
                ...
                return torch.as_tensor(host, dtype=target_torch, device=device)
        ...
        if bins.dtype == torch.bool or bins.dtype in (_TORCH_UINT16, _TORCH_UINT32):
        ...
        try:
            if bins.numel() < 2:
                ...
                increasing = not bool(torch.any(bins[:-1] > bins[1:]).item())
                decreasing = not bool(torch.any(bins[:-1] < bins[1:]).item())
        ...
        try:
            ...
            if parameters["x"] is None:
                ...
                distance = torch.as_tensor(
                    parameters["dx"],
                    dtype=_ensure_supported(device, _torch_dtype(product_dtype)),
                    device=device,
                )
            else:
                x_tensor, x_host, x_dtype = describe(parameters["x"])
                ...
                if x.ndim == 1:
                    ...
                    if x.dtype == torch.bool:
                        ...
                        distance = x[tuple(x_upper)] - x[tuple(x_lower)]
                product_dtype = np.dtype(
                    np.result_type(
            ...
            product_torch = _ensure_supported(device, _torch_dtype(product_dtype))
            ...
            output_torch = _ensure_supported(device, _torch_dtype(output_dtype))
            result = torch.sum(
    ...
    def _numpy_interp(self, args, kwargs):
        ...
        try:
            x_tensor, x_host, _ = describe(parameters["x"], complex_ok=False)
            xp_tensor, xp_host, _ = describe(parameters["xp"], complex_ok=False)
            fp_tensor, fp_host, fp_dtype = describe(parameters["fp"], complex_ok=True)
            output_np = np.dtype(np.complex128 if fp_dtype.kind == "c" else np.float64)
            output_torch = _ensure_supported(device, _torch_dtype(output_np))
        ...
        if period is not None:
            ...
            if xp.numel() > 1 and bool(torch.any(xp[1:] == xp[:-1]).item()):
                return NotImplemented
            xp = torch.cat(
                (
                    xp[-1:] - period_value,
            ...
            def boundary(value, default):
                if value is None:
                    return default
                tensor, host, dtype = describe(value, complex_ok=output_np.kind == "c")
        ...
        if xp.numel() == 1:
            ...
            try:
                ...
                result = torch.where(flat_x == x_lower, fp[lower], result)
        ...
        if xp.numel() > 1:
            nan_value = torch.full((), float("nan"), dtype=output_torch, device=device)
    ...
    def _numpy_set_operand(self, value):
        ...
        return self._wrap(tensor), dtype
    ...
    def _numpy_sorted_unique_values(self, tensor, dtype):
        ...
        if ordered.numel():
            ...
            if dtype.kind == "f":
                duplicates |= torch.isnan(ordered[1:]) & torch.isnan(ordered[:-1])
            keep = torch.ones(
                ordered.shape,
                dtype=torch.bool,
        ...
        if dtype.kind == "f" and values.numel():
            has_negative_zero = torch.any((flattened == 0) & torch.signbit(flattened))
            values = torch.where(
    ...
    def _numpy_union1d(self, args, kwargs):
        ...
        try:
            left_data, left_dtype = self._numpy_set_operand(parameters["ar1"])
            right_data, right_dtype = self._numpy_set_operand(parameters["ar2"])
            target_dtype = np.dtype(np.result_type(left_dtype, right_dtype))
            target_torch = _ensure_supported(
                self.tensor.device, _torch_dtype(target_dtype)
            )
            combined = torch.cat(
            ...
            values = self._numpy_sorted_unique_values(combined, target_dtype)
        except (OverflowError, TypeError, ValueError, RuntimeError):
            return NotImplemented
    ...
    def _numpy_setdiff1d(self, args, kwargs):
        ...
        try:
            ...
            if assume_unique:
                ...
                left = self._numpy_sorted_unique_values(left_data.tensor, left_dtype)
                right = self._numpy_sorted_unique_values(right_data.tensor, right_dtype)
            ...
            if mask is NotImplemented:
                return NotImplemented
            indices = torch.nonzero(mask.tensor, as_tuple=False).reshape(-1)
            execution = (
                left.to(dtype=torch.int64) if left.dtype == _TORCH_UINT32 else left
            )
        ...
        try:
            ...
            if ordered.numel():
                ...
                boundaries = torch.cat(
                    (
                        torch.ones((1,), dtype=torch.bool, device=ordered.device),
                        different,
                        torch.ones((1,), dtype=torch.bool, device=ordered.device),
                    )
                )
    ...
    def _numpy_intersect1d(self, args, kwargs):
        ...
        parameters = {}
        for index, name in enumerate(("ar1", "ar2", "assume_unique", "return_indices")):
            if index < len(args):
                if name in options:
                    return NotImplemented
                parameters[name] = args[index]
        ...
        try:
            ...
            for data, dtype in ((left_data, left_dtype), (right_data, right_dtype)):
                if dtype.kind == "f":
            ...
            if flags["assume_unique"]:
                ...
                if left_unique is NotImplemented or right_unique is NotImplemented:
                    return NotImplemented
    ...
    def _numpy_isin(self, args, kwargs):
        ...
        if (
            not _is_torch_array_data(element)
        ...
        test_values = None
        if _is_torch_array_data(test_elements):
            if (
        ...
        if kind == "table" and not all(
            dtype.kind in "biu" for dtype in (np.dtype(element.dtype), test_dtype)
        ):
            raise ValueError(
        ...
        try:
            ...
            if result is None:
                ...
                for start in range(0, flattened_test.numel(), chunk_size):
                    ...
                    matched |= torch.any(flattened[:, None] == chunk[None, :], dim=1)
    ...
    def _numpy_sort(self, args, kwargs):
        ...
        try:
            result = torch.sort(tensor, dim=dimension, stable=stable_sort).values
        except (TypeError, ValueError, RuntimeError):
            return NotImplemented
        ...
        if (
            ...
            if size:
                ...
                source_positions = torch.where(tensor == 0, positions, size)
                source_positions = torch.sort(source_positions, dim=dimension).values
                source_positions = source_positions.clamp_max(size - 1)
                source_zeros = torch.gather(tensor, dimension, source_positions)
                result_zero = result == 0
                zero_rank = torch.cumsum(result_zero.to(torch.int64), dim=dimension) - 1
    ...
    def _numpy_dot_product(self, function, args, kwargs):
        ...
        try:
            if function is np.outer:
                result = left_tensor.reshape(-1, 1) * right_tensor.reshape(1, -1)
                ...
                if target_np.kind in "bu":
                    result = contraction(left_flat, right_flat, ([0], [0]))
                else:
    ...
    def _numpy_linalg_norm(self, args, kwargs):
        ...
        if mode == "matrix":
            if order is not None and not (
                isinstance(order, (str, np.str_)) and order in ("fro", "f")
            ):
                return NotImplemented
            vector_order = 2.0
        elif order is None:
        ...
        try:
            output_torch = _ensure_supported(tensor.device, _torch_dtype(output_dtype))
            execution = (
                tensor if input_dtype.kind == "c" else tensor.to(dtype=output_torch)
            )
            ...
            if vector_order == np.inf and (
                execution.numel() == 0
                and (dimensions is None or execution.shape[dimensions] == 0)
            ):
                ...
                result = torch.zeros(shape, dtype=output_torch, device=tensor.device)
            else:
    ...
    def numpy_array_function(self, function, *args, **kwargs):
        ...
        if function in (np.empty_like, np.zeros_like, np.ones_like, np.full_like):
        ...
        if not all(_is_torch_array_data(array) for array in arrays):
            return NotImplemented
        if not all(
            _device_matches_active(array.tensor)
            and array.tensor.device == self.tensor.device
        ...
        try:
            ...
            tensors = [array.tensor.to(dtype=target_torch) for array in arrays]
    ...
    def numpy_ufunc(self, ufunc, method, *inputs, **kwargs):
        ...
        if method == "accumulate":
            ...
            if out is not None and not (
                isinstance(out, tuple) and len(out) == 1 and out[0] is None
            ):
                return NotImplemented
            dtype = options.pop("dtype", None)
            if options:
            ...
            if operation is None or np.dtype(self.dtype).kind not in "fc":
                return NotImplemented
            try:
                target_dtype = np.dtype(self.dtype if dtype is None else dtype)
                if target_dtype.kind not in "fc":
                    return NotImplemented
                target_torch = _ensure_supported(
                    self.tensor.device, _torch_dtype(target_dtype)
                ...
                return NotImplemented
            if ufunc in _TORCH_REAL_ACCUMULATE_NUMPY_UFUNCS and tensor.is_complex():
                return NotImplemented
        ...
        if method == "reduce":
            ...
            logical_reduction = ufunc in _TORCH_LOGICAL_REDUCE_NUMPY_UFUNCS
            if logical_reduction:
                ...
                if source_dtype.kind not in "biufc":
                    return NotImplemented
                if dtype is None:
                    if ufunc in (np.add, np.multiply) and source_dtype.kind in "bi":
                        target_dtype = np.dtype(np.int64)
                    elif ufunc in (np.add, np.multiply) and source_dtype.kind == "u":
                        return NotImplemented
                    else:
                ...
                if target_dtype.kind not in "biufc":
                    return NotImplemented
                if ufunc in _TORCH_REAL_REDUCE_NUMPY_UFUNCS and (
                    source_dtype.kind == "c" or target_dtype.kind == "c"
                ):
                    return NotImplemented
                try:
                    target_torch = _ensure_supported(
            ...
            if ufunc in _TORCH_REAL_REDUCE_NUMPY_UFUNCS and tensor.is_complex():
                return NotImplemented
            output_shape = _reduction_output_shape(tuple(tensor.shape), axes, keepdims)
            ...
            try:
                if logical_reduction and reduction_size == 0:
                    reduced = torch.full(
                        output_shape,
                ...
                elif ufunc in _TORCH_REAL_REDUCE_NUMPY_UFUNCS and output_size == 0:
                    reduced = torch.empty(
                        output_shape,
                        dtype=target_torch,
                        device=tensor.device,
                    ...
                    reduced = torch.sum(tensor, dim=axes, keepdim=keepdims)
                elif ufunc is np.multiply:
                    reduced = tensor
                    dimensions = sorted(axes, reverse=not keepdims)
                    for dimension in dimensions:
                        reduced = torch.prod(reduced, dim=dimension, keepdim=keepdims)
                elif ufunc is np.maximum:
                    reducer = torch.any if tensor.dtype == torch.bool else torch.amax
                    reduced = reducer(tensor, dim=axes, keepdim=keepdims)
                elif ufunc is np.minimum:
                    reducer = torch.all if tensor.dtype == torch.bool else torch.amin
                    reduced = reducer(tensor, dim=axes, keepdim=keepdims)
                elif ufunc is np.logical_or:
                    reduced = torch.any(tensor, dim=axes, keepdim=keepdims)
                else:
                    reduced = torch.all(tensor, dim=axes, keepdim=keepdims)
        ...
        if len(inputs) == 1:
            operand = inputs[0]
            if not _is_torch_array_data(operand):
                return NotImplemented
            ...
            supported_kinds = (
                ...
                else ("biu" if ufunc in _TORCH_BITWISE_UNARY_NUMPY_UFUNCS else "fc")
            )
            if np.dtype(operand.dtype).kind not in supported_kinds:
                return NotImplemented
            if ufunc in _TORCH_REAL_UNARY_NUMPY_UFUNCS and operand.tensor.is_complex():
                return NotImplemented
            tensor = operand.tensor
            if (
                ...
                return operand._wrap(operation(tensor.to(torch.int64)).to(tensor.dtype))
            return operand._wrap(operation(tensor))
        if len(inputs) == 2:
            comparison = _TORCH_COMPARISON_NUMPY_UFUNCS.get(ufunc)
            if comparison is not None:
                result = _comparison_ufunc_result(inputs, self, comparison)
            ...
            if ufunc in _TORCH_LOGICAL_BINARY_NUMPY_UFUNCS:
                ...
                return self._wrap(operation(*tensors).to(dtype=output_dtype))
            else:
                tensors = _binary_ufunc_tensors(inputs, self)
            if tensors is None:
                return NotImplemented
            if ufunc in _TORCH_REAL_BINARY_NUMPY_UFUNCS and any(
                tensor.is_complex() for tensor in tensors
            ):
                return NotImplemented
...
# Internal code uses the shorter name to keep the protocol checks readable.
# The descriptive alias is what the stable backend entry point imports.
TorchArrayNumpyCompatibilityMixin = _NumpyMixin
