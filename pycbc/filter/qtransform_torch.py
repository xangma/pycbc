...
def _qseries_batch(fseries, Q, frequencies, return_complex=False):
    ...
    for chunk_start in range(0, len(frequencies), row_limit):
        ...
        for row, f0 in enumerate(chunk):
            ...
            center = (start + end) // 2
            windowed = (
                fseries._data.tensor[start:end].to(dtype=complex_dtype)
                * weight
            )
            ...
            if center < window_size:
                ...
                padded[
                    row, destination:destination + window_size
                ] = windowed
        ...
        del (
            padded, transformed, energy, median_energy, normalized
        )
...
def qseries(fseries, Q, f0, return_complex=False):
    ...
    ctseries = TimeSeries(zeros(tlen, dtype=output_dtype),
...
def qplane(qplane_tile_dict, fseries, return_complex=False):
    ...
    for i, q in enumerate(qplane_tile_dict):
        ...
        qmaxima.append(
            tile_maxima[-1] if i == 0 else tile_maxima.amax()
        )
    ...
    coordinate_dtype = torch.float32 if device.type == "mps" \
        else torch.float64
    times = torch.arange(
        plane_tensor.shape[-1], device=device, dtype=coordinate_dtype
    )
