...
def _cpu_native_dispatch_is_beneficial(length, openmp_runtime):
    ...
    return (
        ...
        and _CPU_NATIVE_MIN_THREADS <= openmp_threads <= _CPU_NATIVE_MAX_THREADS
        and length >= openmp_threads * _CPU_NATIVE_ELEMENTS_PER_THREAD
    )
...
class _CPUNativeBatchCorrelationState:
    ...
    def __init__(self, batch, matchedfilter_cpu, openmp_runtime):
        ...
        self._x_pointer_table = np.asarray(x_pointers, dtype=pointer_dtype)
        self._z_pointer_table = np.asarray(z_pointers, dtype=pointer_dtype)
    ...
    def can_execute(self, batch):
        try:
            ...
            if (
                ...
                or not _cpu_native_batch_runtime_is_stable(self._openmp_runtime)
            ):
                return False
            tensors = (
                *self._x_tensors,
            ...
            return all(
                _batch_tensor_contract(tensor, self._size) for tensor in tensors
    ...
    def execute(self, batch, y):
        ...
        try:
            ...
            if y_tensor is None or not _batch_tensor_contract(y_tensor, self._size):
                return False
...
class _CUDANativeBatchCorrelationState:
    ...
    def __init__(self, batch):
        ...
        if any(t.device != device for t in tensors):
            raise ValueError(
                "CUDA batch correlation tensors must be on the same device"
            )
        ...
        if z_stride is None:
            raise ValueError(
                "CUDA batch correlation output tensors are not uniformly strided"
            )
    ...
    def can_execute(self, batch):
        try:
            ...
            if not _batch_outputs_are_disjoint(
                ...
                return False
            if (
                not self._is_stacked_x
                and _find_uniform_stride(self._x_tensors, self._size) != self._x_stride
            ):
                return False
    ...
    def execute(self, batch, y):
        try:
            ...
            y_sub = (
                y_tensor if y_tensor.numel() == self._size else y_tensor[: self._size]
            )
...
def correlate(x, y, z):
    ...
    torch.mul(torch.conj(x._data.tensor), y._data.tensor, out=z._data.tensor)
...
class TorchCorrelator(_BaseCorrelator):
    def __init__(self, x, y, z):
        ...
        self._cpu_native_static = _cpu_native_static_eligible(self.x, self.y, self.z)
        self._cpu_native = None
        if self._cpu_native_static:
            from . import matchedfilter_cpu

            openmp_runtime = _cpu_native_openmp_runtime(matchedfilter_cpu)
            if _cpu_native_dispatch_is_beneficial(self.x.numel(), openmp_runtime):
...
def batch_correlate_execute(self, y):
    ...
    if num_vectors > 0 and len(self.zs) == num_vectors:
        try:
            x_tensors = tuple(
                getattr(getattr(x, "_data", None), "tensor", None) for x in self.xs
            )
            z_tensors = tuple(
                getattr(getattr(z, "_data", None), "tensor", None) for z in self.zs
            )
            all_tensors = (*x_tensors, *z_tensors)
            if all(isinstance(t, torch.Tensor) for t in all_tensors):
                target_device = y_sub.device
                if not _has_dynamic_autograd_state((*all_tensors, y_sub)) and all(
    ...
    for x, z in zip(self.xs, self.zs, strict=False):
