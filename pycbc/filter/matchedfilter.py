...
def _torch_cpu_native_batch_peak_enabled():
    ...
    if normalized in _TORCH_CPU_NATIVE_BATCH_PEAK_FALSE:
        return False
    choices = ", ".join(
        sorted(_TORCH_CPU_NATIVE_BATCH_PEAK_TRUE | _TORCH_CPU_NATIVE_BATCH_PEAK_FALSE)
    )
    raise ValueError(
...
def _bracket_minimum(function, xa, xb, grow_limit=110.0, max_iterations=1000):
    ...
    while fc < fb:
        ...
        w = xb - ((xb - xc) * tmp2 - (xb - xa) * tmp1) / denominator
    ...
    bracket_has_minimum = (fb < fc and fb <= fa) or (fb < fa and fb <= fc)
    bracket_is_ordered = xa < xb < xc or xc < xb < xa
    bracket_is_finite = numpy.isfinite(xa) and numpy.isfinite(xb) and numpy.isfinite(xc)
    if not (bracket_has_minimum and bracket_is_ordered and bracket_is_finite):
        error = _BracketError(
...
def _brent_minimum(function, bracket, tolerance=1.48e-8, max_iterations=500):
    """Minimize a scalar function with SciPy-compatible Brent updates."""
    try:
        xa, xb, xc, _, fb, _ = _bracket_minimum(function, bracket[0], bracket[1])
    ...
    for _ in range(max_iterations):
        ...
        if numpy.abs(x - midpoint) < (tolerance_2 - 0.5 * (upper - lower)):
            break
        ...
        if numpy.abs(delta_x) <= tolerance_1:
            ...
            if (
                ...
                and numpy.abs(parabola) < numpy.abs(0.5 * tmp2 * previous_delta_x)
            ):
                step = parabola / tmp2
                candidate = x + step
                if candidate - lower < tolerance_2 or upper - candidate < tolerance_2:
                    if midpoint - x >= 0:
                        step = tolerance_1
                    else:
                        step = -tolerance_1
...
def optimized_match(
    ...
    time_shift = _brent_minimum(to_minimize, bracket=(-delta_t, delta_t))
