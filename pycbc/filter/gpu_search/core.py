...
def correlate_and_ifft(
    ...
    if torch is not None and isinstance(cout_workspace, torch.Tensor):
        ...
        def _execute_torch():
            # Clear only inactive bands rather than the entire workspace buffer.
            # Active bins [k_start:k_end] are overwritten by torch.mul below.
            if k_start > 0:
                active_cout[:, :k_start].zero_()
            if k_end < tlen_val:
