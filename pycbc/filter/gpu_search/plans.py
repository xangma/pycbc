...
class WorkspaceBudget:
    ...
    def calculate_workspace_bytes(
        ...
        Correlation array: batch_size * filter_length * dtype_bytes
        Time-domain SNR / IFFT output: batch_size * transform_length * dtype_bytes
        Candidate SoA scratch: ~64 bytes per slot * capacity
        """
        corr_bytes = batch_size * filter_length * dtype_bytes
        snr_bytes = batch_size * transform_length * dtype_bytes
        overwhitened_bytes = filter_length * dtype_bytes
        # 16 * B * N approximation for complex64 correlation and SNR arrays
        return corr_bytes + snr_bytes + overwhitened_bytes
    ...
    def recommend_batch_size(
        ...
        bytes_per_template = (filter_length + transform_length) * 8
...
def prepare_bank(
    ...
    for tile_idx, start_idx in enumerate(range(0, num_templates, tile_size)):
        ...
        t_hash = _compute_hash(tile_data_np.tobytes())
    ...
    composite_hash = _compute_hash("".join(tile_hashes).encode("ascii"))
