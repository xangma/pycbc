        ...
        if torch is not None and self.device != "numpy":
            ...
            if self.use_cluster and self.cluster_function == "symmetric":
                ...
                if len(t_indices) == 0:
                    ...
                    empty_c64 = np.empty(0, dtype=np.complex64)
                    return [
                        ([], float(norms_np[i]), [], empty_uint32, empty_c64)
                        for i in range(b)
                    ]

                cand_by_template = collections.defaultdict(list)
                for cand_pos, tmpl_id in enumerate(t_indices):
                    cand_by_template[int(tmpl_id)].append(cand_pos)
                ...
                for i in range(b):
                    ...
                    cand_pos = cand_by_template.get(i)
                    if not cand_pos:
                        results.append((
                            [],
                            norm_i,
                        ...
                        pos_arr = np.array(cand_pos, dtype=np.int64)
                        tmpl_idx_shifted = s_indices[pos_arr]
                        tmpl_idx = (tmpl_idx_shifted - valid_start).astype(
                            np.uint32
                        )
                        tmpl_snrv = (snr_vals[pos_arr] / norm_i).astype(
                            np.complex64
                        )
