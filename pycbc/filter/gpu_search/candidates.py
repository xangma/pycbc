...
class CandidateBuffer:
    ...
    def __init__(
        ...
        self.max_capacity = (
            int(max_capacity) if max_capacity is not None else None
        )
        self._allocate()
...
def _store_in_buffer(
    ...
    """Store survivor batch in buffer with optional dynamic growth on overflow."""
    if buffer is None:
        return False
    start_c = buffer.count
    if start_c + num_survivors > buffer.capacity:
        if getattr(buffer, "allow_growth", False) and (
            buffer.max_capacity is None
            or start_c + num_survivors <= buffer.max_capacity
        ):
            buffer.resize(start_c + num_survivors)
        else:
...
def _select_torch_threshold_only(
    vals: torch.Tensor,
    norms: torch.Tensor,
    sigmasqs: torch.Tensor,
    valid_start: int,
    ...
    """Execute threshold-only candidate selection on device, returning all points above threshold."""
    sq_mag = vals.real.square() + vals.imag.square()
    sq_mag = torch.nan_to_num(sq_mag, nan=0.0)

    safe_norms = torch.clamp(norms, min=1e-12)
    thresh_sq = (policy.snr_threshold / safe_norms).square().unsqueeze(-1)
    keep = sq_mag >= thresh_sq

    if policy.snr_abort_threshold is not None:
        abort_thresh_sq = (
            (policy.snr_abort_threshold / safe_norms).square().unsqueeze(-1)
        )
        if (sq_mag >= abort_thresh_sq).any():
            return {"aborted": True, "candidates": {}}

    survivor_indices = torch.nonzero(keep, as_tuple=True)
    sel_tmplt = survivor_indices[0]
    sel_sample_rel = survivor_indices[1]
    num_survivors = int(sel_tmplt.numel())
...
def _select_numpy_threshold_only(
    vals: np.ndarray,
    norms: np.ndarray,
    sigmasqs: np.ndarray,
    valid_start: int,
    ...
    """Execute threshold-only candidate selection with NumPy, returning all points above threshold."""
    sq_mag = vals.real**2 + vals.imag**2
    sq_mag = np.nan_to_num(sq_mag, nan=0.0)

    safe_norms = np.maximum(norms, 1e-12)
    thresh_sq = ((policy.snr_threshold / safe_norms) ** 2)[:, np.newaxis]
    keep = sq_mag >= thresh_sq

    if policy.snr_abort_threshold is not None:
        abort_thresh_sq = ((policy.snr_abort_threshold / safe_norms) ** 2)[:, np.newaxis]
        if (sq_mag >= abort_thresh_sq).any():
            return {"aborted": True, "candidates": {}}
...
def select_tile_candidates(
    ...
    if torch is not None and isinstance(out_mem, torch.Tensor):
        ...
        elif policy.cluster_policy == "threshold_only":
            return _select_torch_threshold_only(
                vals, norms, sigmasqs, valid_start, policy, buffer
            )
    ...
    if policy.cluster_policy == "symmetric":
        ...
        return _select_numpy_threshold_only(
            out_np, norms, sigmasqs, valid_start, policy, buffer
        )
