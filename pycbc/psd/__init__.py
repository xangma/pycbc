...
def from_cli_single_ifo(opt, length, delta_f, low_frequency_cutoff, ifo, **kwargs):
    """
    Get the PSD for a single ifo when using the multi-detector CLI
    """
    single_det_opt = copy_opts_for_single_ifo(opt, ifo)
    return from_cli(single_det_opt, length, delta_f, low_frequency_cutoff, **kwargs)


def from_cli_multi_ifos(
    opt,
    length_dict,
    delta_f_dict,
    ...
    **kwargs,
    ...
    for ifo in ifos:
        if strain_dict is not None:
            ...
            strain = None
        psd[ifo] = from_cli_single_ifo(
            opt,
            length_dict[ifo],
            delta_f_dict[ifo],
            low_frequency_cutoff_dict[ifo],
            ifo,
            strain=strain,
            **kwargs,
        )
...
def insert_psd_option_group(parser, output=True, include_data_options=True):
    ...
    psd_options.add_argument("--psd-file", help="Get PSD using given PSD ASCII file")
    psd_options.add_argument("--asd-file", help="Get PSD using given ASD ASCII file")
    psd_options.add_argument(
        "--psd-inverse-length",
        type=float,
    ...
    psd_options.add_argument(
        "--invpsd-trunc-low-freq-fill-value",
        default=0.0,
    ...
    psd_options.add_argument(
        ...
        type=float,
        metavar="SECONDS",
        help="Length of segment for mean square calculation of PSD variation.",
    )
    psd_options.add_argument(
        "--psdvar-short-segment",
        type=float,
        metavar="SECONDS",
        help="Length of short segment "
    ...
    psd_options.add_argument(
        ...
        type=float,
        metavar="SECONDS",
        help="Length of long segment when calculating the PSD variability.",
    )
    psd_options.add_argument(
        "--psdvar-psd-duration",
        type=float,
        metavar="SECONDS",
        help="Duration of short segments for PSD estimation.",
    )
    psd_options.add_argument(
        "--psdvar-psd-stride",
        type=float,
        metavar="SECONDS",
        help="Separation between PSD estimation segments.",
    )
    psd_options.add_argument(
        "--psdvar-low-freq",
        type=float,
        metavar="HERTZ",
        help="Minimum frequency to consider in strain bandpass.",
    )
    psd_options.add_argument(
        "--psdvar-high-freq",
        type=float,
        metavar="HERTZ",
        help="Maximum frequency to consider in strain bandpass.",
    )
    ...
    if output:
        psd_options.add_argument(
            "--psd-output", help="(Optional) Write PSD to specified file"
        )
...
def insert_psd_option_group_multi_ifo(parser):
    ...
    psd_options.add_argument(
        ...
        type=float,
        metavar="SECONDS",
        help="Length of segment when calculating the PSD variability.",
    )
    psd_options.add_argument(
        "--psdvar-short-segment",
        type=float,
...
ensure_one_opt_groups.append(
    ["--psd-file", "--psd-model", "--psd-estimation", "--asd-file"]
)
...
def generate_overlapping_psds(
    opt, gwstrain, flen, delta_f, flow, dyn_range_factor=1.0, precision=None
):
...
def associate_psds_to_single_ifo_segments(
    opt,
    fd_segments,
    gwstrain,
    flen,
