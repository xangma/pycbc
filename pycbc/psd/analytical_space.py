...
def _analytical_psd_tdi_XYZ(
    length, delta_f, low_freq_cutoff, len_arm=None, psd_components=None, tdi=None
):
...
def analytical_psd_lisa_tdi_XYZ(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=2.5e9,
    ...
    fseries = _analytical_psd_tdi_XYZ(
        length, delta_f, low_freq_cutoff, len_arm, psd_components, tdi
    )

    return fseries


def analytical_psd_tianqin_tdi_XYZ(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=np.sqrt(3) * 1e8,  # noqa: B008 - immutable physical default
    ...
    fr = np.linspace(low_freq_cutoff, (length - 1) * 2 * delta_f, length)
    psd_components = np.array(
        tianqin_psd_components(fr, acc_noise_level, oms_noise_level)
    )
    fseries = _analytical_psd_tdi_XYZ(
        length, delta_f, low_freq_cutoff, len_arm, psd_components, tdi
    )
...
def analytical_psd_taiji_tdi_XYZ(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=3e9,
    ...
    fr = np.linspace(low_freq_cutoff, (length - 1) * 2 * delta_f, length)
    psd_components = np.array(
        taiji_psd_components(fr, acc_noise_level, oms_noise_level)
    )
    fseries = _analytical_psd_tdi_XYZ(
        length, delta_f, low_freq_cutoff, len_arm, psd_components, tdi
    )
...
def analytical_csd_lisa_tdi_XY(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=2.5e9,
    ...
    fr = np.linspace(low_freq_cutoff, (length - 1) * 2 * delta_f, length)
    psd_components = np.array(lisa_psd_components(fr, acc_noise_level, oms_noise_level))
    fseries = _analytical_csd_tdi_XY(
        length, delta_f, low_freq_cutoff, len_arm, psd_components, tdi
    )
...
def analytical_psd_lisa_tdi_AE(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=2.5e9,
    ...
    fr = np.linspace(low_freq_cutoff, (length - 1) * 2 * delta_f, length)
    psd_components = np.array(lisa_psd_components(fr, acc_noise_level, oms_noise_level))
    fseries = _analytical_psd_tdi_AE(
        length, delta_f, low_freq_cutoff, len_arm, psd_components, tdi
    )
...
def analytical_psd_tianqin_tdi_AE(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=np.sqrt(3) * 1e8,  # noqa: B008 - immutable physical default
...
def analytical_psd_taiji_tdi_AE(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=3e9,
...
def analytical_psd_tianqin_tdi_T(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=np.sqrt(3) * 1e8,  # noqa: B008 - immutable physical default
...
def analytical_psd_taiji_tdi_T(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=3e9,
...
def averaged_response_tianqin_tdi(
    f,
    len_arm=np.sqrt(3) * 1e8,  # noqa: B008 - immutable physical default
    tdi=None,
):
...
def sensitivity_curve_lisa_semi_analytical(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=2.5e9,
    ...
    fseries = from_numpy_arrays(fr, sense_curve / 2, length, delta_f, low_freq_cutoff)
...
def sensitivity_curve_lisa_SciRD(length, delta_f, low_freq_cutoff):
    ...
    fseries = from_numpy_arrays(fr, sense_curve, length, delta_f, low_freq_cutoff)
...
def confusion_fit_lisa(length, delta_f, low_freq_cutoff, duration=1.0):
    ...
    fseries = from_numpy_arrays(fr, sh_confusion, length, delta_f, low_freq_cutoff)
...
def confusion_fit_tianqin(length, delta_f, low_freq_cutoff, duration=1.0):
    ...
    sh_confusion[(fr > 3e-4) & (fr < 5e-4)] = sh_confusion[(np.abs(fr - 5e-4)).argmin()]
...
def sensitivity_curve_lisa_confusion(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=2.5e9,
    ...
    if base_model == "semi":
        base_curve = sensitivity_curve_lisa_semi_analytical(
            length, delta_f, low_freq_cutoff, len_arm, acc_noise_level, oms_noise_level
        )
    elif base_model == "SciRD":
        base_curve = sensitivity_curve_lisa_SciRD(length, delta_f, low_freq_cutoff)
    else:
        raise ValueError("Must choose from 'semi' or 'SciRD'.")
    if duration < 0 or duration > 10:
        raise ValueError("Must between 0 and 10.")
    fseries_confusion = confusion_fit_lisa(length, delta_f, low_freq_cutoff, duration)
    fseries = from_numpy_arrays(
        base_curve.sample_frequencies,
...
def sensitivity_curve_taiji_confusion(
    ...
    fseries_confusion = confusion_fit_taiji(length, delta_f, low_freq_cutoff, duration)
    fseries = from_numpy_arrays(
        base_curve.sample_frequencies,
        base_curve + fseries_confusion,
        length,
...
def sh_transformed_psd_lisa_tdi_XYZ(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=2.5e9,
    ...
    if str(tdi) in ["1.5", "2.0"]:
        ...
        raise ValueError("The version of TDI, currently only for 1.5 or 2.0.")
    fseries_response = from_numpy_arrays(
        fr, np.array(response), length, delta_f, low_freq_cutoff
    )
    sh = sensitivity_curve_lisa_confusion(
        length,
        delta_f,
        low_freq_cutoff,
        len_arm,
    ...
    fseries = from_numpy_arrays(
        sh.sample_frequencies, psd, length, delta_f, low_freq_cutoff
    )

    return fseries


def semi_analytical_psd_lisa_confusion_noise(
    length, delta_f, low_freq_cutoff, len_arm=2.5e9, duration=1.0, tdi=None
):
    ...
    fseries = from_numpy_arrays(
        fseries_confusion.sample_frequencies,
        psd_confusion,
        length,
        delta_f,
        low_freq_cutoff,
...
def analytical_psd_tianqin_confusion_noise(
    length,
    delta_f,
    low_freq_cutoff,
    len_arm=np.sqrt(3) * 1e8,  # noqa: B008 - immutable physical default
...
def analytical_psd_taiji_confusion_noise(
    length, delta_f, low_freq_cutoff, len_arm=3e9, duration=1.0, tdi=None
):
...
def analytical_psd_lisa_tdi_AE_confusion(
    ...
    psd_AE = analytical_psd_lisa_tdi_AE(
        length, delta_f, low_freq_cutoff, len_arm, acc_noise_level, oms_noise_level, tdi
    )
    psd_X_confusion = semi_analytical_psd_lisa_confusion_noise(
        length, delta_f, low_freq_cutoff, len_arm, duration, tdi
    )
...
def analytical_psd_tianqin_tdi_AE_confusion(
    ...
    psd_AE = analytical_psd_tianqin_tdi_AE(
        length, delta_f, low_freq_cutoff, len_arm, acc_noise_level, oms_noise_level, tdi
    )
...
def analytical_psd_taiji_tdi_AE_confusion(
    ...
    psd_AE = analytical_psd_taiji_tdi_AE(
        length, delta_f, low_freq_cutoff, len_arm, acc_noise_level, oms_noise_level, tdi
    )
