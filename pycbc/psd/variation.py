...
def _torch_bandpass_response(reference, sample_rate, low_freq, high_freq,
                             psd_duration):
    ...
    positions = (
        torch.arange(num_taps, dtype=work_dtype, device=device) - alpha
    )
    left = low_freq / nyquist
    right = high_freq / nyquist
    coefficients = (
        right * torch.sinc(right * positions)
        - left * torch.sinc(left * positions)
    )
    ...
    scale = torch.sum(
        coefficients
        * torch.cos(torch.pi * positions * scale_frequency)
    )
    coefficients /= scale

    padded = torch.zeros(
        response_length, dtype=work_dtype, device=device
    )
...
def calc_filt_psd_variation(strain, segment, short_segment, psd_long_segment,
    ...
    if use_torch:
        ...
        filt = sig.firwin(
            4 * srate, [low_freq, high_freq], pass_zero=False,
