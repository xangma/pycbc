...
class _TorchModuleProxy:
    def __getattr__(self, name):
        ...
        is_compiling = getattr(
            getattr(mod, "compiler", None), "is_compiling", lambda: False
        )
        if not is_compiling():
            globals()["torch"] = mod
...
_P1200087_DATA_FILES = {
    "aLIGOEarlyLowSensitivityP1200087": ("LIGO-P1200087-v18-aLIGO_EARLY_LOW.txt"),
    "aLIGOEarlyHighSensitivityP1200087": ("LIGO-P1200087-v18-aLIGO_EARLY_HIGH.txt"),
    ...
    "aLIGOLateHighSensitivityP1200087": ("LIGO-P1200087-v18-aLIGO_LATE_HIGH.txt"),
    ...
    "AdVBNSOptimizedSensitivityP1200087": ("LIGO-P1200087-v18-AdV_BNS_OPTIMIZED.txt"),
}
_VERSIONED_DATA_FILES = {
    **_P1200087_DATA_FILES,
    ...
    "CosmicExplorerPessimisticP1600143": ("LIGO-P1600143-v18-CE_Pessimistic.txt"),
    ...
    "aLIGOAdVO4IntermediateT1800545": ("LIGO-T1800545-v1-AdV_O4intermediate.txt"),
...
def _iligo_srd(frequencies):
    ...
    seismic = (
        seismic_amplitude * seismic_amplitude * frequencies.pow(2.0 * seismic_exponent)
    )
    thermal = (
        thermal_amplitude * thermal_amplitude * frequencies.pow(2.0 * thermal_exponent)
    )
    shot = (
        shot_amplitude
        * shot_amplitude
...
def _iligo_thermal(frequencies):
    ...
    suspension = suspension_scale * (_ILIGO_SUSPENSION_FREQUENCY / frequencies).pow(5.0)
...
def _first_generation_shot(frequencies, efficiency):
    ...
    storage_time = _ILIGO_ARM_LENGTH * _ILIGO_FINESSE / (lal.PI * lal.C_SI)
    ...
    dc_noise = (
        ...
        * pole_frequency
        * pole_frequency
        / (lal.C_SI * efficiency * _ILIGO_LASER_POWER_BS)
    )
    sensing = (
        torch.exp(2.0 * lal.PI * 1j * frequencies * _ILIGO_ARM_LENGTH / lal.C_SI)
        * math.sinh(2.0 * lal.PI * pole_frequency * _ILIGO_ARM_LENGTH / lal.C_SI)
        / torch.sinh(
            (2.0 * lal.PI * pole_frequency * _ILIGO_ARM_LENGTH / lal.C_SI)
...
def _ground_detector_fit(psd_name, frequencies):
    ...
    if psd_name == "Virgo":
        x = frequencies / 500.0
        return 10.2e-46 * ((7.87 * x).pow(-4.8) + (6.0 / 17.0) / x + 1.0 + x.square())
    if psd_name == "GEO":
        x = frequencies / 150.0
        x2 = x.square()
    ...
    if psd_name == "TAMA":
        x = frequencies / 400.0
        return 75e-46 * (x.pow(-5.0) + 13.0 / x + 9.0 * (1.0 + x.square()))
    if psd_name == "KAGRA":
...
def _aligo_quantum(
    ...
    sql_power = (
        _MIRROR_MASS * _ARM_LENGTH * _ARM_LENGTH * gamma_ac**4 / (4.0 * laser_omega)
    )
    kappa = (
        2.0
    ...
    h_sql = torch.sqrt(8.0 * lal.HBAR_SI / (_MIRROR_MASS * (omega * _ARM_LENGTH) ** 2))
    ...
    d1 = sqrt_efficiency * (
        ...
        + 0.25
        * epsilon
        * (3.0 + rho + 2.0 * rho * exp_four_i_beta + exp_two_i_beta * (1.0 + 5.0 * rho))
        * sin_phi
    ...
    c11 = sqrt_efficiency * (
        ...
        * (
            ...
            + 4.0 * (1.0 + rho * rho) * cos_beta.square() * cos_two_phi
            + (3.0 + exp_two_i_beta) * kappa * (1.0 + rho * rho) * sin_two_phi
        )
        + signal_recycling_loss
        * (
            exp_two_i_beta * rho
            - 0.5 * (1.0 + rho * rho) * (cos_two_phi + kappa / 2.0 * sin_two_phi)
        )
    )
    c21 = (
        sqrt_efficiency
    ...
    numerator.add_(_magnitude_squared(c12 * sin_zeta + c11 * cos_zeta))
    del c11, c12
    ...
    p11 = (
        ...
        * sqrt_signal_loss
        * tau
        * (-2.0 * rho * exp_two_i_beta + 2.0 * cos_two_phi + kappa * sin_two_phi)
    )
    p21 = (
        sqrt_efficiency
        * sqrt_signal_loss
    ...
    numerator.add_(_magnitude_squared(p11 * sin_zeta + p21 * cos_zeta))
    del p21
    p12 = (
        -sqrt_efficiency
        * sqrt_signal_loss
    ...
    numerator.add_(_magnitude_squared(p12 * sin_zeta + p11 * cos_zeta))
    del p11, p12
    ...
    numerator.add_(_magnitude_squared(n11 * sin_zeta + n21 * cos_zeta))
    del n11, n21
    n12 = (
        -sqrt_efficiency
        * math.sqrt(2.0 * epsilon)
    ...
    numerator.add_(_magnitude_squared(n12 * sin_zeta + n22 * cos_zeta))

    return h_sql.square() / (2.0 * kappa * tau * tau * denominator) * numerator
...
def _aligo_thermal(frequencies):
    ...
    return suspension_scale * (
        _SUSPENSION_FREQUENCY / frequencies
    ) ** 5 + coating_scale * (_COATING_FREQUENCY / frequencies)
...
def _read_data_file_asd(psd_name):
    ...
    if path is None:
        raise RuntimeError(f"Unable to resolve LALSimulation PSD data file {filename}")
    rows = []
    try:
        ...
        raise RuntimeError(f"Invalid LALSimulation PSD data file {filename}") from exc
...
def _data_file_psd(psd_name, length, delta_f, low_freq_cutoff, device):
    ...
    if kmin < length - 1:
        ...
        lower = upper - 1
        weight = (frequencies[upper] - sample_frequencies) / (
            frequencies[upper] - frequencies[lower]
        )
        interpolated = weight * log_asd[lower] + (1.0 - weight) * log_asd[upper]
