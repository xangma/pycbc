...
def _sky_grid(owner, angular_values, t_gps, device, dtype, extras=()):
    ...
    if time_grid:
        ...
        next_value += 1
        phase_offsets = (
            relative_time / float(owner.sday) * (2.0 * np.pi)
        )
...
def single_arm_frequency_response(frequency, direction, arm_length):
    """Evaluate the finite-arm transfer function with Torch operations."""
    return _torch_single_arm_frequency_response(
        frequency, direction, arm_length
    )
...
def antenna_pattern(
    ...
    device, dtype = _input_spec(
        inputs, angular_inputs, mps_safe=True
    )
    finite_arm = (
        isinstance(frequency, torch.Tensor)
...
def effective_distance(detector, distance, ra, dec, pol, time, inclination):
    ...
    scale = torch.sqrt(
        (fplus * plus_inclination).square()
        + (fcross * cos_inclination).square()
    )
