...
def _torch_single_arm_frequency_response(frequency, direction, arm_length):
    ...
    sinc1 = torch.sinc(phase * minus / (2.0 * torch.pi))
    cos1 = torch.cos(theta1)
    sin1 = torch.sin(theta1)

    theta2 = 0.5 * phase * (3.0 - direction)
    sinc2 = torch.sinc(phase * plus / (2.0 * torch.pi))
...
def _torch_time_delay(
    ...
    ehat = torch.stack(
        (
            cosdec * cosgha,
            -cosdec * singha,
            torch.sin(declination) + torch.zeros_like(phase_offsets),
        )
    )
...
def _sky_grid(owner, angular_values, t_gps, device, dtype, extras=()):
    ...
    # Retained time grids commonly share one sky position. Keep its CPU
    # trigonometry scalar; the geometry expressions broadcast with time.
    # Broadcasting before sin/cos needlessly repeats that work per time bin.
    if (
        device.type == "cpu"
        and not extras
        and all(value.ndim == 0 for value in converted[: len(angular_values)])
    ):
        broadcast = converted
    else:
        broadcast = torch.broadcast_tensors(*converted)
