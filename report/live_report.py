...
MAIN_REVISION = "607bce53ead14f12af32552a5b2441d3bc667267"
CPU_REVISION = "1a2ebea088d9e0a31cbb22c19ad24f96ffea2b7c"
...
def load_campaign(path, head, threads, revision, run_date):
    raw = path.read_bytes()
    data = json.loads(raw)
    assert data["artifact_type"] == "production_live_batch", path
        ...
        for route in required_routes:
            ...
            for record in records:
                ...
                if (
                    ...
                    or record["seed"] != 7101 + 10000 * replicate + 100 * record["cell_index"]
                    or record["dtypes"]
                    != {
                        "output": "complex64",
                        "psd": "float32",
...
def main():
    ...
    parser.add_argument("--main-revision", default=MAIN_REVISION)
    parser.add_argument("--cpu-revision", default=CPU_REVISION)
    ...
    for head, revision in (("main", args.main_revision), ("cpu", args.cpu_revision)):
        for threads in (1, 4):
            new_rows, metadata = load_campaign(
                args.input_dir / f"{head}-t{threads}.json",
                head,
                threads,
                revision,
                args.run_date,
    ...
    assert len({s["host"] for s in sources}) == 1, (
        "Cross-head inputs used different hosts"
    )
    for key in ("affinity", "snr_threshold"):
        assert len({s["args"][key] for s in sources}) == 1, f"Mismatched {key}"
    summary = {
        "schema_version": 1,
