...
def main():
    ...
    assert accepted['scope'] == 'shared-host finite-workload diagnostic'
    ...
    for name, source, scheme in ROUTES:
        plan.append(dict(case='interval-' + name, mode='filter-timing',
                         source=str(source), scheme=scheme, runner='run-observed-case.py'))
    ...
    for name, _, _ in ROUTES:
        comparisons.append((name + '-repeat-parity', ROOT / f'runs/matched-{name}-r1/triggers.hdf',
                            [ROOT / f'runs/matched-{name}-r{repeat}/triggers.hdf' for repeat in (2, 3)]
                            + [ROOT / f'runs/interval-{name}/triggers.hdf']))
...
if __name__ == '__main__':
    ...
    def observe_host():
        with (ROOT / 'comparison-host-samples.jsonl').open('x') as log:
            while True:
                sample = dict(utc=time.time(), monotonic_ns=time.monotonic_ns(),
