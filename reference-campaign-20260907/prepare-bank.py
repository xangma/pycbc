#!/usr/bin/env python3
"""Prepare a reproducible 384-template workload, not a coverage/search bank.
...
def make_rows():
    ...
    def strata(population, count):
        # Each multiplier is coprime to both counts (246 and 112), giving exactly
        # one midpoint in every marginal stratum, with no random-number state.
        for i in range(count):
            u = [((a * i + b) % count + 0.5) / count
                 for a, b in zip((1, 5, 13, 17), (0, 3, 7, 11))]
            values = [lo + (hi - lo) * x
    ...
    if [sum(r["population"] == p for r in rows) for p in BOUNDS] != [256, 128]:
    ...
    try:
        ...
        metadata = {
            ...
            "scope": [
                ...
                "Longest means maximum estimator result among the selected 384 rows.",
            ...
            "selection": {
                "sha256": selection_digest, "population_counts": {"BNS": 256, "NSBH": 128},
                "bounds": dict(zip(BOUNDS, (dict(zip(AXES, v)) for v in BOUNDS.values()))),
                "method": "BNS:10 boundary+246 interior; NSBH:16 boundary+112 interior. "
