...
class TestTimeSeriesBase(array_base, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.bad2 = TimeSeries(
            [1, 1, 1, 1], self.delta_t, epoch=self.epoch, dtype=self.dtype
        )
        ...
        if self.epoch == 0:
            self.bad4 = TimeSeries(
                [1, 1, 1], self.delta_t, epoch=1000 + 1e-6, dtype=self.dtype
            )
        else:
            self.bad4 = TimeSeries(
                [1, 1, 1], self.delta_t, epoch=None, dtype=self.dtype
            )
    ...
    def test_numpy_init(self):
        with self.context:
            ...
            self.assertRaises(ValueError, TimeSeries, inbad, 0.1)
            self.assertRaises(TypeError, TimeSeries, in1, 0.1, epoch=(5, 1))
    ...
    def test_array_init(self):
        ...
        if self.scheme != "cpu":
            self.assertRaises(
                TypeError, TimeSeries, out4, 0.1, copy=False, epoch=self.epoch
            )
    ...
    def test_list_init(self):
        with self.context:
            ...
            if self.kind == "complex":
                out2 = TimeSeries(
                    [5.0 + 0j, 3 + 0j, 1 + 0j], 0.1, dtype=self.dtype, epoch=self.epoch
                )
            ...
            self.assertRaises(ValueError, TimeSeries, [], 0.1)
            self.assertRaises(TypeError, TimeSeries, [1, 2, 3], 0.1, epoch=(5, 1))
    ...
    def test_at_time(self):
        ...
        self.assertAlmostEqual(a.at_time(2.1), 2.0)
        ...
        self.assertEqual(numpy.ndim(a.at_time(2.1)), 0)
        self.assertEqual(numpy.ndim(a.at_time(i)), 1)

    def test_inject(self):
        a = TimeSeries(numpy.zeros(2**20, dtype=numpy.float32), delta_t=1.0)
        a[2**19] = 1
    ...
    def test_sample_times(self):
        with self.context:
            ...
            self.assertAlmostEqual(
                self.a.sample_times[-1] - self.a.sample_times[0], 0.2
            )
            self.assertEqual(len(self.b.sample_times), 3)
            self.assertAlmostEqual(
                self.b.sample_times[-1] - self.b.sample_times[0], 0.2
            )
            self.assertEqual(len(self.bad3.sample_times), 3)
            self.assertAlmostEqual(
                self.bad3.sample_times[-1] - self.bad3.sample_times[0], 0.4
            )
