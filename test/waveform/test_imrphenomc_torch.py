...
def test_imrphenomc_default_supports_double_precision_devices(
    preserve_scheme,
):
    _activate_scheme(_scheme.TorchScheme("cpu"))
    assert imrphenomc_default_native_supported({})
...
def test_imrphenomc_default_rejects_mps(preserve_scheme):
    _activate_scheme(_scheme.TorchScheme("mps"))
    assert not imrphenomc_default_native_supported({})
...
def test_imrphenomc_native_flag_precedence(
    global_value,
    component_value,
    expected_native,
    monkeypatch,
    ...
    import pycbc.waveform.imrphenomc_torch as native_mod
    import pycbc.waveform.waveform as waveform_mod

    native_generator = native_mod.imrphenomc_fd_torch
    lal_generator = waveform_mod.lalsimulation.SimInspiralChooseFDWaveform
    ...
    monkeypatch.setattr(native_mod, "imrphenomc_fd_torch", recording_native)
    monkeypatch.setattr(
        waveform_mod.lalsimulation,
        "SimInspiralChooseFDWaveform",
        recording_lal,
    ...
    if component_value is not None:
        monkeypatch.setenv("PYCBC_IMRPHENOMC_NATIVE", component_value)
    _activate_scheme(_scheme.TorchScheme("cpu"))
    get_fd_waveform(
...
def test_imrphenomc_default_mps_uses_lalsimulation_fallbacks(
    monkeypatch,
    preserve_scheme,
):
    import pycbc.waveform.imrphenomc_torch as native_mod
    ...
    def unexpected_native(**_params):
        raise AssertionError("default MPS IMRPhenomC request reached Torch")
    ...
    monkeypatch.setattr(native_mod, "imrphenomc_fd_torch", unexpected_native)
    monkeypatch.setattr(
        native_mod,
        "imrphenomc_fd_sequence_torch",
        unexpected_native,
    ...
    _clear_native_flags(monkeypatch)
    _activate_scheme(_scheme.TorchScheme("mps"))
    result = get_fd_waveform(approximant="IMRPhenomC", **params)
