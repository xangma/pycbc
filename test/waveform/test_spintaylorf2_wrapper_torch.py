...
def test_spintaylorf2_native_flag_precedence(
    global_value,
    component_value,
    expected_native,
    monkeypatch,
    ...
    native_generator = spintaylorf2_mod.spintaylorf2_torch
    lal_generator = waveform_mod.lalsimulation.SimInspiralChooseFDWaveform
    ...
    monkeypatch.setattr(spintaylorf2_mod, "spintaylorf2_torch", recording_native)
    monkeypatch.setattr(
        waveform_mod.lalsimulation,
        "SimInspiralChooseFDWaveform",
        recording_lal,
    ...
    if component_value is None:
        monkeypatch.delenv("PYCBC_SPINTAYLORF2_NATIVE", raising=False)
    else:
        monkeypatch.setenv("PYCBC_SPINTAYLORF2_NATIVE", component_value)
    _activate_scheme(_scheme.TorchScheme("cpu"))
    get_fd_waveform(
...
def test_spintaylorf2_default_supports_double_precision_devices(
    preserve_scheme,
):
    _activate_scheme(_scheme.TorchScheme("cpu"))
    assert spintaylorf2_default_native_supported({})
...
def test_spintaylorf2_default_rejects_mps(preserve_scheme):
    _activate_scheme(_scheme.TorchScheme("mps"))
    assert not spintaylorf2_default_native_supported({})
...
def test_spintaylorf2_default_mps_uses_lalsimulation_fallbacks(
    monkeypatch,
    preserve_scheme,
):
    import pycbc.waveform.spintaylorf2_torch as spintaylorf2_mod
    ...
    def unexpected_native(**_params):
        raise AssertionError("default MPS SpinTaylorF2 request reached Torch")
    ...
    monkeypatch.setattr(spintaylorf2_mod, "spintaylorf2_torch", unexpected_native)
    monkeypatch.setattr(
        spintaylorf2_mod,
    ...
    _clear_native_flags(monkeypatch)
    _activate_scheme(_scheme.TorchScheme("mps"))
    result = get_fd_waveform(approximant="SpinTaylorF2", **params)
