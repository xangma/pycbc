...
"""Foundation tests for the JAX-native waveform registry."""
...
def test_registered_modules_are_importable():
    ...
    for port in registry.JAX_NATIVE_WAVEFORMS.values():
...
def test_unregistered_dispatch_uses_lal_fallback():
    ...
    assert registry.try_jax_native_waveform("fd", params) is None


def test_registered_dispatch(monkeypatch):
    port = registry.JAXNativeWaveform(
    ...
    monkeypatch.setattr(
        registry, "JAX_NATIVE_WAVEFORMS", {port.approximant: port}
    )
    monkeypatch.setattr(
    ...
    assert registry.try_jax_native_waveform("fd", params) == 10
...
def test_registered_import_error_is_not_hidden(monkeypatch):
    ...
    with pytest.raises(ImportError, match="broken native implementation"):
        registry.try_jax_native_waveform(
