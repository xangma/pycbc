...
def _coprecessing_deviations(strength=0.4, **fits):
    dtype = torch.float64
    values = (
        torch.tensor(fits.get(name, 0.0), dtype=dtype)
        for name in _COPRECESSING_FIT_NAMES
    )
    return PNRCoprecessingDeviations(
        strength=torch.tensor(strength, dtype=dtype),
        fits=PNRCoprecessingFits(*values),
    )
...
@pytest.mark.parametrize("device_name", ("cpu", "mps", "cuda"))
@pytest.mark.parametrize("tensor_argument", ("chip", "final_spin"))
def test_remnant_mixed_inputs_stay_on_tensor_device(
    device_name,
    tensor_argument,
):
    if device_name == "mps" and not torch.backends.mps.is_available():
    ...
    dtype = torch.float32 if device_name == "mps" else torch.float64
    tensor_value = torch.tensor(0.3, device=device_name, dtype=dtype)
    keyword_arguments = {tensor_argument: tensor_value}
    ...
    for quantity in remnant:
        assert quantity.device.type == device_name
        assert quantity.dtype == dtype
        assert bool(torch.isfinite(quantity))


def test_pnr_coprecessing_shifts_ringdown_frequencies():
    theta = torch.tensor([40.0, 20.0, 0.4, -0.2], dtype=torch.float64)
    deviations = _coprecessing_deviations(nu5=0.003, nu6=0.002)

    with torch_context(theta):
        baseline = xas_torch._get_cutoff_fMs(*theta, chip=0.3)
        shifted = xas_torch._get_cutoff_fMs(
            *theta,
            chip=0.3,
            coprecessing_deviations=deviations,
        )

    torch.testing.assert_close(
        shifted[0], baseline[0] - deviations.strength * deviations.fits.nu5
    )
    torch.testing.assert_close(
        shifted[1], baseline[1] + deviations.strength * deviations.fits.nu6
    )
    torch.testing.assert_close(shifted[2], baseline[2])
    torch.testing.assert_close(shifted[3], baseline[3])


def test_pnr_coprecessing_shifts_intermediate_phase_coefficients():
    theta = torch.tensor([40.0, 20.0, 0.4, -0.2], dtype=torch.float64)
    frequencies = torch.tensor([0.03, 0.05, 0.08], dtype=torch.float64)
    phase_coeffs = xutils.PhenomX_phase_coeff_table.to(dtype=torch.float64)
    deviations = _coprecessing_deviations(zeta1=1.0e-6, zeta2=0.01)

    with torch_context(theta):
        baseline = xas_torch.get_intermediate_raw_phase(
            frequencies,
            theta,
            phase_coeffs,
            1.2,
        ...
        shifted = xas_torch.get_intermediate_raw_phase(
            frequencies,
            theta,
            phase_coeffs,
            1.2,
    ...
    expected = deviations.strength * (
        deviations.fits.zeta2 * torch.log(frequencies)
        - deviations.fits.zeta1 * frequencies.pow(-3.0) / 3.0
    )
    torch.testing.assert_close(shifted - baseline, expected)


def test_pnr_coprecessing_shifts_ringdown_phase_lorentzian():
    theta = torch.tensor([40.0, 20.0, 0.4, -0.2], dtype=torch.float64)
    frequencies = torch.tensor([0.06, 0.09, 0.12], dtype=torch.float64)
    phase_coeffs = xutils.PhenomX_phase_coeff_table.to(dtype=torch.float64)
    deviations = _coprecessing_deviations(nu4=0.02)

    with torch_context(theta):
        _, (baseline_cl, baseline_cv) = (
            xas_torch.get_mergerringdown_raw_phase(
                frequencies,
                theta,
                phase_coeffs,
                chip=0.3,
    ...
    torch.testing.assert_close(
        shifted_cl - baseline_cl,
        deviations.strength * deviations.fits.nu4,
    )
    torch.testing.assert_close(shifted_cv, baseline_cv)
...
def test_amp_plan_matches_deviations_fallback_raw_bytes(monkeypatch):
    theta = torch.tensor([37.0, 16.0, 0.62, -0.31], dtype=torch.float64)
    frequencies = torch.linspace(15.0, 1024.0, 257, dtype=torch.float64)
    amp_coeffs = xutils.PhenomX_amp_coeff_table.to(dtype=torch.float64)
    deviations = _coprecessing_deviations(
        strength=0.37,
        mu1=0.013,
        mu2=-0.021,
        mu3=0.034,
...
def test_pnr_coprecessing_shifts_ringdown_peak_amplitude():
    theta = torch.tensor([40.0, 20.0, 0.4, -0.2], dtype=torch.float64)
    amp_coeffs = xutils.PhenomX_amp_coeff_table.to(dtype=torch.float64)
    deviations = _coprecessing_deviations(mu1=0.07)

    with torch_context(theta):
        _, peak_frequency = xas_torch.get_mergerringdown_Amp(
            torch.tensor(0.1, dtype=torch.float64),
            theta,
            amp_coeffs,
            chip=0.3,
        )
        baseline, baseline_peak = xas_torch.get_mergerringdown_Amp(
            peak_frequency,
            theta,
            amp_coeffs,
            chip=0.3,
        )
        shifted, shifted_peak = xas_torch.get_mergerringdown_Amp(
            peak_frequency,
            theta,
            amp_coeffs,
            chip=0.3,
    ...
    torch.testing.assert_close(shifted_peak, baseline_peak)
    torch.testing.assert_close(
        shifted - baseline,
        deviations.strength * deviations.fits.mu1,
    )


def test_pnr_coprecessing_shifts_ringdown_peak_frequency():
    theta = torch.tensor([40.0, 20.0, 0.4, -0.2], dtype=torch.float64)
    amp_coeffs = xutils.PhenomX_amp_coeff_table.to(dtype=torch.float64)
    deviations = _coprecessing_deviations(mu2=0.08)
    m1, m2, chi1, chi2 = theta
    eta = m1 * m2 / (m1 + m2) ** 2
    ...
    with torch_context(theta):
        ringdown, damping, _, _ = xas_torch._get_cutoff_fMs(
            *theta,
            chip=0.3,
        )
        gamma2 = (
            xutils.Amp_Nospin_CV(
                amp_coeffs[4, : xas_torch.amp_eqspin_indx], eta
            )
            + xutils.Amp_Eqspin_CV(
                amp_coeffs[
                    4,
                    xas_torch.amp_eqspin_indx : xas_torch.amp_uneqspin_indx,
                ],
            ...
            + xutils.Amp_Uneqspin_CV(
                amp_coeffs[4, xas_torch.amp_uneqspin_indx :],
                eta,
                total_spin,
                spin_difference,
            )
        ...
        gamma3 = (
            xutils.Amp_Nospin_CV(
                amp_coeffs[5, : xas_torch.amp_eqspin_indx], eta
            )
            + xutils.Amp_Eqspin_CV(
                amp_coeffs[
                    5,
            ...
            + xutils.Amp_Uneqspin_CV(
                amp_coeffs[5, xas_torch.amp_uneqspin_indx :],
                eta,
                total_spin,
                spin_difference,
            )
            + deviations.strength * deviations.fits.mu2
        )
        expected_peak = torch.where(
            gamma2 <= 1.0,
            torch.abs(
                ringdown
                + damping
                * gamma3
                * (torch.sqrt(1.0 - gamma2 * gamma2) - 1.0)
                / gamma2
            ),
            torch.abs(ringdown - damping * gamma3 / gamma2),
        )
        _, actual_peak = xas_torch.get_mergerringdown_Amp(
            torch.tensor(0.1, dtype=torch.float64),
            theta,
            amp_coeffs,
            chip=0.3,
    ...
    torch.testing.assert_close(actual_peak, expected_peak)


def test_pnr_coprecessing_shifts_intermediate_inverse_amplitude():
    theta = torch.tensor([40.0, 20.0, 0.4, -0.2], dtype=torch.float64)
    amp_coeffs = xutils.PhenomX_amp_coeff_table.to(dtype=torch.float64)
    deviations = _coprecessing_deviations(mu3=0.05)
    ...
    with torch_context(theta):
        ...
        _, _, meco, isco = xutils.get_cutoff_fMs(*theta)
        inspiral_match = meco + 0.25 * (isco - meco)
        collocation_frequency = inspiral_match + 0.5 * (
            peak_frequency - inspiral_match
        )
        baseline = xas_torch.get_intermediate_Amp(
        ...
        shifted = xas_torch.get_intermediate_Amp(
            collocation_frequency,
            theta,
            amp_coeffs,
            peak_frequency,
    ...
    inverse_baseline = collocation_frequency.pow(7.0 / 6.0) / baseline
    expected = collocation_frequency.pow(7.0 / 6.0) / (
        inverse_baseline + deviations.strength * deviations.fits.mu3
    )
    torch.testing.assert_close(shifted, expected)


def test_pnr_coprecessing_deviations_thread_through_xas_carrier():
    theta = torch.tensor([40.0, 20.0, 0.4, -0.2], dtype=torch.float64)
    extrinsic = torch.tensor([500.0, 0.0, 0.3], dtype=torch.float64)
    frequencies = torch.linspace(20.0, 400.0, 100, dtype=torch.float64)
    phase_coeffs = xutils.PhenomX_phase_coeff_table.to(dtype=torch.float64)
    ...
    with torch_context(frequencies):
        baseline = xas_torch._gen_IMRPhenomXAS(
            frequencies,
            theta,
            extrinsic,
            phase_coeffs,
        ...
        zero_strength = xas_torch._gen_IMRPhenomXAS(
            frequencies,
            theta,
            extrinsic,
            phase_coeffs,
            ...
            30.0,
            chip=0.3,
            coprecessing_deviations=_coprecessing_deviations(
                strength=0.0,
                **fit_values,
            ),
        )
        shifted = xas_torch._gen_IMRPhenomXAS(
            frequencies,
            theta,
            extrinsic,
            phase_coeffs,
            ...
            30.0,
            chip=0.3,
            coprecessing_deviations=_coprecessing_deviations(**fit_values),
        )

    torch.testing.assert_close(zero_strength, baseline, rtol=0.0, atol=0.0)
    assert torch.isfinite(shifted).all()
    assert not torch.allclose(shifted, baseline, rtol=1.0e-8, atol=0.0)
