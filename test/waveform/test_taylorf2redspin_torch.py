...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"approximant": "TaylorF2RedSpin", "spin_order": -1.5}, False),
        ({"approximant": "TaylorF2RedSpin", "tidal_order": float("nan")}, False),
        ({"approximant": "TaylorF2RedSpin", "eccentricity_order": "bad"}, False),
...
@pytest.mark.parametrize("approximant", ["TaylorF2RedSpin", "TaylorF2RedSpinTidal"])
@pytest.mark.parametrize("interface", ["regular", "sequence"])
@pytest.mark.parametrize(
    ("mass1", "mass2", "compute_device"),
    [(0.2, 0.2, "cpu"), (1.4, 1.4, "cpu"), (20.0, 20.0, "mps"),
     # At fixed total mass, an unequal mass ratio raises the phase boundary.
     (39.0, 1.0, "cpu")],
)
def test_mps_phase_precision_fallback_preserves_public_waveforms(
    approximant, interface, mass1, mass2, compute_device, monkeypatch,
    preserve_scheme,
):
    ...
    params = dict(
        approximant=approximant, mass1=mass1, mass2=mass2, spin1z=0.1,
        spin2z=0.0, distance=100.0, inclination=0.7, coa_phase=0.3,
    )
    if approximant == "TaylorF2RedSpinTidal":
        params.update(lambda1=333.0, lambda2=777.0)
    if interface == "regular":
        params.update(delta_f=0.25, f_lower=10.0, f_final=512.0)
        generate = get_fd_waveform
        supported = taylorf2redspin_native_supported
    else:
        params["sample_points"] = [80.0, 10.0, 25.0, 300.0]
        generate = get_fd_waveform_sequence
        supported = taylorf2redspin_sequence_native_supported

    _clear_native_flags(monkeypatch)
    _activate_scheme(_scheme.CPUScheme() if interface == "regular"
                     else _scheme.TorchScheme("cpu"))
    reference = tuple(series.numpy().copy() for series in generate(**params))

    evaluated_on = []
    model_samples = native_mod._model_samples

    def record_precision(inputs, coefficients, frequencies, **kwargs):
        evaluated_on.append((frequencies.device.type, frequencies.dtype))
        return model_samples(inputs, coefficients, frequencies, **kwargs)

    monkeypatch.setattr(native_mod, "_model_samples", record_precision)
    _activate_scheme(_scheme.TorchScheme("mps"))
    assert supported(params)
    # Native dispatch stays enabled: a LAL fallback cannot provide the
    # reduced-spin sequence interface at these frequencies.
    actual = generate(**params)
    dtype = torch.float64 if compute_device == "cpu" else torch.float32
    assert evaluated_on == [(compute_device, dtype)]
    tolerance = 2e-6 if compute_device == "cpu" else 1e-3
    for expected, series in zip(reference, actual):
        assert series._data.tensor.device.type == "mps"
        assert series._data.tensor.dtype == torch.complex64
        np.testing.assert_allclose(series.numpy(), expected, rtol=tolerance, atol=0)
