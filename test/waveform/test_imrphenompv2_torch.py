...
def _run_case(params, approximant="IMRPhenomPv2"):
...
def test_imrphenompv2_regular_grid_power_of_two_boundary(approximant):
    params = dict(
        mass1=1.4,
        mass2=1.2,
        spin1x=0.02,
    ...
    assert actual_metadata == reference_metadata
    assert all(metadata[0] == 129 for metadata in actual_metadata)
    for expected, result in zip(reference, actual):
        np.testing.assert_array_equal(result == 0.0, expected == 0.0)
        nonzero = np.abs(expected) > 0.0
        relative_error = np.linalg.norm(
...
def test_imrphenompv2_mps_support_boundary(preserve_scheme):
    params = dict(
        approximant="IMRPhenomPv2_NRTidalv2",
        mass1=1.55,
        mass2=1.15,
    ...
    assert imrphenompv2_native_supported(
        {**params, "delta_f": 4.0, "f_lower": 20.0}
    )
    # f_lower=18 evaluates from the 16 Hz grid bin, below the guard.
    assert not imrphenompv2_native_supported(
        {**params, "delta_f": 4.0, "f_lower": 18.0}
    )
    assert imrphenompv2_sequence_native_supported(
        {**params, "sample_points": [20.0, 100.0, 500.0]}
    )
    assert not imrphenompv2_sequence_native_supported(
        {**params, "sample_points": [10.0, 100.0, 500.0]}
    )
...
@pytest.mark.parametrize(
    ...
    (
        ("regular", "imrphenompv2_fd_torch"),
        ("sequence", "imrphenompv2_fd_sequence_torch"),
    ),
)
@pytest.mark.parametrize(
    "opt_out_flag",
    ("PYCBC_TORCH_NATIVE_PORTS", "PYCBC_IMRPHENOMPV2_NATIVE"),
)
def test_imrphenompv2_default_native_opt_out_uses_lal(
    interface,
    native_name,
    opt_out_flag,
    monkeypatch,
    ...
    def unexpected_native(**_params):
        raise AssertionError("opted-out IMRPhenomPv2 reached Torch")

    monkeypatch.setattr(imrphenompv2_mod, native_name, unexpected_native)
    _clear_native_flags(monkeypatch)
    ...
    if interface == "regular":
        ...
        reference = get_fd_waveform_sequence(
            approximant="IMRPhenomPv2",
            sample_points=[20.0, 30.0, 100.0, 200.0],
            **params,
        )
    reference_arrays = tuple(series.numpy().copy() for series in reference)
...
def test_imrphenompv2_mps_low_frequency_uses_lal_fallback(
    interface,
    lal_name,
    native_name,
    monkeypatch,
    ...
    def unexpected_native(**_params):
        raise AssertionError("unsafe MPS IMRPhenomPv2 reached Torch")
    ...
    if interface == "regular":
        ...
        result = get_fd_waveform_sequence(
            approximant="IMRPhenomPv2_NRTidalv2",
            sample_points=[10.0, 20.0, 100.0, 500.0],
            **params,
        )
...
def test_imrphenompv2_public_native_stays_on_requested_device(
    ...
    spline_devices = []
    natural_cubic_coeff = imrphenompv2_mod._natural_cubic_coeff
    ...
    monkeypatch.setattr(
        imrphenompv2_mod, "_natural_cubic_coeff", recording_spline
    )
    _clear_native_flags(monkeypatch)
