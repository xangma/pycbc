...
def test_mode_availability_preserves_lal_fallbacks(torch_cpu_ctx, domain):
    available = getattr(waveform_modes, f"{domain}_waveform_mode_approximants")
    assert available(torch_cpu_ctx) == available(scheme.CPUScheme())
    assert available(torch_cpu_ctx)
...
def test_sum_modes_rejects_different_frequency_grids(torch_cpu_ctx):
    with torch_cpu_ctx:
        modes = {
