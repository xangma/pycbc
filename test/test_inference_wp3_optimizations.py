...
class TestWP2MultiDetectorBatchedLikelihood(unittest.TestCase):
    ...
    @staticmethod
    def _minimal_numpy_batch_model(n_freq):
        ...
        reference_epoch = 1.0
        hp = FrequencySeries(
            np.ones(n_freq, dtype=np.complex128),
            delta_f=1.0,
            epoch=reference_epoch,
        )
        hc = FrequencySeries(
            np.zeros(n_freq, dtype=np.complex128),
            delta_f=1.0,
            epoch=reference_epoch,
        )
        ...
        def antenna_pattern(ra, _dec, polarization, tc):
            det.antenna_times.append(np.asarray(tc).copy())
            return np.cos(polarization) + 0.001 * tc + 0.0 * ra, 0.0 * tc

        det.antenna_pattern = antenna_pattern
        ...
        class Geometry:
            ...
            @staticmethod
            def time_delay_from_earth_center(ra, _dec, _tc):
                return np.asarray([np.zeros_like(ra) + 0.125])

            @staticmethod
            def antenna_pattern_and_time_delay(ra, dec, pol, tc):
                delay = np.asarray([np.zeros_like(ra) + 0.125])
                # Deliberately evaluate this compatibility API at its input
                # time. The likelihood must instead evaluate at arrival time.
                fp = np.asarray([np.cos(pol) + 0.001 * tc + 0.0 * dec])
                fc = np.zeros_like(fp)
                return fp, fc, delay
    ...
    def test_batch_axis_is_explicit_when_batch_equals_frequency_length(self):
        n_freq = 4
        model, det = self._minimal_numpy_batch_model(n_freq)
        ra = np.linspace(0.1, 0.4, n_freq)
        dec = np.linspace(-0.2, 0.2, n_freq)
        pol = np.linspace(0.0, 0.3, n_freq)
        tc = np.linspace(2.0, 2.3, n_freq)

        actual = model._batched_loglr(
            ra=ra, dec=dec, polarization=pol, tc=tc
        )

        arrival = tc + 0.125
        fp = np.cos(pol) + 0.001 * arrival
        frequencies = np.arange(n_freq)
        projected = fp[:, None] * np.exp(
            -2.0j * np.pi
            * (arrival - 1.0)[:, None]
            * frequencies
        )
        expected = np.sum(projected.conj(), axis=-1).real
        expected -= 0.5 * np.sum(np.abs(projected) ** 2, axis=-1)
        self.assertEqual(actual.shape, (n_freq,))
        np.testing.assert_allclose(actual, expected, rtol=1e-13, atol=1e-13)
        np.testing.assert_allclose(det.antenna_times, [arrival])

    def test_numpy_batch_path_does_not_import_torch(self):
        model, _ = self._minimal_numpy_batch_model(3)
    ...
    def test_batched_likelihood_rejects_radiation_frame_batches(self):
        with self.assertRaisesRegex(
                ValueError, "radiation-frame waveform parameters"):
    ...
    def test_batched_likelihood_rejects_mismatched_sample_lengths(self):
        with self.assertRaisesRegex(ValueError, "common batch length"):
            self.model.batched_loglikelihood(
                ra=np.asarray([0.2, 0.3]),
...
class TestMarginalizedPhaseFactorCache(unittest.TestCase):
    @staticmethod
    def _model():
    ...
    def test_numpy_explicit_phase_values_are_used(self):
        model = self._model()
        first_phase = np.asarray([0.1, 0.2])
        second_phase = np.asarray([0.3, 0.4])

        first = model.phase_fac(2, first_phase)
        second = model.phase_fac(2, second_phase)

        np.testing.assert_allclose(first, np.exp(2j * first_phase))
        np.testing.assert_allclose(second, np.exp(2j * second_phase))
        self.assertFalse(np.array_equal(first, second))

    def test_torch_phase_values_are_distinct_and_keep_gradients(self):
        model = self._model()
        ...
        differentiable_phase = torch.tensor(
            [0.2, 0.6], dtype=torch.float64, requires_grad=True
        )
        model.phase_fac(3, differentiable_phase).real.sum().backward()
        self.assertIsNotNone(differentiable_phase.grad)
        self.assertTrue(torch.all(torch.isfinite(differentiable_phase.grad)))
