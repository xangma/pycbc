...
def _reset_cache():
    wisdom_cache.configure_from_cli(SimpleNamespace(fftw_wisdom_cache=False))
    yield
    wisdom_cache.configure_from_cli(SimpleNamespace(fftw_wisdom_cache=False))
...
def test_cancelled_plan_releases_cold_writer_lock(tmp_path, stable_fingerprint):
    fftw = FakeFFTW()
    wisdom_cache.configure_from_cli(_options(tmp_path))
...
def test_cache_never_changes_unqualified_plans(tmp_path, stable_fingerprint, override):
    wisdom_cache.configure_from_cli(_options(tmp_path))
...
def test_manual_wisdom_options_take_precedence(tmp_path, stable_fingerprint, explicit):
...
def test_fingerprint_change_selects_a_new_cache_file(monkeypatch, tmp_path):
...
def test_default_cache_directory_honors_absolute_xdg_home(monkeypatch, tmp_path):
