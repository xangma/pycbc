...
class TestBenchJaxInspiralCampaign(unittest.TestCase):
    ...
    def test_campaign_parity_uses_available_cpu_reference(self):
        raw = {
            "branch_cpu": [{"triggers_path": "current.hdf"}],
            "jax_cuda_batched": [{"triggers_path": "jax.hdf"}],
        }
        with patch(
            ...
            self.assertEqual(reference, "branch_cpu")
            self.assertEqual(results, {"jax_cuda_batched": {"passed": True}})
            compare.assert_called_once_with(Path("current.hdf"), Path("jax.hdf"))
            raw["original_cpu"] = [{"triggers_path": "original.hdf"}]
            compare.reset_mock()
            reference, results = compare_campaign_parity(raw)
            self.assertEqual(reference, "original_cpu")
            self.assertEqual(set(results), {"branch_cpu", "jax_cuda_batched"})
