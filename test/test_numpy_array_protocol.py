...
def test_torch_numpy_sign_matches_numpy(dtype):
    values = np.array([-np.inf, -2.0, -0.0, 0.0, 2.0, np.inf, np.nan], dtype=dtype)
    with scheme.TorchScheme("cpu"):
        actual = np.sign(Array(values)).numpy()

    expected = np.sign(values)
    assert actual.dtype == expected.dtype
    np.testing.assert_array_equal(actual, expected)
    np.testing.assert_array_equal(np.signbit(actual[:-1]), np.signbit(expected[:-1]))
...
@pytest.mark.parametrize("dtype", (np.bool_, np.int32, np.int64, np.uint32))
