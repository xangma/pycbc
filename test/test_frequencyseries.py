...
class TestFrequencySeriesBase(array_base, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.bad = FrequencySeries(
            [1, 1, 1], 0.1, epoch=self.epoch, dtype=self.other_precision[self.odtype]
        )
        self.bad2 = FrequencySeries(
            [1, 1, 1, 1], 0.1, epoch=self.epoch, dtype=self.dtype
        )
        ...
        if self.epoch is None:
            self.bad4 = FrequencySeries(
                [1, 1, 1], 0.1, epoch=(1000 + 1e-6), dtype=self.dtype
            )
        else:
    ...
    def test_numpy_init(self):
        with self.context:
            ...
            self.assertRaises(ValueError, FrequencySeries, inbad, 0.1)
            self.assertRaises(TypeError, FrequencySeries, in1, 0.1, epoch=(5, 1))
    ...
    def test_array_init(self):
        ...
        with self.context:
            ...
            # Giving complex input and specifying a real dtype should raise an error
                ...
                self.assertRaises(
                    TypeError, FrequencySeries, in1, 0.1, dtype=self.dtype
                )
                self.assertRaises(
                    TypeError, FrequencySeries, in2, 0.1, dtype=self.dtype
                )
            ...
            self.assertRaises(ValueError, FrequencySeries, in1, -1)
            self.assertRaises(ValueError, FrequencySeries, inbad, 0.1)
            self.assertRaises(TypeError, FrequencySeries, in1, 0.1, epoch=(5, 2))
    ...
    def test_list_init(self):
        with self.context:
            ...
            if self.kind == "complex":
                out2 = FrequencySeries(
                    [5 + 0j, 3 + 0j, 1 + 0j], 0.1, dtype=self.dtype, epoch=self.epoch
                )
            ...
            self.assertRaises(ValueError, FrequencySeries, [], 0.1)
            self.assertRaises(TypeError, FrequencySeries, [1, 2, 3], 0.1, epoch=(5, 2))
    ...
    def test_sample_frequencies(self):
        with self.context:
            ...
            self.assertAlmostEqual(
                self.a.sample_frequencies[-1] - self.a.sample_frequencies[0], 0.2
            )
            self.assertEqual(len(self.b.sample_frequencies), 3)
            self.assertAlmostEqual(
                self.b.sample_frequencies[-1] - self.b.sample_frequencies[0], 0.2
            )
            self.assertEqual(len(self.bad3.sample_frequencies), 3)
            self.assertAlmostEqual(
                self.bad3.sample_frequencies[-1] - self.bad3.sample_frequencies[0], 0.4
            )
...
def fs_test_maker(dtype, odtype, epoch):
    ...
    TestFrequencySeries.__name__ = (
        _scheme + " " + dtype.__name__ + " with " + odtype.__name__
    )
...
for t, otypes in types:
    for ot in otypes:
        for epoch in epochs:
            na = "test" + str(i)
            vars()[na] = fs_test_maker(t, ot, epoch)
