...
COMPLEX_CASES = [
    (device, dtype) for device in DEVICES for dtype in (np.complex64, np.complex128)
]
...
def test_batched_real_transforms_preserve_row_boundaries(device, size):
    ...
    with scheme.TorchScheme(device):
        ...
        frequency_output = zeros(rows * (size // 2 + 1), dtype=np.complex64)
        torchfft.FFT(real_input, frequency_output, nbatch=rows, size=size).execute()

        frequency_input = Array(frequency_values.ravel())
        real_output = zeros(rows * size, dtype=np.float32)
        torchfft.IFFT(frequency_input, real_output, nbatch=rows, size=size).execute()
    ...
    expected_inverse = np.fft.irfft(frequency_values, n=size, axis=-1) * size
    np.testing.assert_allclose(got_forward, expected_forward, rtol=2e-6, atol=2e-6)
    np.testing.assert_allclose(got_inverse, expected_inverse, rtol=2e-6, atol=2e-6)
...
def test_batched_complex_ifft_handles_inplace_and_partial_aliases(device):
    ...
    with scheme.TorchScheme(device):
        ...
        torchfft.IFFT(inplace, inplace, nbatch=rows, size=size).execute()
        got_inplace = inplace.numpy().reshape(rows, size)

        storage = Array(np.concatenate((values.ravel(), np.zeros(1, np.complex64))))
        source = storage[:-1]
        target = storage[1:]
        torchfft.IFFT(source, target, nbatch=rows, size=size).execute()
        got_overlap = target.numpy().reshape(rows, size)

    np.testing.assert_allclose(got_inplace, expected, rtol=2e-6, atol=2e-6)
    np.testing.assert_allclose(got_overlap, expected, rtol=2e-6, atol=2e-6)
...
def test_batched_inplace_real_transforms_skip_per_row_padding(device, size):
    ...
    with scheme.TorchScheme(device):
        ...
        forward_rows[:, :size].copy_(torch.as_tensor(real_values, device=device))
        forward_output = forward_storage.view(np.complex64)
        torchfft.FFT(forward_storage, forward_output, nbatch=rows, size=size).execute()
        ...
        torchfft.IFFT(inverse_input, inverse_storage, nbatch=rows, size=size).execute()
        got_inverse = inverse_storage.numpy().reshape(rows, padded_size)[:, :size]
...
def _single_precision_case(kind, rows, size):
    if kind.startswith("c2c"):
        values = _complex_values(rows, size, np.complex64, seed=1900 + size)
        output_dtype = np.complex64
        output_size = size
        if kind == "c2c_fft":
            expected = np.fft.fft(values.astype(np.complex128), axis=-1)
        else:
        ...
        values = _complex_values(rows, size // 2 + 1, np.complex64, seed=2100 + size)
        output_dtype = np.float32
        output_size = size
        expected = np.fft.irfft(values.astype(np.complex128), n=size, axis=-1) * size
    return values, output_dtype, output_size, expected


@pytest.mark.parametrize("device", SINGLE_PRECISION_DEVICES)
@pytest.mark.parametrize("kind", ["c2c_fft", "c2c_ifft", "r2c_fft", "c2r_ifft"])
def test_single_precision_batches_reuse_promoted_workspaces(monkeypatch, device, kind):
    ...
    engine_type = (
        torchfft.FFT
        if kind.endswith("fft") and not kind.endswith("ifft")
        else torchfft.IFFT
    )
    ...
    with scheme.TorchScheme(device):
        ...
        assert workspace_ptrs == (plan.source.data_ptr(), plan.target.data_ptr())
        got = target.numpy().reshape(rows, output_size)

    difference = got.astype(expected.dtype) - expected
    relative_l2 = np.linalg.norm(difference.ravel()) / np.linalg.norm(expected.ravel())
    max_abs = np.max(np.abs(difference))
    assert relative_l2 < 4e-8
    assert max_abs < (2 * np.finfo(np.float32).eps * np.max(np.abs(expected)))


@pytest.mark.parametrize("device", SINGLE_PRECISION_DEVICES)
@pytest.mark.parametrize("kind", ["c2c_fft", "c2c_ifft", "r2c_fft", "c2r_ifft"])
def test_promoted_batches_match_or_exceed_legacy_fftw(monkeypatch, device, kind):
    ...
    with scheme.CPUScheme(1):
        ...
        legacy_type(legacy_source, legacy_target, nbatch=rows, size=size).execute()
...
def test_promoted_workspace_chunks_rows_and_handles_final_partial(
    ...
    rows, size = 5, 64
    monkeypatch.setitem(torchfft._PROMOTED_BATCH_MAX_ELEMENTS, "cpu", 2 * size)
    ...
    with scheme.TorchScheme("cpu"):
        ...
        inplace = Array(values.ravel())
        inplace_engine = torchfft.IFFT(inplace, inplace, nbatch=rows, size=size)
    ...
    np.testing.assert_allclose(got_inplace, expected, rtol=5e-7, atol=5e-7)


@pytest.mark.parametrize("target_offset", [-1, 1])
def test_promoted_chunks_snapshot_cross_pointer_overlap(monkeypatch, target_offset):
...
def test_direct_batch_ifft_execution_and_env_gate(device, monkeypatch):
    ...
    with scheme.TorchScheme(device):
        ...
        # In-place test
        inplace = Array(values.ravel())
        engine_inplace = torchfft.IFFT(inplace, inplace, nbatch=rows, size=size)
    ...
    with scheme.TorchScheme(device):
        ...
        np.testing.assert_allclose(got_fallback, expected, rtol=2e-6, atol=2e-6)
