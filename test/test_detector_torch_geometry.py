...
def test_scalar_sky_over_retained_time_grid_matches_numpy_and_gradients():
    ...
    sky = [1.37, -1.26, 0.23]
    angles = [torch.tensor(value, dtype=torch.float64, requires_grad=True)
              for value in sky]
    ...
    for value, reference, single in zip(actual, expected, separate):
        ...
        torch.testing.assert_close(value, torch.as_tensor(reference))
        torch.testing.assert_close(value, single)
    ...
    for index, angle in enumerate(angles):
        ...
        derivative /= 2.0 * step
        assert angle.grad.item() == pytest.approx(derivative, rel=1e-7, abs=1e-9)
...
def test_combined_geometry_preserves_detector_overrides(
    ...
    ra = torch.tensor([0.3, 0.7], dtype=torch.float64, requires_grad=True)
    dec = torch.tensor([-0.2, 0.4], dtype=torch.float64)
