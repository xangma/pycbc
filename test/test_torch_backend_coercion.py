...
def test_mixed_coercion_preserves_device_shapes_and_gradients(device):
    ...
    first = torch.tensor([[1.0], [2.0]], device=device, requires_grad=True)
    second = torch.tensor([3.0, 4.0, 5.0], requires_grad=True)
    host = numpy.array([6.0, 7.0, 8.0], dtype=numpy.float64)
    ...
    torch.testing.assert_close(first.grad, torch.full_like(first, 3.0))
    torch.testing.assert_close(second.grad, torch.full_like(second, 2.0))
...
def test_coercion_dtype_and_public_storage(dtype):
    ...
    expected = torch.get_default_dtype() if dtype in ("int64", "bool") else tensor.dtype
    assert module is torch
...
def test_user_tensor_subclass_dispatch_and_gradient():
    ...
    original = torch.tensor([3.0, 5.0], requires_grad=True)
    ...
    result = conversions.primary_mass(value, 2.0)
...
def test_type_cache_and_unhashable_metaclass():
    ...
    assert backend.torch_module_for(torch.tensor(1.0, requires_grad=True)) is torch
