"""Focused tests for Gaussian-noise JAX batched likelihood evaluation."""
...
from pycbc.inference.models.gaussian_noise import (  # noqa: E402
    BaseGaussianNoise,
    GaussianNoise,
...
def test_numpy_batch_path_does_not_import_jax():
    model, _ = _minimal_numpy_batch_model(3)
    original_import = builtins.__import__

    def reject_jax_import(name, *args, **kwargs):
        if name == "pycbc.inference.models.tools_jax":
            raise AssertionError("NumPy batch path imported tools_jax")
        return original_import(name, *args, **kwargs)

    values = numpy.linspace(0.1, 0.3, 3)
    with mock.patch("builtins.__import__", side_effect=reject_jax_import):
...
def test_detector_frame_batch_contract_accepts_jax_parameters():
    generator = types.SimpleNamespace(
        location_args={"tc", "ra", "dec", "polarization"}
    )
    supplied = {
        "ra": jnp.linspace(0.1, 0.4, 4),
        "dec": jnp.array(0.2),
        "polarization": jnp.linspace(0.0, 0.3, 4),
        "tc": jnp.array(2.0),
    }
    assert _detector_frame_batch_size(generator, supplied, supplied) == 4

    invalid = dict(supplied, mass1=jnp.ones(4))
...
def test_real_batch_matches_scalar_likelihood(real_model):
    models, params, backend = real_model
    for model in models:
        expected = []
        for index in range(2):
            model.update(**{name: float(numpy.asarray(value)[index])
                            for name, value in params.items()})
            expected.append(float(model.loglr))
        ...
        if backend == "numpy":
            ...
            numpy.testing.assert_allclose(
                numpy.asarray(actual), expected, atol=1e-9, rtol=1e-10
            )
            numpy.testing.assert_allclose(
                numpy.asarray(model.batched_loglikelihood(**params)),
                numpy.asarray(actual) + model.lognl,
            )
...
def test_real_batch_waveform_failure_matches_scalar_policy(
    ...
    for model in models:
        ...
        monkeypatch.setattr(model.waveform_generator.rframe_generator,
                            "generate", fail)
        model.update(**{name: float(numpy.asarray(value)[0]) for name, value in params.items()})
        ...
        if backend == "numpy":
            ...
            assert numpy.isneginf(numpy.asarray(actual)).all()
            assert numpy.isneginf(numpy.asarray(model.batched_loglikelihood(**params))).all()
...
def test_batch_failure_with_integer_tensor_angles(real_model, monkeypatch):
    ...
    params = dict(params, ra=jnp.array([0, 1]))
    ...
    for model in models:
        ...
        actual = model.batched_loglr(**params)
        assert actual.shape == (2,)
        assert str(actual.dtype).startswith("float")
