...
def test_array_backends_execute_independent_complex_batches(
    ...
    if inverse:
        engine = array_fft_backend.IFFT(source, target, nbatch=rows, size=size)
        expected = np.fft.ifft(values, axis=-1) * size
    else:
        engine = array_fft_backend.FFT(source, target, nbatch=rows, size=size)
        expected = np.fft.fft(values, axis=-1)
    engine.execute()

    np.testing.assert_array_equal(source.numpy(), source_before)
    np.testing.assert_allclose(
        target.numpy().reshape(rows, size),
        expected,
...
def test_array_backends_execute_independent_real_batches(
    ...
    frequency_output = zeros(rows * (size // 2 + 1), dtype=complex_dtype)
