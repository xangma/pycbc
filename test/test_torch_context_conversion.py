...
def test_context_transfer_preserves_autograd(monkeypatch, source, target, operation):
...
def test_absolute_sample_times_preserve_gps_spacing(device):
    ...
    with scheme.TorchScheme(device):
        series = TimeSeries(
            np.ones(8, dtype=np.float32), delta_t=1 / 4096, epoch=1126259462.125
        )
        times = series.sample_times.backend_array
        assert times.dtype == torch.float64
        expected = (
            1126259462.125 + torch.arange(8, device=device, dtype=torch.float64) / 4096
        )
...
def test_mps_absolute_sample_times_reject_loss_of_precision():
    ...
    with scheme.TorchScheme("mps"):
        ...
        with pytest.raises(TypeError, match="Absolute sample times require float64"):
