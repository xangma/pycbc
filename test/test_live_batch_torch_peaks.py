...
def test_live_batch_power_matrices_init():
    ...
    with scheme.TorchScheme("cpu"):
        ...
        assert 0 in batch.power_matrices
        pm = batch.power_matrices[0]
...
@pytest.mark.parametrize("ondevice", [False, True])
def test_live_batch_normalization_with_shared_workspace(
        torch_device, ondevice, monkeypatch):
    monkeypatch.setenv("PYCBC_TORCH_ONDEVICE_PEAKS", str(int(ondevice)))
    monkeypatch.delenv("PYCBC_BATCH_MAXELEMENTS", raising=False)
    group_size = 32
    template_size = 17
    psd1 = FrequencySeries(np.ones(template_size, dtype=np.float32),
                           delta_f=0.25)
    psd2 = psd1.copy()
    psd2[1] = 2.0
    psd2[2] = 0.5
    # Reuse each PSD across groups and blocks, then revisit the original PSD.
    psds = (psd1, psd1, psd2, psd1)
    data = np.zeros(template_size, dtype=np.complex64)
    data[1:3] = [6.0, 2.0]

    def build_batch():
        templates = []
        for index in range(2 * group_size):
            values = np.zeros(template_size, dtype=np.complex64)
            amplitude = 1.0 + index / 16.0
            values[1:3] = amplitude * np.array(
                [1.0, (0.25, 0.5, 1.0, 4.0)[index % 4]]
            )
            template = FrequencySeries(values, delta_f=0.25)
            template.id = index
            template.params = np.array(
                [(20.0 + index,)], dtype=[("mass1", np.float32)]
            )[0]
            template.sigmasq = lambda psd, h=template: matchedfilter.sigmasq(h, psd)
            templates.append(template)
        batch = matchedfilter.LiveBatchMatchedFilter(
            templates, snr_threshold=4.9, chisq_bins=0, sg_chisq=None,
            maxelements=group_size * 2 * (template_size - 1),
            enable_cuda_graphs=False, enable_async_streams=False,
        )
        assert len(batch.tgroups) == 2
        assert batch.mids[0] == batch.mids[1]
        assert len(batch.out_mem) == len(batch.cout_mem) == 1
        return batch

    def process_block(batch, psd):
        stilde = FrequencySeries(data / psd.numpy(), delta_f=0.25)
        stilde.psd = psd
        batch.set_data(types.SimpleNamespace(
            overwhitened_data=lambda _df: stilde,
            trim_padding=0, blocksize=8, sample_rate=4, start_time=100.0,
        ))
        results = [batch._process_batch()[0] for _ in batch.tgroups]
        assert batch._process_batch() == (None, None)
        return batch.combine_results(results)

    with scheme.DefaultScheme():
        reference = build_batch()
        expected = [process_block(reference, psd) for psd in psds]
    assert 0 < len(expected[2]["snr"]) < len(expected[0]["snr"]) < 2 * group_size

    def reject_scalar_sigmasq(_psd):
        raise AssertionError("optimized normalization fell back to scalar sigmasq")

    with scheme.TorchScheme(torch_device):
        batch = build_batch()
        for tgroup in batch.tgroups:
            for template in tgroup:
                template.sigmasq = reject_scalar_sigmasq
        cached = {}
        for psd, expected_result in zip(psds, expected, strict=True):
            result = process_block(batch, psd)
            np.testing.assert_array_equal(
                result["template_id"], expected_result["template_id"]
            )
            for key in ("snr", "sigmasq", "end_time"):
                np.testing.assert_allclose(
                    result[key], expected_result[key], rtol=2e-6
                )
            assert set(batch.power_matrices) == set(batch._psd_cache) == {0, 1}
            for group_id in range(len(batch.tgroups)):
                entry = batch._psd_cache[group_id][id(psd)]
                cache_key = (group_id, id(psd))
                if cache_key in cached:
                    assert entry is cached[cache_key]
                cached[cache_key] = entry
        assert all(len(cache) == 2 for cache in batch._psd_cache.values())
...
def test_live_batch_vectorized_sigmasq_and_caching(torch_device):
    ...
    with scheme.TorchScheme(torch_device):
        ...
        assert 0 in batch._psd_cache
        assert id(psd1) in batch._psd_cache[0]
        cached_sigmasqs, cached_norms = batch._psd_cache[0][id(psd1)]
        expected_inv_psd = (4.0 * 0.25) / psd_vals1
        expected_sigmasqs = batch.power_matrices[0].dot(expected_inv_psd)
        ...
        assert id(psd2) in batch._psd_cache[0]
        expected_inv_psd2 = (4.0 * 0.25) / psd_vals2
        expected_sigmasqs2 = batch.power_matrices[0].dot(expected_inv_psd2)
...
def test_live_batch_vectorized_sigmasq_cache_bounding():
    ...
    with scheme.TorchScheme("cpu"):
        ...
        assert len(batch._psd_cache[0]) == 32
