...
"""Integration checks for diffgw native IMR waveform generation (PhenomD/PhenomXAS)."""
...
def phenom_row(mass1=30.0, mass2=20.0, spin1z=0.1, spin2z=-0.2,
               f_lower=20.0, approximant='IMRPhenomD'):
    return dict(mass1=mass1, mass2=mass2, spin1z=spin1z, spin2z=spin2z,
                f_lower=f_lower, approximant=approximant)
...
def test_phenom_batch_parity_and_match(tmp_path, approximant, device, storage):
    rows = [
    ...
    kwargs = dict(filter_length=4097, delta_f=0.25, dtype=np.complex128,
    ...
    with scheme.TorchScheme(device=device):
        batch, templates = bank.get_batch_tensor(
            list(range(len(rows))), device=device, dtype=storage
        )
        assert batch.shape == (len(rows), 4097)
        assert batch.dtype == storage

        for i, (tmpl, ref) in enumerate(zip(templates, ref_templates)):
            assert tmpl.waveform_provider in ('diffgw', 'torchwave')
            # Verify match against PyCBC LAL reference waveform
            tmpl_fs = ref.copy()
            tmpl_fs.data[:] = batch[i].detach().cpu().numpy()
            m, _ = match(ref, tmpl_fs, low_frequency_cutoff=ref.f_lower,
...
def test_phenom_mixed_fallback(tmp_path):
    ...
    bank = FilterBank(filename, filter_length=4097, delta_f=0.25,
                      enable_diffgw=True, dtype=np.complex128,
                      low_frequency_cutoff=20.0)

    batch, templates = bank.get_batch_tensor([0, 1, 2])
    assert templates[0].waveform_provider in ('diffgw', 'torchwave')
    assert templates[1].waveform_provider == 'reference'
    assert templates[2].waveform_provider in ('diffgw', 'torchwave')
