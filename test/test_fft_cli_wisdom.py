...
def live_wisdom_code():
    ...
    shutdown = [
        ...
        if isinstance(node, ast.If) and ast.unparse(node.test) == "evnt.rank == 1"
    ]
...
def test_live_preserves_manual_wisdom_order_and_export_rank(
    ...
    assert cache.calls == [("configure", opt)] + ([("pending",)] if rank == 1 else [])


@pytest.mark.parametrize("rank", (0, 1, 2))
def test_live_exports_pending_automatic_wisdom(monkeypatch, live_wisdom_code, rank):
    ...
    assert cache.calls == ([("pending",), ("export", native)] if rank == 1 else [])
    assert native.calls == []
...
@pytest.mark.parametrize("fft_backends", BACKEND_SHAPES)
def test_import_wisdom_from_cli_uses_required_order(monkeypatch, fft_backends):
    fftw = RecordingFFTW()
...
@pytest.mark.parametrize("fft_backends", BACKEND_SHAPES)
def test_export_wisdom_from_cli_uses_required_order(monkeypatch, fft_backends):
    fftw = RecordingFFTW()
    monkeypatch.setattr(parser_support, "_load_fftw_for_wisdom", lambda: fftw)
    opt = SimpleNamespace(
        fft_backends=fft_backends,
...
def test_wisdom_helpers_propagate_unavailable_fftw(monkeypatch, helper, opt):
...
def test_wisdom_helpers_propagate_native_failures(monkeypatch, helper, opt, method):
    fftw = RecordingFFTW()
...
def test_wisdom_helpers_are_exported_from_fft_package():
    assert pycbc.fft.import_wisdom_from_cli is parser_support.import_wisdom_from_cli
    assert pycbc.fft.export_wisdom_from_cli is parser_support.export_wisdom_from_cli
...
def test_automatic_cache_cli_defaults_and_overrides(tmp_path):
    ...
    assert parser.parse_args(["--no-fftw-wisdom-cache"]).fftw_wisdom_cache is False
