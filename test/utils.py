...
def parse_args_all_schemes(feature_str):
    ...
    explicit_scheme = any(
        arg in ("--scheme", "-s") or arg.startswith("--scheme=") for arg in arguments
    )
    requested = "cpu" if explicit_scheme else os.environ.get("PYCBC_TEST_SCHEME", "cpu")
...
class array_base(object):
    def setNumbers(self):
        ...
        if self.okind == "real":
            ...
            self.b = self.type(
                [10 + 6j, 8 + 4j, 6 + 2j], dtype=self.odtype, **self.kwds
            )
        ...
        if self.kind == "real" and self.okind == "real":
            ...
            # self.div = [.5, 3./8., 1./6.]
            self.div = self.type(
                [0.5, 0.375, 0.16666666666666666667],
                dtype=self.result_dtype,
                **self.kwds,
            )
            # self.div_s = [1., 3./5., 1./5.]
            self.div_s = self.type(
                [1.0, 0.6, 0.2], dtype=self.result_dtype, **self.kwds
            )

            # self.rdiv = [2., 8./3., 6.]
            self.rdiv = self.type(
                [2.0, 2.66666666666666666667, 6.0], dtype=self.result_dtype, **self.kwds
            )
            # self.rdiv_s = [1., 5./3., 5.]
            self.rdiv_s = self.type(
                [1.0, 1.66666666666666666667, 5.0], dtype=self.result_dtype, **self.kwds
            )
            ...
            self.pow1 = self.type([25.0, 9.0, 1.0], dtype=self.dtype, **self.kwds)
            # self.pow2 = [pow(5,-1.5), pow(3,-1.5), pow(1,-1.5)]
            self.pow2 = self.type(
                [0.08944271909999158786, 0.19245008972987525484, 1.0],
                dtype=self.dtype,
                **self.kwds,
            )
        ...
        if self.kind == "real" and self.okind == "complex":
            self.cumsum = self.type([5, 8, 9], dtype=self.dtype, **self.kwds)
            self.mul = self.type(
                [50 + 30j, 24 + 12j, 6 + 2j], dtype=self.result_dtype, **self.kwds
            )
            self.mul_s = self.type(
                [25 + 10j, 15 + 6j, 5 + 2j], dtype=self.result_dtype, **self.kwds
            )

            self.add = self.type(
                [15 + 6j, 11 + 4j, 7 + 2j], dtype=self.result_dtype, **self.kwds
            )
            self.add_s = self.type(
                [10 + 2j, 8 + 2j, 6 + 2j], dtype=self.result_dtype, **self.kwds
            )

            # self.div = [25./68.-15.j/68., 3./10.-3.j/20., 3./20.-1.j/20.]
            self.div = self.type(
                [
                    0.36764705882352941176 - 0.22058823529411764706j,
                    0.3 - 0.15j,
                    0.15 - 0.05j,
                ],
                dtype=self.result_dtype,
            ...
            self.rdiv = self.type(
                [
                    2.0 + 1.2j,
                    2.66666666666666666667 + 1.33333333333333333333j,
                    6.0 + 2.0j,
                ],
                dtype=self.result_dtype,
                **self.kwds,
            ...
            self.rdiv_s = self.type(
                [
                    1.0 + 0.4j,
                    1.66666666666666666667 + 0.666666666666666666667j,
                    5.0 + 2.0j,
                ],
                dtype=self.result_dtype,
                **self.kwds,
            ...
            self.sub = self.type(
                [-5 - 6j, -5 - 4j, -5 - 2j], dtype=self.result_dtype, **self.kwds
            )
            self.sub_s = self.type(
                [0 - 2j, -2 - 2j, -4 - 2j], dtype=self.result_dtype, **self.kwds
            )

            self.rsub = self.type(
                [5 + 6j, 5 + 4j, 5 + 2j], dtype=self.result_dtype, **self.kwds
            )
            self.rsub_s = self.type(
                [0 + 2j, 2 + 2j, 4 + 2j], dtype=self.result_dtype, **self.kwds
            )
        ...
        if self.kind == "complex" and self.okind == "real":
            self.cumsum = self.type(
                [5 + 1j, 8 + 4j, 9 + 9j], dtype=self.dtype, **self.kwds
            )
            self.mul = self.type(
                [50 + 10j, 24 + 24j, 6 + 30j], dtype=self.result_dtype, **self.kwds
            )
            self.mul_s = self.type(
                [25 + 5j, 15 + 15j, 5 + 25j], dtype=self.result_dtype, **self.kwds
            )

            self.add = self.type(
                [15 + 1j, 11 + 3j, 7 + 5j], dtype=self.result_dtype, **self.kwds
            )
            self.add_s = self.type(
                [10 + 1j, 8 + 3j, 6 + 5j], dtype=self.result_dtype, **self.kwds
            )
            ...
            self.sub = self.type(
                [-5 + 1j, -5 + 3j, -5 + 5j], dtype=self.result_dtype, **self.kwds
            )
            self.sub_s = self.type(
                [0 + 1j, -2 + 3j, -4 + 5j], dtype=self.result_dtype, **self.kwds
            )
            ...
            self.pow1 = self.type(
                [24.0 + 10.0j, 0.0 + 18.0j, -24.0 + 10.0j],
                dtype=self.dtype,
                **self.kwds,
            )
            ...
            self.conj = self.type(
                [5 - 1j, 3 - 3j, 1 - 5j], dtype=self.dtype, **self.kwds
            )
        ...
        if self.kind == "complex" and self.okind == "complex":
            ...
            self.mul = self.type(
                [44 + 40j, 12 + 36j, -4 + 32j], dtype=self.result_dtype, **self.kwds
            )
            self.mul_s = self.type(
                [23 + 15j, 9 + 21j, -5 + 27j], dtype=self.result_dtype, **self.kwds
            )

            self.add = self.type(
                [15 + 7j, 11 + 7j, 7 + 7j], dtype=self.result_dtype, **self.kwds
            )
            self.add_s = self.type(
                [10 + 3j, 8 + 5j, 6 + 7j], dtype=self.result_dtype, **self.kwds
            )

            # self.div = [7./17.-5.j/34., 9./20.+3.j/20., 2./5.+7.j/10.]
            self.div = self.type(
                [
                    0.41176470588235294118 - 0.14705882352941176471j,
                    0.45 + 0.15j,
                    0.4 + 0.7j,
                ],
                dtype=self.result_dtype,
            ...
            self.sub = self.type(
                [-5 - 5j, -5 - 1j, -5 + 3j], dtype=self.result_dtype, **self.kwds
            )
            self.sub_s = self.type(
                [0 - 1j, -2 + 1j, -4 + 3j], dtype=self.result_dtype, **self.kwds
            )
    ...
    def test_weighted_inner(self):
        ...
        with self.context:
            ...
            # Hand calculate the relative tolerance for a scalar answer
            self.assertTrue(
                abs(c - self.weighted_inner) <= self.tol * abs(self.weighted_inner)
            )
