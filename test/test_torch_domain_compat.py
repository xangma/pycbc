...
@pytest.mark.parametrize(
    ...
    (
        ("maximum(x, 0)", lambda x: numpy.maximum(x, 0), lambda x: x > 0),
        ("minimum(1, x)", lambda x: numpy.minimum(1, x), lambda x: x < 1),
...
def test_custom_expressions_accept_scalar_operands(
    dtype, expression, expected, derivative, torch_device
):
    ...
    gradient, = torch.autograd.grad(result.sum(), x)
    assert result.dtype == dtype
    assert result.device == x.device
    torch.testing.assert_close(result, x.new_tensor(expected(values)))
    torch.testing.assert_close(gradient, x.new_tensor(derivative(values)))
...
def test_custom_expression_does_not_truncate_scalar_against_integer_tensor():
    ...
    torch.testing.assert_close(result, torch.tensor([0.5, 1, 2]))
