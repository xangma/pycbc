...
def test_torch_integer_scalar_ufuncs_match_numpy_pycbc(dtype, scalar, ufunc, reverse):
...
def test_torch_reductions_use_supported_device_accumulators(device):
    ...
    try:
        with ctx:
            ...
            assert actual_values.abs_arg_max() == int(np.argmax(np.abs(values)))
...
def test_torch_self_inner_and_abs_arg_max():
    ...
    try:
        with ctx:
            # Complex array
            c_vals = np.array([3 + 4j, 1 + 2j, 6 + 8j, 2 - 1j], dtype=np.complex64)
            c_arr = Array(c_vals)
            ...
            assert c_arr.inner(c_arr) == pytest.approx(expected_inner, rel=1e-6)
            ...
            assert r_arr.inner(r_arr) == pytest.approx(np.sum(r_vals**2), rel=1e-6)
