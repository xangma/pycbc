...
def test_numpy_argsort_stays_on_device(torch_device_ctx, monkeypatch):
    ...
    with ctx:
        ...
        torch.testing.assert_close(
            sorted_values._data.tensor,
            torch.tensor(
                [-1.0, 2.0, 3.0, np.nan],
                device=device,
                dtype=sorted_values._data.tensor.dtype,
            ),
            equal_nan=True,
        )
...
def test_numpy_nonzero_preserves_tuple_and_plain_array_contract(
    ...
    with ctx:
        ...
        assert type(selected) is TimeSeries
        assert selected._data.tensor.device.type == device
        assert selected.delta_t == values.delta_t
        assert selected.start_time == values.start_time
        torch.testing.assert_close(
            selected._data.tensor,
            torch.tensor(
                [2, -3], device=device, dtype=selected._data.tensor.dtype
            ),
        )
...
def test_numpy_matrix_helpers_stay_on_torch_device(
    ...
    with ctx:
        ...
        with monkeypatch.context() as patch:
            ...
            lower = np.tril(batch, k=np.array(1))
            upper = np.triu(m=vector, k=0)
        ...
        expected = (
            ...
            (lower, np.tril(batch_source, k=np.array(1))),
            (upper, np.triu(vector_source, k=0)),
        )
        for actual, wanted in expected:
            assert type(actual) is Array
            assert actual._data.tensor.device.type == device
...
def test_numpy_matrix_helpers_dtype_errors_and_fallback_contract():
    ...
    with scheme.TorchScheme("cpu"):
        ...
        results = (
            ...
            (np.tril(batch, k=0), np.tril(batch_source, k=0)),
            (np.triu(vector, k=1), np.triu(vector_source, k=1)),
        )
...
def test_coherent_chisq_cache_stays_on_torch_device(
    ...
    for value in (cache, dof_cache, missing, remaining, selected, selected_dof):
