...
def _series(dtype):
    ...
    return TimeSeries(data.astype(dtype), delta_t=1 / 1024, epoch=1_000_000_000)


@pytest.mark.parametrize("dtype", (np.float32, np.float64, np.complex64, np.complex128))
@pytest.mark.parametrize("interpolate", (None, "linear", "quadratic"))
@pytest.mark.parametrize("sample_offset", (-0.25, 0.0, 0.25, 17.375, 62.625))
def test_torch_at_time_host_scalar_matches_vector_path(
    ...
    with torch_cpu_ctx:
        ...
        expected = series.at_time(np.asarray([query]), interpolate=interpolate)[0]
...
def test_torch_at_time_host_scalar_returns_owning_tensor(torch_cpu_ctx):
    ...
    torch.testing.assert_close(series._data.tensor, series_before, rtol=0, atol=0)


def test_torch_at_time_host_scalar_avoids_coordinate_tensor(torch_cpu_ctx, monkeypatch):
    with torch_cpu_ctx:
        series = _series(np.complex128)
        query = float(series.start_time) + 21.625 * series.delta_t
        expected = series.at_time(np.asarray([query]), interpolate="quadratic")[0]
...
@pytest.mark.parametrize("coordinate_kind", ("tensor", "vector", "numpy_vector"))
...
def test_torch_at_time_accelerator_host_scalar_uses_existing_path(
    ...
    with scheme.TorchScheme(device_name):
        tensor = torch.sin(torch.arange(64, device=device, dtype=dtype) / 7.0)
        series = TimeSeries(
        ...
        with monkeypatch.context() as patch:
            patch.setattr(
                torch.Tensor,
                "numpy",
                lambda _self: pytest.fail(f"{device_name} storage visited NumPy"),
            )
            actual = series.at_time(query, interpolate="quadratic")
...
def test_torch_at_time_nearest_sample_uses_existing_path(torch_cpu_ctx, monkeypatch):
    with torch_cpu_ctx:
        series = _series(np.float64)
        query = float(series.start_time) + 17.625 * series.delta_t
        expected = series.at_time(np.asarray([query]), nearest_sample=True)[0]

        with monkeypatch.context() as patch:
            patch.setattr(
                torch.Tensor,
                "numpy",
                lambda _self: pytest.fail("nearest-sample lookup visited NumPy"),
            )
...
def test_torch_at_time_host_scalar_errors_match_vector_path(torch_cpu_ctx):
    with torch_cpu_ctx:
        ...
        with pytest.raises(IndexError):
            series.at_time(np.asarray([out_of_bounds]), interpolate="quadratic")
        with pytest.raises(ValueError, match="Unsupported extrapolate"):
            series.at_time(float(series.start_time), extrapolate="unsupported")
    ...
    with torch_cpu_ctx:
        ...
        with monkeypatch.context() as patch:
            patch.setattr(
                torch.Tensor,
                "numpy",
                lambda _self: pytest.fail("gradient-carrying storage visited NumPy"),
            )
            actual = series.at_time(
                query, interpolate="quadratic", extrapolate=extrapolate
            )
            actual_loss = actual.real + 0.25 * actual.imag
            actual_gradient = torch.autograd.grad(actual_loss, data, retain_graph=True)[
                0
            ]
...
def test_torch_at_time_host_scalar_preserves_forward_ad(torch_cpu_ctx, monkeypatch):
    with torch_cpu_ctx:
        with torch.autograd.forward_ad.dual_level():
            ...
            with monkeypatch.context() as patch:
                patch.setattr(
                    torch.Tensor,
                    "numpy",
                    lambda _self: pytest.fail("forward-AD storage visited NumPy"),
                )
                actual = series.at_time(query, interpolate="quadratic")

            actual_primal, actual_tangent = torch.autograd.forward_ad.unpack_dual(
                actual
            )
    ...
    torch.testing.assert_close(actual_primal, expected_primal, rtol=0, atol=0)
    torch.testing.assert_close(actual_tangent, expected_tangent, rtol=0, atol=0)


@pytest.mark.parametrize("storage_kind", ("noncontiguous", "conjugate", "negative"))
    ...
    with torch_cpu_ctx:
        ...
        with monkeypatch.context() as patch:
            patch.setattr(
                torch.Tensor,
                "numpy",
                lambda _self: pytest.fail(f"{storage_kind} storage visited NumPy"),
            )
            actual = series.at_time(query, interpolate="quadratic")
...
def test_torch_at_time_host_scalar_preserves_extrapolation_gradient(torch_cpu_ctx):
    with torch_cpu_ctx:
        series = _series(np.complex128)
        data = series._data.tensor.requires_grad_()
        ...
        expected = series.at_time(np.asarray([query]), extrapolate=2.0)[0]
