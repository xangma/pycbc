...
class TestParams(unittest.TestCase):
    def setUp(self, *args):
        ...
        self.m1 = numpy.random.uniform(1.0, 100.0, size=self.numtests)
        self.m2 = numpy.random.uniform(1.0, 100.0, size=self.numtests)
        ...
        self.spin1_amp = numpy.random.uniform(0.0, 1.0, size=self.numtests)
        rvals = spin_angledist.rvs(size=self.numtests)
        self.spin2_polar = rvals["theta"]
        self.spin2_az = rvals["phi"]
        self.spin2_amp = numpy.random.uniform(0.0, 1.0, size=self.numtests)
        ...
        self.tau0 = conversions.tau0_from_mtotal_eta(
            self.mtotal, self.eta, self.f_lower
        )
        self.tau3 = conversions.tau3_from_mtotal_eta(
            self.mtotal, self.eta, self.f_lower
        )
        self.spin1x, self.spin1y, self.spin1z = coordinates.spherical_to_cartesian(
            self.spin1_amp, self.spin1_az, self.spin1_polar
        )
        self.spin2x, self.spin2y, self.spin2z = coordinates.spherical_to_cartesian(
            self.spin2_amp, self.spin2_az, self.spin2_polar
        )
        self.effective_spin = conversions.chi_eff(
            self.m1, self.m2, self.spin1z, self.spin2z
        )
        self.chi_p = conversions.chi_p(
            self.m1, self.m2, self.spin1x, self.spin1y, self.spin2x, self.spin2y
        )
        self.primary_spinx = conversions.primary_spin(
            self.m1, self.m2, self.spin1x, self.spin2x
        )
        self.primary_spiny = conversions.primary_spin(
            self.m1, self.m2, self.spin1y, self.spin2y
        )
        self.primary_spinz = conversions.primary_spin(
            self.m1, self.m2, self.spin1z, self.spin2z
        )
        self.secondary_spinx = conversions.secondary_spin(
            self.m1, self.m2, self.spin1x, self.spin2x
        )
        self.secondary_spiny = conversions.secondary_spin(
            self.m1, self.m2, self.spin1y, self.spin2y
        )
        self.secondary_spinz = conversions.secondary_spin(
            self.m1, self.m2, self.spin1z, self.spin2z
        )
    ...
    def test_physical_consistency(self):
        ...
        """
        self.assertTrue(
            (self.mp >= self.ms).all(), "primary mass not >= secondary mass"
        )
        self.assertTrue((self.q >= 1.0).all(), "mass ratio not >= 1")
        self.assertTrue((self.invq <= 1.0).all(), "inverse mass ratio not <= 1")
        self.assertTrue((self.eta <= 0.25).all(), "eta not <= 0.25")
        self.assertTrue(
            (abs(self.effective_spin) <= 1.0).all(), "abs(effective spin) not <= 1"
        )
        for which_comp in ["primary", "secondary"]:
            for coord in ["x", "y", "z"]:
                spinparam = "{}_spin{}".format(which_comp, coord)
                self.assertTrue(
                    (abs(getattr(self, spinparam)) <= 1.0).all(),
    ...
    def test_round_robin(self):
        ...
        fchecks = [
            ...
            (
                conversions.tau0_from_mass1_mass2,
                (self.m1, self.m2, self.f_lower),
                "tau0",
            ),
            (
                conversions.tau3_from_mass1_mass2,
                (self.m1, self.m2, self.f_lower),
                "tau3",
            ),
            (conversions.mchirp_from_tau0, (self.tau0, self.f_lower), "mchirp"),
            (
                conversions.mtotal_from_tau0_tau3,
                (self.tau0, self.tau3, self.f_lower),
                "mtotal",
            ),
            (
                conversions.eta_from_tau0_tau3,
                (self.tau0, self.tau3, self.f_lower),
                "eta",
            ),
            (
                conversions.mass1_from_tau0_tau3,
                (self.tau0, self.tau3, self.f_lower),
                "mp",
            ),
            (
                conversions.mass2_from_tau0_tau3,
                (self.tau0, self.tau3, self.f_lower),
                "ms",
            ),
            (
        ...
        for func, args, compval in fchecks:
            passed, maxdiff, maxidx = almost_equal(
                func(*args), getattr(self, compval), self.precision
            )
            if not passed:
    ...
    def test_chip_compare_lalsuite(self):
        ...
        for i in range(len(self.m1)):
            _, _, tmp, _, _, _, _ = (
                lalsim.SimIMRPhenomPCalculateModelParametersFromSourceFrame(
                    self.m1[i] * lal.MSUN_SI,
                    self.m2[i] * lal.MSUN_SI,
                    f_ref,
                    0.0,
                    0.0,
                    self.spin1x[i],
        ...
        self.assertTrue(
            passed, msg.format("conversions.chi_p", "chi_p", maxdiff, failinputs)
        )
        ...
        for ra1, dec1, time1 in vals:
            for ref in ref_frames:
                for target in target_frames:
                    ...
                    conv = conversions.det_tc(target, ra1, dec1, time1, ref_frame=ref)
                    # convert back
                    back = conversions.det_tc(ref, ra1, dec1, conv, ref_frame=target)
    ...
    def test_tidal_conversions(self):
        ...
        scalar_lambda = conversions.lambda_tilde(1.6, 1.2, 300.0, 900.0)
        scalar_delta = conversions.delta_lambda_tilde(1.6, 1.2, 300.0, 900.0)
        scalar_primary = conversions.lambda1_from_delta_lambda_tilde_lambda_tilde(
            scalar_delta, scalar_lambda, 1.6, 1.2
        )
        scalar_secondary = conversions.lambda2_from_delta_lambda_tilde_lambda_tilde(
            scalar_delta, scalar_lambda, 1.6, 1.2
        )
        self.assertTrue(
            all(
                numpy.isscalar(value)
                for value in (
                    scalar_lambda,
                    scalar_delta,
                    scalar_primary,
        ...
        lambda_eff = conversions.lambda_tilde(mass1, mass2, lambda1, lambda2)
        delta_lambda_eff = conversions.delta_lambda_tilde(
            mass1, mass2, lambda1, lambda2
        )
        recovered_primary = conversions.lambda1_from_delta_lambda_tilde_lambda_tilde(
            delta_lambda_eff, lambda_eff, mass1, mass2
        )
        recovered_secondary = conversions.lambda2_from_delta_lambda_tilde_lambda_tilde(
            delta_lambda_eff, lambda_eff, mass1, mass2
        )
