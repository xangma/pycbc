...
class TestPSD(unittest.TestCase):
    ...
    def test_analytical(self):
        ...
        with self.context:
            ...
            for psd_name in psd_list:
                psd = pycbc.psd.analytical.from_string(
                    psd_name, self.psd_len, self.psd_delta_f, self.psd_low_freq_cutoff
                )
                psd_min = psd.min()
                self.assertTrue(psd_min >= 0, msg=(psd_name + ": negative values"))
                self.assertTrue(
                    psd.min() < 1e-40, msg=(psd_name + ": unreasonably high minimum")
                )

    def test_read(self):
        """Test reading PSDs from text files"""
        test_data = numpy.zeros((self.psd_len, 2))
        test_data[:, 0] = numpy.linspace(
            0.0, (self.psd_len - 1) * self.psd_delta_f, self.psd_len
        )
        test_data[:, 1] = numpy.sqrt(test_data[:, 0])
        file_desc, file_name = tempfile.mkstemp()
        os.close(file_desc)
        numpy.savetxt(file_name, test_data)
        test_data[test_data[:, 0] < self.psd_low_freq_cutoff, 1] = 0.0
        with self.context:
    ...
    def test_estimate_welch(self):
        ...
        for seg_len in (2048, 4096, 8192):
            noise_model = (numpy.linspace(1.0, 100.0, seg_len // 2 + 1)) ** (-2)
            ...
            for max_len in (1024, 512, 256):
                with self.context:
                    ...
                    psd_trunc = pycbc.psd.inverse_spectrum_truncation(
                        psd, max_len, low_frequency_cutoff=self.psd_low_freq_cutoff
                    )
