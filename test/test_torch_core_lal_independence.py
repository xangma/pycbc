...
def test_torch_surface_without_core_lal_or_lalsimulation():
    ...
    script = textwrap.dedent(
        ...
        assert times.start_time.gpsSeconds == 0
        assert times.start_time.gpsNanoSeconds == -100_000_000
        assert frequencies.epoch.gpsSeconds == 1
        assert frequencies.epoch.gpsNanoSeconds == 200_000_000
        for value in (
            array,
            times,
            frequencies,
    ...
    script = textwrap.dedent(
        ...
        with TorchScheme("cpu"):
            ...
            try:
                ...
                assert "detector-frame waveform generation" in str(exc)
            else:
                raise AssertionError("detector-frame use accepted without LAL")
