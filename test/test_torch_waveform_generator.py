...
def test_detector_frame_generator_moves_output_to_torch(monkeypatch, device):
    ...
    torch.testing.assert_close(
        result._data.tensor,
        torch.ones(8, device=device, dtype=expected_dtype),
    )


@pytest.mark.parametrize("source_device,target_device", [
    ("cpu", "cuda"), ("cuda", "cpu"), ("cpu", "mps"), ("mps", "cpu"),
])
def test_detector_frame_generator_moves_existing_torch_output(
        monkeypatch, source_device, target_device):
    devices = (source_device, target_device)
    ...
    with scheme.TorchScheme(source_device):
        raw = torch.ones(
            8, device=source_device, dtype=torch.complex64, requires_grad=True
        )
        hp = FrequencySeries(wrap_backend_array(raw), delta_f=0.25, copy=False)
        hc = FrequencySeries(
            wrap_backend_array(1j * raw), delta_f=0.25, copy=False
        )
    ...
    with scheme.TorchScheme(target_device):
        detgen = generator.FDomainDetFrameGenerator(
            CachedGenerator, epoch=0.0, detectors=None, delta_f=0.25
        )
        result = detgen.generate()["RF"]

    converted = backend_array(result, "torch")
    assert converted.device.type == target_device
    assert backend_array(hp, "torch").device.type == source_device
    torch.testing.assert_close(converted.detach().cpu(), raw.detach().cpu())
    converted.real.sum().backward()
