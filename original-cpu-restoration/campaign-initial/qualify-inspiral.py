...
def psd_validity(values, kmin, kmax):
    """Validate the filter's used bins while retaining excluded-band infinities."""
    import numpy

    if values.ndim != 1 or not 0 <= kmin < kmax <= values.size:
        raise ValueError("PSD dimensions or actual filter bounds are invalid")
    used = values[kmin:kmax]
    positive_infinity = numpy.flatnonzero(numpy.isposinf(values))
    result = {
        "filter_bin_start": kmin,
        "filter_bin_stop": kmax,
        "filter_bins_are_half_open": True,
        "used_bins_finite_positive": bool(numpy.all(numpy.isfinite(used) & (used > 0))),
        "nan_count": int(numpy.isnan(values).sum()),
        "negative_infinity_count": int(numpy.isneginf(values).sum()),
        "nonpositive_count": int((values <= 0).sum()),
        "positive_infinity_count": int(positive_infinity.size),
        "positive_infinity_first_bin": int(positive_infinity[0])
        if positive_infinity.size
        else None,
        "positive_infinity_last_bin": int(positive_infinity[-1])
        if positive_infinity.size
        else None,
        "positive_infinity_only_in_excluded_bins": bool(
            numpy.all((positive_infinity < kmin) | (positive_infinity >= kmax))
        ),
        "excluded_infinity_policy": "Positive infinity is allowed only outside the actual filter slice; inverse-spectrum truncation can produce it where the inverse spectrum vanishes.",
    }
    result["valid_for_filter"] = (
        result["used_bins_finite_positive"]
        and result["nan_count"] == 0
        and result["negative_infinity_count"] == 0
        and result["nonpositive_count"] == 0
        and result["positive_infinity_only_in_excluded_bins"]
    )
    return result
...
class Qualification:
    ...
    def observe_psds(self, segments, kmin, kmax):
        import numpy
        ...
        for segment_index, segment in enumerate(segments):
            ...
            record.update(
                ...
                validity=psd_validity(values, kmin, kmax),
    ...
    def install(self):
        ...
        def observed_controller_init(obj, *args, **kwargs):
            ...
            self.observe_psds(obj.segments, int(obj.kmin), int(obj.kmax))
            ...
            record = {
                ...
                "filter_bin_start": int(obj.kmin),
                "filter_bin_stop": int(obj.kmax),
    ...
    def checks(self):
        ...
        checks["all_segment_psds_saved"] = bool(psds) and sorted(
            i for row in psds for i in row["segment_indices"]
        ) == list(range(segments))
        checks["all_segment_psds_valid_for_filter"] = bool(psds) and all(
            row["validity"]["valid_for_filter"] for row in psds
        )
