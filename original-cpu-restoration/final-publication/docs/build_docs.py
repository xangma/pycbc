...
WORKTREE = Path('/private/tmp/pycbc-original-cpu-restoration-docs-20260908')
RUNTIME = Path('/private/tmp/pycbc-original-cpu-docs-runtime-20260908')
FROZEN = 'aa6b795a63bb18c4e63e4f4c203ca6e7c039d0f0'
DOC_BASE = '1d22031fd31c6e5bcb48a68fe11772e960bd406b'
...
assert not subprocess.check_output(['git', '-C', str(WORKTREE), 'diff', '--name-only', DOC_BASE, FROZEN, '--', 'docs'], text=True)
...
# Generate only the repository's version metadata, without setup/install/build.
version_command = [sys.executable, '-c', "import ast; from pathlib import Path; tree=ast.parse(Path('setup.py').read_text()); fn=next(n for n in tree.body if isinstance(n, ast.FunctionDef) and n.name=='get_version_info'); code=ast.Module(body=[fn],type_ignores=[]); ns={}; exec(compile(code, 'setup.py', 'exec'),ns); ns['get_version_info']()"]
subprocess.run(version_command, cwd=RUNTIME, env=env, check=True)
...
result = dict(command=command, returncode=code, pages=pages, runtime=provenance,
              ...
              source_mapping='Documentation base is byte-identical under docs/ to measured aa6 source before these six documentation edits. Runtime imports measured aa6 source directly. Later published-source formatting mapping is a separate receipt.',
