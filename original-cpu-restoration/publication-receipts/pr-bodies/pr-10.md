Provide shared waveform operations and dispatch for Torch-backed data.
...
Waveform callers need consistent dispatch and shared operations for Torch-backed data, including explicit fallback behavior for unsupported generators.

## Contents

Waveform registration and mode dispatch, spherical harmonics, ringdown, decompression and utility adapters. The phase-shift primitive already introduced by filtering is reused.

Torch CPU linear decompression reuses the compiled implementation through shared array views when its contract is satisfied, preserving existing output and fallback behavior. Ordinary CPU waveform caches and decompression behavior are preserved.

## Links to any issues or associated PRs

Depends on [#9](https://github.com/xangma/pycbc/pull/9). PR #20 is withdrawn from this stack.

Base: `torch-pr5-search`. Head: `torch-pr6-waveform` at `1bd71a607336a2455892134762f45192f4c92a27`.
