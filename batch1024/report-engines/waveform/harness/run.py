...
from worker import ROUTES, THREAD_VARS
...
def main():
    ...
    p.add_argument("--batches", nargs="+", type=int, default=[1, 8, 32, 128, 512, 1024])
    ...
    if a.sample_ms <= 0 or a.timeout <= 0:
        p.error("sample-ms and timeout must be positive")
    if any(len(values) != len(set(values)) for values in (a.batches, a.threads, a.routes, a.precisions)):
        p.error("route, batch, precision and thread lists must contain no duplicates")
    if any("cuda" in route for route in a.routes) and 1 not in a.threads:
        p.error("CUDA routes require a one-host-thread phase")
    a.out = a.out.resolve()
    ...
    manifest = {"schema": 1, "command": sys.argv, "cwd": os.getcwd(), "pid": os.getpid(),
                ...
                "conditions": {"batches": a.batches, "threads": a.threads, "routes": a.routes,
                               "precisions": a.precisions, "replicates": a.replicates,
                               "samples": a.samples, "sample_ms": a.sample_ms, "max_inner": a.max_inner},
                "jobs": []}
    ...
    for threads, route, batch, precision in itertools.product(a.threads, a.routes, a.batches, a.precisions):
        if "cuda" in route and threads != 1:
            continue
        ...
        for rep in range(1, a.replicates + 1):
            ...
            with log.open("w") as stream:
                ...
                try:
                    ...
                    os.killpg(active.pid, signal.SIGKILL)
                    active.wait()
                    job["returncode"], job["timeout"] = active.returncode, True
                active = None
