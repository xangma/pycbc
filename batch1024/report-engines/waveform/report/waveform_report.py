...
Usage: waveform_report.py --input-dir FULLRUN --output-dir REPORT --expected-sha SHA
...
CUDA_ROUTES = ("torch-cuda-scalar", "torch-cuda-batch")
...
def verify_worker(raw, job, cell, manifest, expected_sha):
    ...
    if source.get("sha") != expected_sha or source.get("clean") is not True or source.get("status_porcelain") != "":
    ...
    if status != "ok":
        ...
        if status == "unsupported" and precision == "single":
    ...
    if precision != "double":
        errors.append("unsupported precision reported successful timing")
    if raw.get("environment", {}).get("PYCBC_TAYLORF2_TRITON") != "0":
        errors.append("baseline did not explicitly disable the optional Triton evaluator")
...
def verify_campaign(input_dir, expected_sha, harness_dir):
    ...
    global_errors = []
    conditions = dict(batches=list(BATCHES), threads=[1, 4],
                      routes=list(CPU_ROUTES + CUDA_ROUTES),
                      precisions=["double", "single"], replicates=3,
                      samples=5, sample_ms=50, max_inner=64)
    if manifest.get("schema") != 1 or manifest.get("conditions") != conditions:
        global_errors.append("manifest does not describe the complete 288-worker matrix")
    if manifest.get("interrupted_signal"):
        global_errors.append("campaign was interrupted")
    for name, filename in (("worker_sha256", "worker.py"), ("orchestrator_sha256", "run.py")):
        if manifest.get(name) != digest(harness_dir / filename):
            global_errors.append(f"{filename} hash differs from the supplied archived harness")
    ...
    if set(jobs_by_key) != expected_keys or len(manifest.get("jobs", [])) != 288:
        global_errors.append("manifest must contain all 288 jobs in the fixed full-run matrix")
    ...
    for cell in expected_cells():
        ...
        for job in jobs:
            ...
            record_errors, median = verify_worker(raw, job, cell, manifest, expected_sha)
    ...
    for batch, rows in parameters_by_batch.items():
        if max(BATCHES) in parameters_by_batch and rows != parameters_by_batch[max(BATCHES)][:batch]:
    ...
    timed_cells = sum(c["eligible"] and c["precision"] == "double" for c in cells.values())
    unsupported_cells = sum(c["status"] == "unsupported" and c["precision"] == "single" for c in cells.values())
    return {"schema": 1, "source_sha": expected_sha, "input_dir": str(input_dir),
            "verification_passed": verification_passed,
            "qualification_passed": verification_passed and timed_cells == 48 and unsupported_cells == 48,
            "expected_worker_count": 288, "raw_worker_files": len(raw_hashes),
            "expected_timed_cells": 48, "qualified_timed_cells": timed_cells,
            "expected_unsupported_cells": 48, "verified_unsupported_cells": unsupported_cells,
            "harness_dir": str(harness_dir),
            "harness_sha256": {name: digest(harness_dir / name) for name in ("run.py", "worker.py")},
...
def markdown(report):
    lines = ["# TaylorF2 public API benchmark", "", f"Source: `{report['source_sha']}`.", "",
             f"Independent record verification: **{'passed' if report['verification_passed'] else 'failed'}**. "
             f"Full campaign qualification: **{'passed' if report['qualification_passed'] else 'failed'}**. "
             f"{report['qualified_timed_cells']}/48 double precision cells and "
             f"{report['verified_unsupported_cells']}/48 unsupported single precision cells; "
             f"{report['raw_worker_files']}/288 worker files.", "",
    ...
    lines += ["", "## Unsupported, failed or unverified cells", "",
              ...
              "The public CPU/CUDA API at this source SHA selects complex128 internally; it exposes no single precision selector. Optional Triton is disabled here and measured in a separate campaign. "
...
def plot(report, output_dir):
    ...
    with plt.rc_context({"font.family": "DejaVu Sans", "font.size": 10,
        ...
        fig.text(0.065, 0.925, f"PyCBC {report['source_sha'][:12]}  •  complex128  •  4097 frequency bins  •  both polarizations", fontsize=10.5)
        ...
        fig.text(0.065, 0.035, "Median of 3 independent process medians × 5 samples. Whiskers: observed replicate min/max, not confidence intervals.\n"
                 "Cold calls are reported separately. Unsupported single precision requests have no plotted numeric values; Triton is measured separately.", fontsize=9, color="#52616B")
...
def main():
    ...
    parser.add_argument("--harness-dir", type=Path, default=Path(__file__).resolve().parent.parent / "harness")
    parser.add_argument("--tables-only", action="store_true")
    args = parser.parse_args()
    input_dir, output_dir = args.input_dir.resolve(), args.output_dir.resolve()
    report = verify_campaign(input_dir, args.expected_sha, args.harness_dir.resolve())
    ...
    print(json.dumps({"verification_passed": report["verification_passed"],
                      ...
                      "timed_cells": report["qualified_timed_cells"],
                      "unsupported_cells": report["verified_unsupported_cells"],
