...
def load_campaign(path, head, threads, revision, run_date):
    ...
    expected_keys = {
        (batch, route, replicate)
        for batch in BATCHES
        for route in required_routes
        for replicate in range(3)
    }
    observed_keys = [
        (record["batch"], record["route"], record["replicate"])
        for record in data["records"]
    ]
    assert len(set(observed_keys)) == len(observed_keys), "Duplicate worker cell"
    assert set(observed_keys) <= expected_keys, "Unexpected worker cell"
...
def main():
    ...
    return 0 if summary["qualified_rows"] == summary["expected_rows"] else 1
