#!/usr/bin/env python3
"""Summarize 60 untimed dispatch probes without importing benchmark code.
...
PROBE_SHA256 = hashlib.sha256(
    Path(__file__).with_name("live_dispatch_probe.py").read_bytes()
).hexdigest()
...
def load_row(path, cell):
    ...
    try:
        ...
        data = {}
        issues.append(f"Invalid probe JSON: {exc}")
    checks = {
        ...
        "source was not checked after probe": data.get("source_unchanged_after")
        is True,
        ...
        "full logical batch or FFT allocations not verified": data.get(
            "batch_structure"
        )
        == [
            {
                "chunks": [batch],
                "template_group_sizes": [batch],
                "fft_output_elements": [batch * 131072],
                "fft_input_elements": [batch * 131072],
            }
        ],
    }
    ...
    return {
        ...
        "batch_structure": data.get("batch_structure"),
...
def main():
    ...
    REVISIONS.update(main=args.main_revision, cpu=args.cpu_revision)
    rows = []
    configurations = {}
    ...
    return 0 if summary["qualified_rows"] == len(expected_cells()) else 1
