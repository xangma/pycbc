...
before = source_info()
assert before == dict(commit=json.loads((root / 'source-v3.json').read_text())['commit'], status='')
tests = [
...
inputs = [Path(__file__), root / 'config.json', source / 'bin/pycbc_inspiral',
          source / 'pycbc/scheme.py', root / 'source-v3.json',
...
with (root / 'unit-tests-v3.log').open('x') as log:
...
record.update(returncode=code, finished_utc=utc(),
              ...
              log_sha256=digest(root / 'unit-tests-v3.log'))
