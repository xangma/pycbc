...
output = root / 'inputs/bank-compressed.hdf'
pilot = root / 'inputs/bank-pilot-compressed.hdf'
assert not output.exists() and not pilot.exists()
assert metadata['longest']['duration_seconds'] + 16 <= 112
command = ['/usr/bin/time', '-v', '-o', str(root / 'compression-time.txt'),
           ...
           '--precision', 'single', '--tolerance', '0.0001', '--nprocesses', '1',
...
with open(root / 'compression.log', 'w') as log:
