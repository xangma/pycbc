...
Usage: python build-reference-report-v3.py --output-dir final-report
Requires the six final plans/status ledgers, precision-reference-tuning-decision.json,
profiles-precision.json, all named runs, and the two final scientific validation receipts.
...
PARENT_COMMIT = '6c82155044d58f3344b281869d87745f71ba2285'
...
PLANS = ('precision-reference-qualifications-plan', 'precision-reference-timings-plan',
         'precision-final-qualifications-plan', 'precision-reference-profiles-plan',
         'precision-matched-backends-plan', 'precision-torch-profiles-plan')
...
def plan_cases(selected):
    ...
    return {
        PLANS[0]: {f'qual-precision-cpu-l{length}': ('qualify', 'cpu:1', length)
                   for length in (256, 512, 1024)},
        PLANS[1]: {f'tune-precision-cpu-l{length}-r{rep}': ('timing', 'cpu:1', length)
                   for length in (256, 512, 1024) for rep in (1, 2, 3)},
        PLANS[2]: {f'qual-selected-{label}-l{selected}': ('qualify', scheme, selected)
                   for label, scheme in zip(('cpu', 'torch-cpu', 'torch-cuda'), SCHEMES)},
        PLANS[3]: {f'reference-precision-cpu-l{selected}-{mode}': (mode, 'cpu:1', selected)
                   for mode in ('cprofile', 'perf')},
        PLANS[4]: {f'matched-precision-{label}-l{selected}-r{rep}': ('timing', scheme, selected)
                   for label, scheme in zip(('cpu', 'torch-cpu', 'torch-cuda'), SCHEMES)
                   for rep in (1, 2, 3)},
        PLANS[5]: {
            **{f'profile-precision-torch-{device}-l{selected}-{mode}': (mode, scheme, selected)
               for device, scheme in zip(('cpu', 'cuda'), SCHEMES[1:])
               for mode in ('cprofile', 'perf')},
            f'profile-precision-torch-cuda-l{selected}-torchprofile':
...
class Evidence:
    ...
    def recorded_inputs(self, before, after=None):
        ...
        for name, digest in before.items():
            ...
            if relative is not None and not relative.startswith(('source/', 'source-v2/', 'source-v3/', 'source-v4/')):
...
def validate_science(evidence, qualification_cases, bank_hashes):
    waveform = evidence.read('waveform-validation-precision.json')
    boundary = evidence.read('boundary-injections-precision.json')
    ...
    required_inputs(evidence, waveform['input_sha256'],
                    [f'runs/{case}/qualification.json' for case in qualification_cases])
    boundary_qualification = 'runs/qual-precision-cpu-l1024/qualification.json'
...
def profiles(evidence, selected, receipts):
    summary = evidence.read('profiles-precision.json')
...
def read_source(evidence):
    source = evidence.read('source-v4.json')
    source_path = Path(source['source'])
    require(source_path.is_absolute() and source_path.name == 'source-v4' and
    ...
    parent = evidence.read('source-v3.json')
    require(parent['commit'] == PARENT_COMMIT and parent['parent'] == PREVIOUS_COMMIT and
            parent['source'] == str(evidence.remote / 'source-v3') and
            parent['changed_paths'] == list(CHANGED_PATHS), 'Wrong precision source parent')
    native = source['native_modules_sha256']
    require(len(native) == 11 and
            native == previous['native_modules_sha256'] == parent['native_modules_sha256'] and
    ...
    require(set(source['changed_files_sha256']) == set(CHANGED_PATHS) and
            all(re.fullmatch('[0-9a-f]{64}', digest)
    ...
    require(all(source['changed_files_sha256'][name] == parent['changed_files_sha256'][name]
                for name in CHANGED_PATHS if name != 'test/test_sigmasq_series_precision.py'),
            'Final revision changes runtime files beyond the reviewed precision source')
    for name, record in (('inspiral-source-v2.bundle', previous),
                         ('inspiral-source-v3.bundle', parent),
                         ('inspiral-source-v4.bundle', source)):
        evidence.bind(name, record['bundle_sha256'])
    proof = evidence.read('source-precision-provenance-v4.json')
    expected_checks = ('only_reviewed_python_and_test_paths_changed',
                       'native_hashes_equal', 'both_sources_clean',
                       'all_final_runs_require_new_source', 'tests_only_parent_delta')
    ...
    required_inputs(evidence, proof['input_sha256'], (
        'setup-source-v4.py', 'source-v2.json', 'source-v4.json',
        'source-v3.json', 'inspiral-source-v2.bundle', 'inspiral-source-v3.bundle',
        'inspiral-source-v4.bundle'))
...
def read_unit_tests(evidence, source, config):
    receipt = evidence.read('unit-tests-v4.json')
    ...
    required_inputs(evidence, receipt['input_sha256'],
                    ('run-unit-checks-v4.py', 'config.json', 'source-v4.json'))
    ...
    log = evidence.bind('unit-tests-v4.log', receipt['log_sha256'])
    ...
    return dict(receipt='unit-tests-v4.json', log='unit-tests-v4.log',
...
def read_decision(evidence):
    decision = evidence.read('precision-reference-tuning-decision.json')
...
def collect_qualifications(evidence, receipts, data, bank_hashes, config, comparator, selected):
    ...
    duplicate = {f'qual-precision-cpu-l{selected}', f'qual-selected-cpu-l{selected}'}
    ...
    final = f'qual-selected-cpu-l{selected}'
...
def validate_decision(evidence, decision, selected, tuning_rows, matched_rows):
    ...
    for item in grid:
        ...
        values = [rows[f'tune-precision-cpu-l{length}-r{rep}']['wall_seconds']
                  for rep in (1, 2, 3)]
...
def build(evidence):
    ...
    for length in (256, 512, 1024):
        ...
        baseline = (f'qual-selected-cpu-l{length}' if length == selected[0]
                    else f'qual-precision-cpu-l{length}')
    ...
    return dict(schema_version=1, status='pass', gates=evidence.gates,
                ...
                source_provenance=dict(receipt='source-precision-provenance-v4.json',
