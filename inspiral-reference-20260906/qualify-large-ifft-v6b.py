...
class Route:
    def __init__(self, name, size, nthreads):
        ...
        if name.startswith("legacy_"):
            ...
            with scheme.CPUScheme(num_threads=nthreads):
                ...
                # Array.data/numpy() outside this CPUScheme can migrate an
                # Array to DefaultScheme and replace its storage. Native plans
                # cache the original pointers, so retain and use those buffers
                # directly for their entire lifetime.
                self.source_buffer = self.source.data
                self.target_buffer = self.target.data
            self.assign = lambda values: self.source_buffer.__setitem__(
                slice(None), values
            )
            self.output = lambda: self.target_buffer.copy()
            self.input = lambda: self.source_buffer.copy()
    ...
    def check_buffers(self):
        if self.name.startswith("legacy_"):
            valid = (
                self.source._data is self.source_buffer
                and self.target._data is self.target_buffer
                and self.owner.iptr == self.source_buffer.ctypes.data
                and self.owner.optr == self.target_buffer.ctypes.data
            )
        elif self.name == "current_torch_dispatch":
            valid = (
                self.owner.invec._data.tensor is self.source
                and self.owner.outvec._data.tensor is self.target
            )
        elif self.name.startswith("mkl_"):
            valid = self.owner.can_execute(self.source, self.target)
        else:
            valid = True
        if not valid:
            raise RuntimeError(f"{self.name} changed its bound buffers or plan state")

    def checked_output(self, values):
        self.check_buffers()
        actual_input = self.input()
        expected_input = values.astype(actual_input.dtype, copy=False)
        if not np.array_equal(
            actual_input.view(np.uint8), expected_input.view(np.uint8)
        ):
            raise RuntimeError(f"{self.name} mutated its source")
        output = self.output()
        if not np.all(np.isfinite(output)):
            raise RuntimeError(f"{self.name} produced nonfinite output")
        return output
...
def timed_route(name, size, threads, repeats, values):
    ...
    output = route.checked_output(values)
    ...
    result = {
        ...
        "timing_validation_errors": error_metrics(output, truth),
        "bound_buffers_verified": True,
        "input_storage_preserved": True,
    }
...
def main():
    ...
    result = {
        "schema": "torch-large-ifft-qualification-v6b",
        "state": "running",
                    ...
                    for pattern in ("dense", "banded", "impulse"):
                        for scale in args.scales:
                            ...
                            for name, route in routes.items():
                                ...
                                route.check_buffers()
                                route.execute()
                                outputs[name] = route.checked_output(values)
