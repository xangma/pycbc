...
def native_profile(path, top):
    ...
    column_names = re.split(r"\s{2,}", columns.lstrip()[1:].strip())
    has_samples, has_period = "Samples" in column_names, "Period" in column_names
    required_columns = ["Overhead"] + (["Samples"] if has_samples else []) + (["Period"] if has_period else []) + ["Command", "Shared Object", "Symbol"]
    if column_names[:len(required_columns)] != required_columns or len(set(column_names)) != len(column_names):
        raise ValueError("Require distinct perf columns ordered Overhead,[Samples],[Period],Command,Shared Object,Symbol")
    extra_names = column_names[len(required_columns):]
    rows = []
    for line in content.splitlines():
        ...
        symbol, extras = fields[2], []
        if extra_names:
            # Split trailing fields from the right to preserve whitespace within
            # demangled symbols. Header padding does not match the data widths.
            trailing = list(re.finditer(r"\s{2,}", symbol))[-len(extra_names):]
            if len(trailing) != len(extra_names):
                raise ValueError(f"Missing named extra perf columns: {line}")
            extras = [symbol[gap.end():following.start()].strip() for gap, following in zip(trailing, trailing[1:])]
            extras.append(symbol[trailing[-1].end():].strip())
            symbol = symbol[:trailing[0].start()].rstrip()
        if not symbol or any(not value for value in extras):
            raise ValueError(f"Empty perf symbol or extra column: {line}")
        row.update(command=fields[0], shared_object=fields[1], symbol=symbol,
                   extra_columns=dict(zip(extra_names, extras)), classification=native_label(symbol))
        rows.append(row)
    if not rows:
    ...
    return dict(input=identity(path), event=event, samples_header=sample_text,
                ...
                native_functions=rows, top_native_functions=rows[:top], column_names=column_names, headers=headers,
