...
def main():
    ...
    try:
        ...
        def observed_init(obj, *a, **kw):
            ...
            def observed_filter(segnum, *fa, **fk):
                ...
                if context.get('template_hash') == args.template_hash:
                    target_segments_seen.append(int(segnum))
        ...
        def observed_values(obj, corr, snrv, snr_norm, psd, indices, template):
            ...
            if context.get('template_hash') == args.template_hash and context['segment']['number'] == args.segment:
        ...
        assert code == 0, code
        record['target_segments_seen'] = target_segments_seen
        assert sorted(target_segments_seen) == list(range(5)), target_segments_seen
        assert len(record['captures']) == 1, len(record['captures'])
