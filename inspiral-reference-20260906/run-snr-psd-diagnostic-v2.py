...
def main():
    ...
    inputs = [Path(__file__).resolve(), root / 'diagnose-snr-psd-v2.py', root / 'config.json', root / 'source-v2.json', root / 'inputs/bank-compressed-1e5.hdf', root / 'chisq-input-capture-cpu/conditioned-strain.npy', root / 'chisq-input-capture-cpu/capture-3-corr.npy', root / 'chisq-input-capture-cpu/status.json']
    ...
    inputs += [source / path for path in ('pycbc/psd/estimate.py', 'pycbc/filter/matchedfilter.py', 'pycbc/waveform/decompress_torch.py', 'pycbc/scheme.py', 'pycbc/fft/torchfft.py', 'pycbc/strain/strain.py', 'pycbc/filter/matchedfilter_cpu.pyx')]
    inputs += [root / 'diagnose-snr-psd.py', root / 'chisq-input-capture-cuda/status.json']
    assert all(p.is_file() for p in inputs), [str(p) for p in inputs if not p.is_file()]
    hashes = {str(p): digest(p) for p in inputs}
    out = root / 'snr-psd-diagnostic-v2'
    assert not out.exists()
    status = root / 'snr-psd-diagnostic-v2.status.json'
    assert not status.exists()
    command = ['taskset', '-c', str(cfg['core']), sys.executable, '-u', str(root / 'diagnose-snr-psd-v2.py'), '--root', str(root), '--output-dir', str(out), '--segment', '3', '--indices', '1884341', '--forward-data']
