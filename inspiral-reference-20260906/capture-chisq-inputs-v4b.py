...
def main():
    ...
    assert frozen['source_info']['commit'] == 'f2c0abe61e787a26f41208f489c62c877bbd5667'
    ...
    manifest_path = root / 'source-v4.json'
    manifest = json.loads(manifest_path.read_text())
    assert manifest['commit'] == frozen['source_info']['commit']
    inputs = dict(frozen['input_sha256'])
    inputs[str(manifest_path)] = digest(manifest_path)
    inputs.update({str(source / p): h for p, h in manifest['changed_files_sha256'].items()})
    ...
    record = dict(state='running', started_utc=utc(), pid=os.getpid(), hostname=os.uname().nodename, cwd=str(root), source_info=frozen['source_info'], input_sha256=inputs, executable_cli=cli, template_hash=args.template_hash, captures=[], observed_filter_segments=[])
    ...
    try:
        ...
        def observed_init(obj, *a, **kw):
            ...
            def observed_filter(segnum, *fa, **fk):
                ...
                if context.get('template_hash') == args.template_hash:
                    record['observed_filter_segments'].append(int(segnum))
        ...
        assert code == 0, code
        assert record['observed_filter_segments'] == list(range(5)), record['observed_filter_segments']
        # Chi-squared is only called for segments with surviving SNR points.
        assert 0 < len(record['captures']) <= 5, len(record['captures'])
        assert len({r['segment']['number'] for r in record['captures']}) == len(record['captures'])
