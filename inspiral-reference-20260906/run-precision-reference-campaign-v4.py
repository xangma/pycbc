...
status = root / 'precision-reference-campaign.status.json'
assert not status.exists()
for path in root.glob('*.status.json'):
    assert json.loads(path.read_text()).get('state') != 'running', path
cfg = json.loads((root / 'config.json').read_text())
fixed_env = dict(cfg['environment'], PYTHONPATH=str(root / 'source-v4'), PYTHONDONTWRITEBYTECODE='1')
...
common = ['--bank-metadata', str(root / 'inputs/bank-metadata.json'),
          ...
          '--source', str(root / 'source-v4')]
qualifications = [root / 'runs' / f'qual-precision-cpu-l{length}' / 'qualification.json'
                  for length in (256, 512, 1024)]
commands = [
    ('waveform-validation-precision', ['taskset', '-c', str(cfg['core']), sys.executable,
      '-u', str(root / 'validate-waveforms.py'), *common,
      '--output', str(root / 'waveform-validation-precision.json'),
      *[arg for path in qualifications for arg in ('--qualification', str(path))]]),
    ('boundary-injections-precision', ['taskset', '-c', str(cfg['core']), sys.executable,
      '-u', str(root / 'check-boundary-injections.py'), *common,
      '--output', str(root / 'boundary-injections-precision.json'),
      '--qualification', str(qualifications[2])]),
    ('precision-reference-timings', [sys.executable, '-u', str(root / 'run-series-v4.py'),
                         str(root / 'precision-reference-timings-plan.json')]),
]
inputs = [Path(__file__), *[root / name for name in
          ('source-v4.json', 'run-series-v4.py', 'run-case.py',
           'qualify-inspiral.py', 'validate-waveforms.py', 'check-boundary-injections.py',
           'precision-reference-qualifications-plan.json', 'precision-reference-timings-plan.json', 'config.json')]]
