...
fixed_env = dict(cfg['environment'], PYTHONPATH=str(root / 'source-v3'), PYTHONDONTWRITEBYTECODE='1')
...
commands = [
    ...
    ('precision-reference-timings', [sys.executable, '-u', str(root / 'run-series-v3.py'),
                         str(root / 'precision-reference-timings-plan.json')]),
]
inputs = [Path(__file__), *[root / name for name in
          ('source-v3.json', 'run-series-v3.py', 'run-case.py',
