...
before = source_info()
assert before == dict(commit=json.loads((root / 'source-v4.json').read_text())['commit'], status='')
tests = [
...
inputs = [Path(__file__), root / 'config.json', source / 'bin/pycbc_inspiral',
          source / 'pycbc/scheme.py', root / 'source-v4.json',
          *[source / p for p in ['pycbc/filter/matchedfilter.py', 'pycbc/vetoes/chisq.py',
...
with (root / 'unit-tests-v4.log').open('x') as log:
...
record.update(returncode=code, finished_utc=utc(),
              ...
              log_sha256=digest(root / 'unit-tests-v4.log'))
