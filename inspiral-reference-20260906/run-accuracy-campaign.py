#!/usr/bin/env python3
"""Qualify tighter compression before timing it; retain all earlier evidence."""
import datetime
import hashlib
import json
import os
...
status = root / 'accuracy-campaign.status.json'
assert not status.exists()
for path in root.glob('*.status.json'):
    assert json.loads(path.read_text()).get('state') != 'running', path
cfg = json.loads((root / 'config.json').read_text())
fixed_env = dict(cfg['environment'], PYTHONPATH=str(root / 'source'), PYTHONDONTWRITEBYTECODE='1')
...
qualifications = [root / 'runs' / f'qual-1e5-cpu-l{length}' / 'qualification.json'
                  for length in (256, 512, 1024)]
commands = [
    ('compression-1e5', [sys.executable, '-u', str(root / 'compress-bank-1e5.py')]),
    ('accuracy-qualifications', [sys.executable, '-u', str(root / 'run-series-1e5.py'),
                               str(root / 'accuracy-qualifications-plan.json')]),
    ('waveform-validation-1e5', ['taskset', '-c', str(cfg['core']), sys.executable,
      '-u', str(root / 'validate-waveforms.py'), *common,
      '--output', str(root / 'waveform-validation-1e5.json'),
      *[arg for path in qualifications for arg in ('--qualification', str(path))]]),
    ('boundary-injections-1e5', ['taskset', '-c', str(cfg['core']), sys.executable,
      '-u', str(root / 'check-boundary-injections.py'), *common,
      '--output', str(root / 'boundary-injections-1e5.json'),
      '--qualification', str(qualifications[2])]),
    ('accuracy-timings', [sys.executable, '-u', str(root / 'run-series-1e5.py'),
                         str(root / 'accuracy-timings-plan.json')]),
]
inputs = [Path(__file__), *[root / name for name in
          ('compress-bank-1e5.py', 'run-series-1e5.py', 'run-case.py',
           'qualify-inspiral.py', 'validate-waveforms.py', 'check-boundary-injections.py',
           'accuracy-qualifications-plan.json', 'accuracy-timings-plan.json', 'config.json')]]
