...
def pins():
    ...
    for name, expected in read(ROOT / 'science-manifest-v3.json').items():
...
def negative_probe(lock, checker):
    """Validate the already completed, checksum-pinned adversarial run."""
    for name, expected in read(ROOT / 'negative-reuse-pins.json').items():
        assert sha(ROOT / name) == expected, name
    row = read(ROOT / 'negative-result.json')
    folder = ROOT / 'runs/cuda-negative'
    q = read(folder / 'qualification.json')
    assert q['source_root'] == str(SOURCE)
    assert q['executable']['sha256'] == sha(SOURCE / 'bin/pycbc_inspiral')
    graph = q['observations']['offline_graph']
    assert row['returncode'] != 0 and q['status'] == 'failed'
    assert row['qualification_sha256'] == sha(folder / 'qualification.json')
    assert row['log_sha256'] == sha(folder / 'stderr.log')
    assert q['error']['type'] == 'AssertionError'
    assert q['error']['message'] == 'Full correlation/SNR byte mismatch'
    assert graph['replays'] == row['replays'] == 6
    assert len(graph['calls']) == row['completed_comparisons'] == 5
    assert len(graph['captures']) == row['captures'] == 5
    row.update(detected_suppressed_replay=True, reused=True,
               original_result_sha256=sha(ROOT / 'negative-result.json'),
               detection_error=q['error'])
    save(ROOT / 'negative-result-v3.json', row)
    return row


def main():
    status_path = ROOT / 'science-status-v3.json'
