...
def pins():
    ...
    for name, expected in read(ROOT / 'science-manifest-v2.json').items():
...
def compare(name, left, right, exact):
    ...
    baseline = comparator.load(left / 'triggers.hdf')
    candidate = comparator.load(right / 'triggers.hdf')
    raw = comparator.compare(baseline, candidate, comparator.DEFAULTS)
    # The unmodified comparator intentionally requires the same source tree.
    # Preserve that verdict, then allow only independently pinned provenance
    # differences. Numerical arrays, timing identities and budgets stay intact.
    save(ROOT / ('v2-' + name + '-raw.json'), raw)
    expected_inputs = read(ROOT / 'input-pins.json')
    executable_sha = read(ROOT / 'source-pins.json')['tracked_files']['bin/pycbc_inspiral']
    refs = read(ROOT / 'science-reference-pins.json')
    for folder, loaded in ((left, baseline), (right, candidate)):
        receipt = read(folder / 'receipt.json')
        source = receipt['source_info']
        assert source['status'] == source['tracked_diff'] == ''
        if folder.parent == ROOT / 'runs':
            assert source == source_record()
        else:
            reference, = [r for r in refs.values() if Path(r['remote']) == folder]
            assert sha(folder / 'receipt.json') == reference['sha256']['receipt.json']
        assert not loaded['metadata']['issues']
        executable = receipt['executable_cli'][0]
        assert loaded['metadata']['consumed_input_sha256'] == dict(expected_inputs, **{executable: executable_sha})
    fixed = copy.deepcopy(candidate)
    substitutions = []
    for key in ('source_snapshot', 'consumed_input_sha256'):
        before = fixed['metadata'][key]
        if before == baseline['metadata'][key]:
            continue
        if key == 'consumed_input_sha256':
            old = read(right / 'receipt.json')['executable_cli'][0]
            new = read(left / 'receipt.json')['executable_cli'][0]
            renamed = dict(before)
            assert old != new and renamed[old] == baseline['metadata'][key][new] == executable_sha
            renamed[new] = renamed.pop(old)
            assert renamed == baseline['metadata'][key]
        substitutions.append(dict(field=key, before=before, after=baseline['metadata'][key]))
        fixed['metadata'][key] = copy.deepcopy(baseline['metadata'][key])
    ...
    path = ROOT / ('v2-' + name + '.json')
    save(path, dict(result=result, raw_result=raw, provenance_substitutions=substitutions,
                    tolerances=comparator.DEFAULTS, exact_H1=certificates))
...
def negative_probe(lock, checker):
    """Replay observer still counts calls; numerical replay is suppressed."""
    folder = ROOT / 'runs' / 'cuda-negative'
    folder.mkdir(parents=True, exist_ok=False)
    cfg = read(ROOT / 'config-graph.json')
    cli = [str(SOURCE / 'bin/pycbc_inspiral'), *cfg['common_args'],
           '--bank-file', BANK, '--processing-scheme', 'torch:cuda:0',
    ...
    command = ['taskset', '-c', '8', PYTHON, str(ROOT / 'checked-inspiral.py'),
               '--receipt', str(folder / 'runtime.json'), '--config', str(ROOT / 'config-graph.json'),
               '--source', str(SOURCE), '--scheme', 'torch:cuda:0', '--lock-fd', str(lock.fileno()),
               '--', str(ROOT / 'qualify_production_graph_negative.py'),
               '--receipt', str(folder / 'qualification.json'), '--', *cli]
    row = dict(command=command, cwd=str(ROOT), log=str(folder / 'stderr.log'), started=time.time())
    with (folder / 'stderr.log').open('x') as log:
        child = subprocess.Popen(command, cwd=ROOT, env=checker.clean_environment(os.environ, cfg, SOURCE),
                                 stdin=subprocess.DEVNULL, stdout=log, stderr=subprocess.STDOUT,
                                 pass_fds=(lock.fileno(),))
        row['pid'] = child.pid
        save(ROOT / 'negative-launch.json', row)
        try:
            row['returncode'] = child.wait(timeout=300)
        finally:
            if child.poll() is None:
                child.kill()
                child.wait()
    row['finished'] = time.time()
    row['log_sha256'] = sha(folder / 'stderr.log')
    q = read(folder / 'qualification.json')
    graph = q['observations']['offline_graph']
    row.update(replays=graph['replays'], completed_comparisons=len(graph['calls']),
               captures=len(graph['captures']), qualification_sha256=sha(folder / 'qualification.json'))
    row['detected_suppressed_replay'] = (
        row['returncode'] != 0 and q['status'] != 'success'
        and 'Full correlation/SNR byte mismatch' in (folder / 'stderr.log').read_text()
        and row['replays'] == 6 and row['completed_comparisons'] == row['captures'] == 5)
    save(ROOT / 'negative-result.json', row)
    assert row['detected_suppressed_replay'], row
    return row


def main():
    status_path = ROOT / 'science-status-v2.json'
    ...
    try:
        with LOCK.open('r') as lock:
            ...
            pins()
            status['negative_probe'] = negative_probe(lock, checker)
            save(status_path, status)
            refs = read(ROOT / 'science-reference-pins.json')
            for name, scheme in [('standard', 'cpu:1'), ('cpu', 'torch:cpu:1'), ('cuda', 'torch:cuda:0')]:
                ...
                if folder.exists():
                    ...
                    for path, expected in read(ROOT / 'science-reuse-pins.json').items():
                        assert sha(ROOT / path) == expected, path
                    record = read(folder / 'receipt.json')
                    assert record['state'] == 'complete' and record['returncode'] == 0
                    assert record['source_info'] == source_record()
                    assert record['source_status_after'] == ''
                    assert record['input_sha256'] == record['input_sha256_after']
                    assert record['runtime_sha256'] == sha(folder / 'runtime.json')
                    assert record['trigger_sha256'] == sha(folder / 'triggers.hdf')
                    runtime = read(folder / 'runtime.json')
                    expected_environment = checker.fixed_environment(read(ROOT / 'config.json'), SOURCE)
                    assert runtime['state'] == 'complete'
                    assert record['environment'] == runtime['environment'] == expected_environment
                    assert runtime['source'] == runtime['imported_source'] == str(SOURCE)
                    for stage in ('before_executable', 'at_first_bank', 'after_executable'):
                        checker.check(runtime[stage], expected_environment, bank=stage == 'at_first_bank', scheme=scheme)
                    record['qualification'] = qualify(folder, scheme)
                    record['reused_receipt_sha256'] = sha(folder / 'receipt.json')
                ...
                cli = [str(SOURCE / 'bin/pycbc_inspiral'), *cfg['common_args'],
                       ...
                       '--segment-length', '512', '--segment-start-pad', '112', '--segment-end-pad', '16',
                       '--output', str(folder / 'triggers.hdf')]
                qualifier = 'qualify_production_graph_v2.py' if name == 'cuda' else 'qualify-inspiral.py'
                command = ['taskset', '-c', '8', PYTHON, str(ROOT / 'checked-inspiral.py'),
