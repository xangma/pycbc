...
``66789ac4a7468094b0cc3ca1498a1de67e0311f6``. Compare it with restacked main
``f582b6fd250d0b82612492979e01e645d5c07afc`` using normal CPU, Torch CPU and
Torch CUDA as separate arms. The CPU corrections and current results are
separated from the historical unchanged-baseline comparison in
...
Torch routes. Conditioned strain, segment geometry and used PSD bins match
exactly. The full-PSD gate fails for Torch below 30 Hz. The strict controller
...
Current corrected-baseline campaign instructions and configuration are
**PENDING_IMMUTABLE_REPRODUCTION_LINK** and
**PENDING_IMMUTABLE_CONFIGURATION_LINK**. The evidence must retain expanded
...
For a new comparison, prepare the explicitly named corrected baseline and
restacked main commits in separate clean checkouts with matching dependencies.
