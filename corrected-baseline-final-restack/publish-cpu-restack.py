"""Guard and atomically publish the reviewed CPU-based Torch stack."""
import argparse
...
def main():
    ...
    args = p.parse_args()
    updates = json.loads((V / 'restacked-pr-bodies/updates.json').read_text())
    old = {r['number']: r for r in json.loads((V / 'live-prs-before-cpu-restack.json').read_text())}
    ...
    for row, now in zip(updates, current):
        ...
        run(['git', 'merge-base', '--is-ancestor', '66789ac4a7468094b0cc3ca1498a1de67e0311f6', row['new_head']])
    cpu = get_pr(20)
    ...
    completed = []
    for row in updates:
        path = V / 'restacked-pr-bodies' / f'pr-{row["number"]}-request.json'
    ...
    print('PASS: all 15 draft PR heads, dependencies, descriptions and labels published and verified')
