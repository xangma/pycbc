...
WORKTREE = Path('/private/tmp/pycbc-corrected-baseline-docs-20260908')
RUNTIME = Path('/private/tmp/pycbc-cpu-base-restack-20260908')
FROZEN = 'f582b6fd250d0b82612492979e01e645d5c07afc'
...
changed = subprocess.check_output(['git', '-C', str(WORKTREE), 'diff', '--name-only', FORMATTED], text=True).splitlines()
assert changed and all(p.startswith('docs/') for p in changed), changed
import ast
format_paths = subprocess.check_output(['git', '-C', str(WORKTREE), 'diff', '--name-only', FROZEN, FORMATTED], text=True).splitlines()
assert format_paths == ['pycbc/filter/matchedfilter.py', 'pycbc/vetoes/chisq.py'], format_paths
receipt = json.loads((OUT / 'primary-format-review.json').read_text())
for path in format_paths:
    before = subprocess.check_output(['git', '-C', str(WORKTREE), 'show', f'{FROZEN}:{path}'])
    after = subprocess.check_output(['git', '-C', str(WORKTREE), 'show', f'{FORMATTED}:{path}'])
    assert ast.dump(ast.parse(before)) == ast.dump(ast.parse(after)), path
    assert hashlib.sha256(after).hexdigest() == receipt['files'][path]['after_sha256'], path
...
for name in pages:
    ...
    assert 'PENDING_' not in text, f'Final evidence is still pending in {name}'
...
(source / 'index.rst').write_text('Corrected CPU baseline and Torch\n================================\n\n.. toctree::\n   :maxdepth: 2\n\n   torch\n   waveform\n   waveform_plugin\n   install\n   api\n')
...
result = dict(command=command, returncode=code, pages=pages, runtime=provenance,
              documentation_base=FORMATTED, measured_runtime=FROZEN,
              source_mapping='Two complete module ASTs identical excluding locations; all other tracked source files byte-identical; documentation-only changes after formatted main.',
