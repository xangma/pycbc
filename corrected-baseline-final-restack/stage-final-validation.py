"""Archive the completed final-source mapping and documentation validation."""
import hashlib
import json
...
REPO = V.parent / 'cpu-precision-evidence-20260908'
SOURCE = V / 'corrected-baseline-final-restack'


def main():
    review = json.loads((V / 'primary-final-publication-review.json').read_text())
    assert review['status'] == 'PASS'
    out = REPO / 'corrected-baseline-final-restack'
    ...
    if (SOURCE / 'draft-before-restack').is_dir():
        shutil.copytree(SOURCE / 'draft-before-restack', out / 'draft-before-restack',
                        dirs_exist_ok=True)
    for name in ['publication-manifest.json', 'primary-final-publication-review.json',
                 'review-final-publication.py', 'stage-final-validation.py',
                 'prepare-restacked-prs.py', 'publish-cpu-restack.py']:
        shutil.copy2(V / name, out / name)
        files.append(name)
    shutil.copy2(V / 'corrected-baseline-campaign/evidence-publication.json',
                 out / 'benchmark-evidence-publication.json')
    main_head = review['publication_main']
    (REPO / 'cpu-base-restack/README.md').write_text(f'''# CPU prerequisite and final Torch stack validation

The final publication main is `{main_head}`. All 15 PR heads inherit the
...
''')
    sums = {str(p.relative_to(REPO)): hashlib.sha256(p.read_bytes()).hexdigest()
            for p in sorted(REPO.rglob('*')) if p.is_file()
            and '.git' not in p.relative_to(REPO).parts
            and p.name != 'SHA256SUMS.json'}
    (REPO / 'SHA256SUMS.json').write_text(json.dumps(sums, indent=2) + '\n')
    print(json.dumps(dict(final_artifacts=len(files), total_files=len(sums),
                          publication_main=main_head)))
