...
for name in ('candidate-stack-final.json', 'candidate-stack.json', 'publication-final.bundle',
             ...
             'qualify-quality.py', 'qualify-final-v6.py', 'test-cpu-integration-v6.py'):
    inputs[str(PUB / name)] = Q.sha(PUB / name)
for name in ('quality/result.json', 'cpu-integration-v6-tests/result.json'):
...
try:
    for part, old, reviewed in zip(stack['candidates'], prior['candidates'], proof['rows']):
        ...
        key = Q.KEYS[n]
        repo = ROOT / ('quality-final-v6-' + key + '-checkout')
        ...
        assert details == reviewed['files'] and reviewed['after_head'] == part['head']
        ...
        Q.save(OUT / (key + '-reviewed-source.json'), dict(original_review=original_review,
            polish=reviewed, binary_source=str(binary_source), binary_source_head=expected_binary_head,
